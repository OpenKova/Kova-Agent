...
_SESSION_CHAT_TYPE: ContextVar = ContextVar("KOVA_SESSION_CHAT_TYPE", default=_UNSET)
...
# intentionally separate from KOVA_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("KOVA_UI_SESSION_ID", default=_UNSET)
...
_SESSION_PROFILE: ContextVar = ContextVar("KOVA_SESSION_PROFILE", default=_UNSET)
...
_CRON_SESSION: ContextVar = ContextVar("KOVA_CRON_SESSION", default=_UNSET)
...
_VAR_MAP = {
    ...
    "KOVA_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    ...
    "KOVA_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "KOVA_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "KOVA_SESSION_PROFILE": _SESSION_PROFILE,
    "KOVA_CRON_SESSION": _CRON_SESSION,
...
def set_current_session_id(session_id: str) -> None:
    ...
    ``KOVA_SESSION_ID`` for the rest of the process — leaking the child id
...
def set_session_vars(
    ...
    ``os.environ["KOVA_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``KOVA_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``KOVA_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
# value (``telegram``) to KOVA_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind KOVA_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# KOVA_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``KOVA_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("KOVA_PLATFORM") or get_session_env("KOVA_SESSION_PLATFORM", "")
...
def declare_stateless_channel() -> None:
    ...
    See OpenKova/Kova-Agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    or a one-shot runner that exits after its final response (``kova -z``,
    cron — see :func:`declare_stateless_channel`) — and dispatcher-spawned
    Kanban workers (identified by ``KOVA_KANBAN_TASK``), which are one-shot
