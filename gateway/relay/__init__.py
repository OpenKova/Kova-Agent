...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from kova_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Kova Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``kova gateway enroll`` CLI. Two modes, in precedence order:
