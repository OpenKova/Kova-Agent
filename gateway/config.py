...
# dashboard's pre-write mutation validation (kova_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Kova is thinking…"). Platforms with textless indicators (Discord,
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # (e.g. via `kova config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `kova config set gateway.multiplex_profiles true`
