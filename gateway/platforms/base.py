...
# as MPEG-2 Layer II are audio to Kova but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (
        ...
        and raw_message.get("_kova_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``KOVA_SESSION_PLATFORM`` contextvar — that contextvar is
...
def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_kova_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_kova_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from kova_cli.config import load_config_readonly as _load_config
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Kova root.
    ...
    """
    roots: List[Path] = []
    profiles_dir = _KOVA_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("KOVA_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _KOVA_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    for kova_root in (_HERMES_HOME, _KOVA_ROOT):
...
class BasePlatformAdapter(ABC):
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Kova home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Kova home layout.
        """
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_kova_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # KOVA_SESSION_PLATFORM contextvar the tool would
