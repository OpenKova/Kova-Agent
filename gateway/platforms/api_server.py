...
- GET  /v1/models                  — lists kova-agent and any configured model_routes aliases
...
def _kova_version() -> str:
    """Return the canonical Kova Agent version string.

    ``kova_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``kova_agent-*.dist-info`` after a source update until the environment is
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from kova_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("KOVA_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``kova-agent``)
    ...
    ``direct_model_requests`` config value here, while Kova-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Kova-aware and is
    always honored.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Kova-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        :func:`kova_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from kova_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "kova-agent")
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_kova_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _ensure_session_db(self):
        ...
        redirects ``get_kova_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_kova_home())
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "kova_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "kova_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        # kova_cli.model_switch.resolve_effective_model (session /model
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list kova-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Kova provider/model inventory.
        ...
        Kova provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "kova.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Kova-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Kova-Error"] = _redact_api_error_text(err_msg, limit=200)
        ...
        try:
            ...
            async def _emit(item):
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="kova.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Kova-Session-Id header.  Without this,
        ...
        response_headers = {"X-Kova-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Kova-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_kova_api_runtime", {}) or {})
