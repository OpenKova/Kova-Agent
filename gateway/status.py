...
def _get_process_kova_home() -> Path:
    """Return the process-level KOVA_HOME, skipping context-local overrides.
    ...
    ``get_kova_home()`` honors ``_KOVA_HOME_OVERRIDE`` contextvar used for
...
def _canonical_kova_home(path: Path | str) -> Path:
    """Return a stable absolute KOVA_HOME path for persisted identity data."""
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Kova' lifetime
...
def _build_pid_record() -> dict:
    return {
        ...
        # KOVA_HOME-local.  Persist the owning gateway's process home so an
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-KOVA_HOME --replace while
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's KOVA_HOME rather than the replacer's.  Same-home callers omit
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target KOVA_HOME's gateway PID record, and the live
