...
Defenses 1 and 2 (the ``_KOVA_GATEWAY`` guard on ``kova gateway
...
State lives in ``<KOVA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_kova_home() / "gateway" / "restart_loop.json"
