...
(`plugins/observability/nemo_relay/`) and its Kova-owned subscribers.
...
The OpenTelemetry SDK is an optional extra (`pip install 'kova-agent[otlp]'`),
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/kova_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/kova_otel_capture.jsonl --wait 8
