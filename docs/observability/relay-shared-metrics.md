...
Kova includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Kova and does not require `kova plugins enable
observability/nemo_relay`. Kova remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Kova execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `kova-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Kova requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Kova installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Kova process so configured interceptors can
...
Collection remains off unless Kova policy enables it:
...
Kova core owns one Relay host and one isolated Relay session scope per Kova
session. Core lifecycle producers use
`kova_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Kova plugin registration. Shared-metrics marks must
...
```text
Kova turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Kova shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Kova sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Kova used for the logical call, such as
...
New calls use `kova.model_route.count`. The previous
`kova.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `kova.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Kova
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Kova processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `kova.task_run`, parented to
the owning Kova session. The start counter contains only bounded execution
...
provider attempts for the same Kova API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Kova tool-call
...
cancellations. Active task ownership follows the task ID if Kova rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`kova.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Kova
...
tool or plugin names. Kova does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `kova.tool_approval` marks and
...
Successful skill mutations emit `kova.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `kova.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Kova derives reuse and patch-generation
...
are written with atomic replacement. Each package records the Kova version,
...
The install identity is scoped to one `HERMES_HOME`. To reset it, stop Kova
...
Run a real Kova CLI turn against the deterministic local model server:
