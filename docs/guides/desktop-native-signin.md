...
description: "How the Kova Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Kova Desktop app connects to a **gated gateway** (a hosted or
...
For Kova specifically, native sign-in means:
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh) — the loopback-callback
  pattern for provider/MCP OAuth on remote machines.
- [Run Kova with Nous Portal](./run-kova-with-nous-portal)
