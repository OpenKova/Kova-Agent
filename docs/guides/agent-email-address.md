...
This is **not** the same as the [Email gateway adapter](../user-guide/messaging/email), which lets people chat with Kova *by* emailing it (send a mail, get a reply in-thread). This guide is about the agent *operating a mailbox* — reading, searching, composing, and organising mail as part of its tasks. You can run both, ideally on separate accounts.
:::
...
Ask Kova to do this for you — the skill contains the full procedure — or do it manually:
...
The Himalaya path is pull-based: the agent only sees mail when it looks. Add a [cron job](automate-with-cron) so it looks regularly:
...
Every 15–30 minutes is plenty for most uses. If you need real replies-in-thread with sub-minute latency, use the [Email gateway adapter](../user-guide/messaging/email) instead, which holds a persistent IMAP connection.
...
- [Himalaya skill reference](../user-guide/skills/bundled/email/email-himalaya) — full command set the agent uses
- [Email gateway adapter](../user-guide/messaging/email) — chat with Kova over email instead
- [Automate with Cron](automate-with-cron) — scheduling patterns
- [Security](../user-guide/security) — the wider prompt-injection and credential-handling picture
