...
# → Kova opens or prints an accounts.x.ai verification URL
...
On servers, containers, browser-only consoles (Cloud Shell, Codespaces, EC2 Instance Connect), or SSH sessions where Kova cannot open a browser locally, Kova prints the xAI verification URL and user code. Open the URL in any browser on your laptop or in the cloud console, enter the code if prompted, and Kova will keep polling until xAI approves the login. No SSH tunnel or local callback listener is required.
...
The same device-code flow applies when you sign in from the web dashboard or the desktop app: Kova shows the verification URL and user code, then polls in the background until you approve access.

## How the Login Works

1. Kova requests a device code from `auth.x.ai`.
2. You open the verification URL, sign in, enter the displayed code if prompted, and approve access.
3. Kova polls xAI until approval, then saves tokens to `~/.kova/auth.json`.
...
```bash
kova config set model.default grok-4.6
...
Device-code approval has a finite expiry window (xAI sets `expires_in` on the device-code response, typically on the order of tens of minutes). If you do not approve the login in time, Kova raises a timeout error.

**Fix:** re-run `kova auth add xai-oauth` (or `kova model`). The flow starts fresh.

### Logging in from a remote server

On SSH or container sessions Kova prints the verification URL and user code instead of opening a browser. Open that URL in a browser on your laptop or in a cloud console — no SSH port forward is needed for xAI Grok OAuth.

```bash
kova auth add xai-oauth --no-browser
```

For loopback-redirect providers (Spotify, MCP servers), see [OAuth over SSH / Remote Hosts](./oauth-over-ssh).
...
This is **not** a stale-token problem — re-running `kova model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/OpenKova/Kova-Agent/issues/26847)).
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh) — SSH tunnels for loopback-redirect providers (Spotify, MCP); xAI uses device code and does not need a tunnel
- [AI Providers reference](../integrations/providers)
- [Environment Variables](../reference/environment-variables)
- [Configuration](../user-guide/configuration)
- [Voice & TTS](../user-guide/features/tts)
