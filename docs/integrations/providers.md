...
| **OpenAI Codex** | `kova model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth, uses Codex models) |
...
| **xAI Grok OAuth (SuperGrok)** | `kova model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth) |
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Kova marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `kova auth add openai-codex` (or `kova model` → **ChatGPT or Codex Subscription**) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
| **OpenAI Codex — ChatGPT plan OAuth** | ✅ Yes — `kova model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth device-code login, uses Codex models) | *Not currently documented* | *Not currently documented* | Docs cover auth and token refresh only; plan-quota semantics are not yet documented |
...
**xAI Web Search backend.** When the [Web Search](../user-guide/features/web-search) toolset is enabled, `web.backend: xai` routes search through xAI's hosted search endpoint using the same `XAI_API_KEY` / OAuth credentials. No additional setup required if xAI is already configured as a provider.
...
kova model   # → "Google Vertex AI" → project → region → model
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Kova lazy-installs `google-auth` on first use; run `kova setup` if the managed install needs repair. See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.
...
If you use LM Studio's Just-In-Time loading / Auto-Evict feature and want LM Studio to manage model loading and eviction from normal chat requests, skip Kova' explicit preload step:

```bash
kova config set model.lmstudio_load_mode jit
```

Set it back to the default explicit preload behavior with:

```bash
kova config set model.lmstudio_load_mode explicit
```
