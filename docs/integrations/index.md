...
- **[AI Providers](/integrations/providers)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Kova auto-detects capabilities like vision, streaming, and tool use per provider.
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `kova slack manifest --agent-view` generates |
...
- **[Buzz](/integrations/buzz)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Kova as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Kova identity server-side, or the native gateway platform joins Buzz channels with full Kova memory/skills/approvals/cron. The overview page compares all three.
...
- **[Build a Plugin](/developer-guide/plugins)** — Step-by-step guide for creating Kova plugins with tools, hooks, and CLI commands.
