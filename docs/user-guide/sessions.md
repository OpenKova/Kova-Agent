...
option first: `kova sessions optimize` merges FTS5 index segments and
...
kova --resume latest --in ./my-project

# Works with the TUI too
kova --tui --resume latest --in ./my-project
```
...
```bash
kova --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`kova sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
kova sessions list --workspace my-project
kova sessions list --workspace ~/code/kova-agent
```
...
`kova sessions export` is one surface for every export format, selected with `--format`:
...
kova sessions export backup.jsonl --redact
```
...
kova sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
kova sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
kova sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
kova sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
kova sessions export --format trace

# One session to a local trace file
kova sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
kova sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.kova/session-exports`).

```bash
# Export one session to Markdown
kova sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
kova sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
kova sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
kova sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
kova sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
kova sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
kova sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
kova sessions prune --newer-than 5h

# Explicit window with absolute timestamps
kova sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
kova sessions prune --source telegram
kova sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
kova sessions prune --newer-than 5h --title "smoke test"   # title substring
kova sessions prune --older-than 30 --max-messages 3        # tiny sessions
kova sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
kova sessions prune --model gpt-5 --older-than 1w           # by model (substring)
kova sessions prune --provider openrouter --older-than 60   # by billing provider
kova sessions prune --branch feature/old-experiment         # by git branch
kova sessions prune --user 12345678 --chat-type group       # by messaging origin
kova sessions prune --max-tokens 500 --older-than 7         # by token usage
kova sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
kova sessions prune --newer-than 5h --dry-run
...
the implicit 90-day default, so `kova sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `kova sessions prune` keeps the 90-day cutoff. Every
...
`kova sessions archive` takes the same filters as `prune` but soft-hides
...
kova sessions archive --newer-than 5h

# Archive by title substring, preview first
kova sessions archive --title "dry run" --dry-run
kova sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `kova sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`kova sessions list` and `/resume` but remain in the database and can be
...
`kova sessions repair-routing` finds message-bearing session rows with no
...
kova sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
kova sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.kova/state.db ~/.kova/state.db.bak`).
...
| Gateway routing index | `gateway_routing` table in `~/.kova/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.kova/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.kova/sessions/sessions.json` is a **legacy mirror** of it,
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Kova process in the same `HERMES_HOME`
