...
:::tip
Kova also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `kova --tui` — see the [TUI](tui) guide.
:::
...
kova --resume latest        # Resume the most recent session (same as -c)
kova --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir
...
The `kova plugins` commands manage native Kova plugins and portable Agent
...
kova plugins disable <plugin-name>
kova plugins update <plugin-name>
kova plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Kova currently
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.kova/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
The first time you press Enter while Kova is working, Kova prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::
...
kova --resume latest                     # Resume the most recent session (same as -c)
kova --resume latest --in ./my-project   # Latest session for ./my-project's workspace
