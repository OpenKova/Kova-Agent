...
The TUI is the modern front-end for Kova — a terminal UI backed by the same Python runtime as the [Classic CLI](cli). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.
...
The classic CLI remains the shipped default. Anything documented in [CLI Interface](cli) — slash commands, quick commands, skill preloading, personalities, multi-line input, interrupts — works in the TUI identically.
...
Contributors who run `kova --tui --dev` from many worktrees can share one `node_modules` instead of installing per checkout — see [TUI & Desktop from Worktrees](../developer-guide/worktree-ui-dev).
:::
...
<img alt="Kova TUI Session Orchestrator with one live session and a +new row" src="/docs/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/docs/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="Kova TUI Session Orchestrator demo" style={{maxWidth: '100%'}}></video>
...
If you want multiple surfaces to share one set of sessions, use the shared `~/.kova/state.db` (see [Sessions](sessions)) or the web dashboard's embedded chat (see [Web Dashboard](features/web-dashboard.md#chat)) — not a hand-set gateway URL.
