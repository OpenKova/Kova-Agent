...
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `kova setup` or `kova model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles) / [`configuration.md`](./configuration). If you ever see an empty string in `config.yaml`, run `kova model` (or click **Change** in the dashboard) and Kova will write the dict form for you.
:::
...
![Models page overview](/assets/docs/dashboard-models/overview.png)
...
![Model picker dialog](/assets/docs/dashboard-models/picker-dialog.png)
...
![Auxiliary panel expanded](/assets/docs/dashboard-models/auxiliary-expanded.png)
...
|---|---|
| **Title Gen** | When title latency or cost matters more than matching the main model. Pin a known-good flash model, or set `auxiliary.title_generation.prefer_fast_model: true` to let Kova choose the provider's fast tier. |
...
![Use as dropdown](/assets/docs/dashboard-models/use-as-dropdown.png)
...
Edit `~/.kova/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration) for the full schema.
