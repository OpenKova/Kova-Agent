...
Kova has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `kova profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `kova profile update` pulls new versions | Re-send the file |
...
| Made with | `kova profile install` / `update` | `/export` and `/import`, or `kova profile export` / `import` |
...
```bash
kova profile export research-bot
kova profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
kova profile import ./research-bot.tar.gz
kova profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `kova -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.kova`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.kova`) is exported through an allow-list — only known Kova artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.kova/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
- [`kova profile export` / `import`](../reference/profile-commands.md#kova-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Kova](../guides/use-soul-with-kova) — authoring personalities
