...
| `cron_mode` | `deny` | How [cron jobs](./features/cron) behave headlessly when they trigger a dangerous-command prompt. `deny` blocks the command (the agent must find another path); `approve` auto-approves everything in cron context. |
...
Do not ask the agent to `patch` `~/.kova/cron/jobs.json` directly. Use the `cronjob` tool, [`kova cron`](./features/cron), or `/cron` — they update the job store through the supported API. The same applies to other Kova control files when write safety blocks direct edits.
