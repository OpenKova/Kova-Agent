...
- **CLI** (`kova`) and **[TUI](./tui)** (`kova --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard)** (`kova dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Kova Desktop](../getting-started/installation).
...
Kova Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Kova sessions remain available.
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Kova to open PR**. You can also create and switch branches from here.
...
Talk to Kova and hear it back, the same [voice mode](./features/voice-mode) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Skills** — browse, install, and manage [skills](./features/skills).
...
- **Profiles** — switch between [Kova profiles](./profiles) (isolated config/skills/sessions).
- **Messaging** — set up gateway channels.
- **Agents** and **Command Center** — orchestration surfaces for multi-agent work.

### Keyboard & navigation

- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Kova, and more.
...
- **Remote gateway** — enter the URL of a `kova serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **Kova Cloud** — sign in once to Kova Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
:::info The remote backend is a running `kova serve` process
"Remote backend" means a **`kova serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `kova serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
kova serve --host 0.0.0.0 --port 9119
```

Keep that `kova serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `kova serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index) for gateway setup.
...
The remote gateway host is configured per [profile](./profiles), so each profile can point at its own remote backend (or stay on its local one). Switching profiles switches which remote host the app connects to.
:::
...
reference. (This is separate from the [web dashboard plugin system](./features/extending-the-dashboard).)
...
certificate once and tell Kova to use it:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Kova Local Signing`, Identity Type: *Self-Signed Root*,
   Certificate Type: **Code Signing**.
3. `kova config set desktop.macos_signing_identity "Kova Local Signing"`
...
`tccutil reset All com.nousresearch.kova` and re-grant.

## See also

- [CLI Guide](./cli) — the terminal interface
- [TUI](./tui) — the modern terminal UI used by `kova --tui` and the dashboard chat tab
- [Web Dashboard](./features/web-dashboard) — browser admin panel with an embedded chat tab
- [Configuration](./configuration) — config that the desktop app reads and writes
- [Windows (Native)](./windows-native) — native Windows install path
