...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Kova MCP catalog. The
...
Two halves, in this order: the editor side must be up before Kova connects.
...
`~/.kova/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Kova — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Kova connects
...
2. Start the Kova session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Kova
they appear as:
...
then happens at `kova mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Kova runs batched calls
...
- **Start order matters.** Editor + server up first, then the Kova
...
- **Timeouts on long operations.** Kova' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.kova/config.yaml` for
