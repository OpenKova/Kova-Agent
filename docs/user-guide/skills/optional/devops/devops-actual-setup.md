---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Kova"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Kova"
---
...
Set up Actual Computer (actual.inc) inference in Kova.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `kova skills install official/devops/actual-setup` |
| Path | `optional-skills/devops/actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + Kova Agent |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Kova inference
...
- User has an `ac_` key and wants Kova routed through their Actual cluster.
...
- Kova has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Kova — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.kova/.env`.
...
3. Select provider + model:
   ```bash
   kova config set model.provider actual
   kova config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   kova chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Kova at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.kova/.env`, then:
   ```bash
   kova config set model.provider actual
   kova config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   kova chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Kova since the first-class provider).**
   ...
   request still 400s this way on an old Kova, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Kova' default toolset
   ...
   servers emit a bare `data: [DONE]` — Kova reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Kova the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
kova chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
kova chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
