...
| List/filter by metadata (age, source, cost, tokens, workspace) | `kova sessions list` / `stats` via terminal |
| Rename | `kova sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `kova sessions archive <filters>` |
| Delete (destructive) | `kova sessions delete` / `kova sessions prune <filters>` |
| Export before deleting anything valuable | `kova sessions export --session-id <id> --format md` |
...
`kova sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `kova sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `kova sessions list`
