...
kova profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
kova profile import coder.tar.gz   # install an archive as a new profile
```
...
- `HERMES_HOME` is the profile boundary. It controls Kova config, `.env`,
...
kova profile export coder
kova profile import ./coder.tar.gz --name coder
```
...
Use an export file for a one-time handoff or a move; use a distribution for an agent you'll keep shipping. See **[Profile Distributions: Share a Whole Agent](./profile-distributions)** for both — the comparison table, authoring, publishing, update semantics, and the security model.
