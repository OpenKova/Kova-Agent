...
Prerequisites: see [Microsoft Teams](./teams) for the underlying bot/credential setup.
...
The bind host is read from the platform's `extra.host` in `config.yaml` (there is no `MSGRAPH_WEBHOOK_HOST` env var — see the [webhook listener reference](msgraph-webhook)).
