...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.kova/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.
...
- **Outbound attachments are supported.** Kova sends images, voice
  ...
  media.
- **Native polls are supported.** Kova sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Kova sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `kova send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<kova-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Kova is set
