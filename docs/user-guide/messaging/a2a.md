...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Kova A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Kova over HTTP.

It interoperates with any A2A-compliant peer — another Kova, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Kova ↔ Kova across machines** — let your desktop agent hand tasks to a Kova on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Kova so other frameworks' agents can send it tasks.

When you want multiple agents on the **same machine**, prefer [delegation](../features/delegation) (in-process subagents) or the [kanban board](../features/kanban) (durable multi-profile work queue) — A2A is for crossing process/machine/framework boundaries.

## Enable

```bash
kova gateway setup      # pick A2A
```
...
The outbound client tools ship as the `a2a` toolset, **off by default** — enable it with `kova tools`.
...
With the platform enabled, Kova serves:
...
- **Audit log** — every exchange appends to `~/.kova/a2a_audit.jsonl`.
