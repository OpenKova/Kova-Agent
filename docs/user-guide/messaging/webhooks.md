...
For dynamic subscriptions, add the `toolsets` key by editing `~/.kova/webhook_subscriptions.json` directly:
...
- `kova webhook subscribe` deliberately does **not** accept a toolsets flag. Granting elevated tools is a manual config-file edit, so an agent creating its own subscription at runtime cannot self-grant `terminal`.
