...
Tool calls in the `output` array were already executed server-side by the Kova agent — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.
...
enumerate every authenticated provider/model combination Kova can route to,
and it does not do pricing or capability enrichment.

### GET /api/model/options

Kova-aware clients can request the same curated provider/model inventory used
...
Normal opens are intentionally conservative for custom providers: Kova probes
...
authenticated UI needs the richer Kova-specific picker metadata.
...
Authenticated clients can override Kova' default model selection per request
by sending:

- `model` — the target model id for this turn
- `provider` — the Kova provider slug to resolve credentials/runtime for this turn
...
alias, Kova rejects the request with `400` instead of silently remixing route
...
Requests that include an explicit `provider` — and the Kova-native
...
- Requests to `/p/<profile>/v1/...` must present that profile's own
  `API_SERVER_KEY` (from `~/.kova/profiles/<profile>/.env`). The default
...
- **Simple OpenAI clients still see an alias** — `/v1/models` advertises the
  stable Kova alias (`kova-agent` or the active profile name). Richer
