...
**→ [Build a Kova Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.
...
```bash
kova plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Kova checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `kova plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`kova plugins install <source> --force --ref <new-commit>`. The
...
Kova tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Kova runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Kova never
materializes the prompt on another surface.

Kova still owns hardline blocks, sudo-stdin protection, user deny rules,
...
Plugins can register the 26 lifecycle events currently accepted by `kova_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Kova Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Kova Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Kova Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
kova plugins search <term>                 # search the community plugin index
kova plugins install <name>                # install by index name (resolved to repo @ pinned ref)
...
kova plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
...
kova plugins capabilities [my-plugin]      # declared vs granted capabilities
```
...
When a plugin declares capabilities, `kova plugins install` (and
`kova plugins enable`) shows the list with one-line risk descriptions and
...
granted, `kova plugins update` surfaces the additions and asks again. New
...
`kova plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
kova plugins capabilities             # all plugins with declared/granted capabilities
kova plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Kova has not reviewed the plugin's code. Only install
...
`kova plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
kova plugins search telegram               # fuzzy search
kova plugins search                        # browse the whole index
kova plugins search --capability platform  # filter by declared capability
kova plugins search media --json           # machine-readable output
kova plugins search --refresh              # bypass the 24h local cache
```
...
```bash
kova plugins install kova-media-studio
```
...
(`https://raw.githubusercontent.com/NousResearch/kova-plugin-index/main/index.json`,
overridable via `kova config set plugins.index_url <url>`). Fetches are
cached under `~/.kova/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Kova — so search works fully offline.
...
```json
{
  ...
  "repo": "NousResearch/kova-media-studio",
  "ref": "<40-char commit SHA>",
  "subdir": null,
  "homepage": "https://github.com/NousResearch/kova-media-studio",
...
(`kova_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[kova-plugin-index](https://github.com/NousResearch/kova-plugin-index)
...
A **plugin pack** is a declarative, shareable YAML file (`kova-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: kova-media-studio            # bare community-index name…
...
```bash
kova plugins pack show ./kova-pack.yaml     # dry-run review
kova plugins pack install ./kova-pack.yaml  # review → confirm → install
kova plugins pack export > kova-pack.yaml   # snapshot the current install
kova plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `kova plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `kova plugins install`.
...
provenance (installed via `kova plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`kova skills`). Wiring
...
- Kova reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Kova rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Kova can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Kova drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Kova checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Kova' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
