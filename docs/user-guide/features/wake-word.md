...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey kova"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey kova"** — a model for it ships with Kova, so
...
```bash
cd ~/.kova/kova-agent && uv pip install -e ".[wake]"
```
...
wake_word:
  ...
  openwakeword:
    model: hey_kova         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
default) to let Kova choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Kova installs
...
"Hey Kova" works out of the box — the bundled openWakeWord model
(`model: hey_kova`) is the default. To wake on something else, the easiest
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.kova/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
