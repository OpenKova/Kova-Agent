...
guard is active only for dispatcher-spawned workers (`KOVA_KANBAN_TASK` is
set) and can be disabled with `KOVA_KANBAN_STOP_NUDGE=0`.
...
kova kanban notify-list [<id>] [--json] [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
...
[`merge-reconciler` skill](https://github.com/OpenKova/Kova-Agent/blob/main/skills/autonomous-ai-agents/merge-reconciler/SKILL.md)
