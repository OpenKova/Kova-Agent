...
- Pets are installed into your profile's `pets/` directory
  (`<HERMES_HOME>/pets/<slug>/`), so each [profile](../profiles) keeps its
  own set.
...
Beyond installing pre-made pets from the gallery, Kova can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `kova pets` → the generate flow.
...
- The petdex npm CLI installs to `~/.codex/pets`; Kova uses its own
  profile-scoped `<HERMES_HOME>/pets/` instead — install through `kova pets`.

## See also

- The [`kova-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kova-agent)
