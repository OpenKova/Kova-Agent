...
The peer model above covers CLI, TUI, and desktop sessions, where every conversation resolves to `peerName`. The [gateway](../../developer-guide/gateway-internals) adds a second axis: users arrive with platform-native runtime IDs (Telegram UID, Discord snowflake, Slack user), and three keys decide which peer each ID resolves to.
...
**Switching modes:** Re-run `kova memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
Then run `kova memory setup` and enter the API key printed by the local
