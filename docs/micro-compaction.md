...
has to be thrown away or summarized. Kova has always done this in one batch:
...
Kova folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Kova's normal session flush is
...
With `micro_compact` unset or `false` Kova behaves exactly as it always has:
