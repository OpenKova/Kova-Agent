...
- `kova_cli/runtime_provider.py` — credential resolution, custom-endpoint runtime resolution
...
If you are trying to add a new first-class inference provider, read [Adding Providers](./adding-providers) and the [Model Provider Plugin guide](./model-provider-plugin) alongside this page.
...
Set `AI_GATEWAY_API_KEY` in `~/.kova/.env` and run with `--provider ai-gateway`. Kova fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

Kova contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
4. **Config flow**:
   - CLI: reads the fallback chain via `kova_cli/fallback_config.get_fallback_chain()` → passes to `AIAgent(fallback_model=...)`
