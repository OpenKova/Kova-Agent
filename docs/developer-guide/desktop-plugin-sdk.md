...
description: "Extend the native Kova Desktop app â€” panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Kova Desktop](/user-guide/desktop) app is contribution-driven: every
surface in the window â€” panes, routes, sidebar nav, status-bar items, palette
entries, keybinds, themes â€” registers into one central registry. Core registers
...
It imports one module â€” `@kova/plugin-sdk` â€” and gets everything: the app's
...
desktop app** (`kova desktop`) SDK â€” the `@kova/plugin-sdk` module and
...
- **`host.state.*`** â€” readonly views over the app's live state (nanostore
  atoms): active session, cwd, gateway status, model, profile, viewport.
- **`host.*` actions** â€” curated safe verbs: toast, navigate, tail logs,
  restart the gateway, subscribe to the gateway event stream.
- **`host.request`** â€” the gateway JSON-RPC door: sessions, config, skills,
  cron â€” everything the app itself calls.
- **`ctx.rest` / `ctx.socket`** â€” your plugin's own backend namespace
  (`/api/plugins/<id>`) if you ship a `plugin_api.py`.
- **`ui.*`** â€” the design language: the app's real components, theme variables,
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$HERMES_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none â€” plain ESM, loaded uncompiled |
| **Unified package** | `$HERMES_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none â€” same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `KovaPlugin` contract, appear in **Settings â†’ Plugins**,
and enable/disable live. A unified package is just the disk door scanning inside
your agent plugin's folder â€” see
...
differences. No desktop plugins ship in the core tree today â€” reference demos
...
and hot-reloads every later save in place. If it doesn't appear, run âŒ˜K â†’
**Reload desktop plugins**. If loading fails, a toast names the error â€” fix and
save again.
...
`react/jsx-runtime` â€” everything else fails to resolve, on purpose.
:::
...
interface KovaPlugin {
  /** Stable slug â€” becomes the `plugin:<id>` source and the id namespace. */
  id: string
  /** Human name for Settings / about UI. Defaults to `id`. */
  name?: string
  /** Registers on load when the user hasn't chosen (default true). Set false
   *  for opt-in plugins: they inventory in Settings â–¸ Plugins, off until the
...
directly â€” the context auto-tags provenance (`source: 'plugin:<id>'`) and
...
## Contribution areas â€” the cookbook
...
| âŒ˜K palette | `PALETTE_AREA` | `data: PaletteContribution` |
...
A pane is a tile in the layout tree. `placement` is the semantic role â€” the pane
...
specific **edge** instead of stacking, add a `dock` gesture â€” the same thing as
...
be re-enabled from **Settings â†’ Plugins**. When a plugin contributes multiple
...
`StatusbarItem` (`{ id, label?, icon?, detail?, variant?, menuItems?, â€¦ }`).
...
colors, â€¦). It appears in the theme picker like a built-in.
...
readonly â€” read with `.get()` in handlers, subscribe with `useValue(atom)` in
components.

```ts
host.state.activeSessionId  // ReadableAtom<string | null>
host.state.cwd              // ReadableAtom<string>
host.state.gateway          // ReadableAtom<string>  ('idle' | 'connecting' | 'open' | â€¦)
...
ctx.os.openExternal(url)                   // OS default handler (browser, mail, spotify:) â†’ Promise<boolean>
ctx.os.revealPath(path)                    // reveal in Finder / Explorer â†’ Promise<boolean>
ctx.os.writeClipboard(text)                // system clipboard â†’ Promise<boolean>
...
host.request<T>(method, params?)           // gateway JSON-RPC â€” the real power
```

`host.request` is the same JSON-RPC the app itself uses (sessions, config, skills,
cron, kanban, â€¦). Profile-shaped plugins get first-class methods too:
...
`clone_all`, `no_skills`, `soul`, optional `model` + `provider` pin) â€” the
ws twins of the dashboard's `/api/profiles` REST routes.
`host.onEvent` streams live gateway events (message deltas,
session lifecycle, tool activity). Listeners are isolated â€” a throw in your
...
`ctx.os` is the curated OS door â€” every way a plugin reaches outside the app
window, in one namespace attributed to your plugin. `ctx.os.notify` posts a
**native OS notification** â€” the same Electron pipeline the app's own
...
they're looking at the app. Users can silence it per device under Settings â–¸
Notifications â–¸ "Plugin notifications", and repeats from the same plugin are
throttled, so treat it as a signal for genuinely notable events â€” not a log.
...
shell, plain browser) â€” branch on the result rather than sniffing the bridge.

## Data layer â€” React Query + nanostores

Plugins share the app's single `QueryClient`, so plugin queries cache, dedupe,
poll, and invalidate exactly like core screens â€” never hand-roll a fetch loop.
...
`computed` â€” the same primitive `host.state` uses. Subscribe in the leaf that
...
`useI18n` (localized copy â€” your plugin stays translatable), and
`evaluateRuntimeReadiness`.

**Style with theme variables, never hardcoded colors.** Panes already sit on the
app's editor background â€” leave the background alone and use vars for everything
...
through `ctx.rest` / `ctx.socket` â€” a namespace scoped to your plugin **by
construction**.
...
desktop app also scans `$HERMES_HOME/plugins/<id>/` â€” the regular agent-plugin
root â€” for a `desktop/plugin.js`, and loads it through the exact same pipeline
as the standalone disk door (hot reload included):

```
~/.kova/plugins/<id>/           # ONE installable folder
â”œâ”€â”€ plugin.yaml                   # the agent half: tools, hooks, commands
â”œâ”€â”€ skills/â€¦
â”œâ”€â”€ dashboard/
â”‚   â”œâ”€â”€ manifest.json             # { "name": "<id>", "api": "plugin_api.py" }
â”‚   â””â”€â”€ plugin_api.py             # backend routes â†’ /api/plugins/<id>/
â””â”€â”€ desktop/
    â””â”€â”€ plugin.js                 # the desktop half: panes, commands, ctx.rest
```

The `desktop/plugin.js` half is an ordinary disk plugin â€” same contract, same
imports, same `ctx.rest('/â€¦')` reaching the `plugin_api.py` sitting beside it.
...
desktop half ships opt-in â€” it inventories in **Settings â†’ Plugins** but stays
disabled until the user toggles it â€” matching the Python half's
...
off â€” `ctx.rest` returns errors, not crashes.

:::note
The scan is local to the machine the desktop app runs on. Against a remote
backend, the remote box's `~/.kova/plugins` is not reachable as a filesystem â€”
...
~/.kova/plugins/<id>/
â””â”€â”€ dashboard/
    â”œâ”€â”€ manifest.json      # { "name": "<id>", "api": "plugin_api.py" }
    â””â”€â”€ plugin_api.py      # exports `router = APIRouter()`
```
...
Routes mount under `/api/plugins/<id>/` (`GET /api/plugins/<id>/board`, â€¦).
Backend code runs inside the gateway process, so it can import from the
kova-agent codebase directly (`kova_state`, `kova_cli.config`, â€¦). See
[Extending the Dashboard â†’ Backend API routes](/user-guide/features/extending-the-dashboard#backend-api-routes)
for the full backend reference â€” the mount is identical.

:::caution The Python backend is gated separately
Enabling a plugin in the desktop **Settings â†’ Plugins** panel is a renderer-side
...
on OAuth remotes** (single-use WS tickets are core-managed) â€” treat the socket as
...
Every plugin â€” enabled or not â€” inventories in **Settings â†’ Plugins**, where the
user toggles it live (no app restart), reveals its folder, or rescans. The user's
choice is remembered:

- No choice yet â†’ the plugin's own `defaultEnabled` (default `true`). Set
  `defaultEnabled: false` to ship an opt-in plugin that stays dark until the user
  flips it on.
- Explicit choice â†’ persisted and honored across restarts. A disabled plugin
  stays disabled â€” don't fight it; the user turned you off.
...
export a `KovaPlugin`). It's discovered by `discoverBundledPlugins()` at boot â€”
no import, no registry edit â€” and shares the exact inventory + live
...
2. It's still lint-fenced to `@kova/plugin-sdk` + `react` only â€” no `@/â€¦` app
   internals.
...
authority** â€” the React singleton, the whole SDK (`host.request` gateway RPC,
...
This is acceptable for **local** sources â€” a disk file can already run code on
your machine â€” which is why the disk door only loads local files you (or your
agent) wrote. The optional `integrity` (`sha256-â€¦`) check only proves the bytes
...
- **JSX won't parse in a disk plugin.** The file loads uncompiled â€” use `jsx()` /
...
- **Reference only what you imported.** A component you forgot to import (e.g.
  `StatusDot`) is a `ReferenceError` at render â€” double-check every identifier in
  your `jsx()` calls appears in the import line.
- **Read state imperatively in handlers** (`$atom.get()`), never from a render
  closure â€” rapid events will otherwise see stale values. Subscribe (`useValue`)
  only in the leaf that renders the value.
- **Canvas panes must track their container** with a `ResizeObserver` and resize
  the canvas (width/height attributes, not just CSS) â€” panes resize constantly.
...
| Plugin contract | `KovaPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `Contribution` |
...
**`kova-desktop-plugins`** skill â€” it carries the same contract as this page in
...
export `id`. Run âŒ˜K â†’ **Reload desktop plugins**. Check the app for an error
...
**`ctx.socket` never fires.** On an OAuth remote it's a no-op by design â€” use your
