...
Plugins can launch and supervise fresh Kova child sessions without importing
...
active agent turn fails closed with `No active Kova parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Kova can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Kova's existing unsafe-tool block remains enforced.
