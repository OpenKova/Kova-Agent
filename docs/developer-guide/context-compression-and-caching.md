...
| `codex_app_server_auto` | `native` | `native`, `kova`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
```bash
kova config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Kova' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Kova
  records the resulting compaction events (compression counters, session
  events). Set `kova` to let Kova' compression threshold initiate
  app-server compaction, or `off` to disable Kova-initiated automatic
  compaction entirely (codex may still compact natively).

Kova' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Kova' summary compressor.
...
encrypted `compaction` output item. Kova captures that item into the
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Kova can avoid; user-facing docs for `/model`, fallback providers, and
