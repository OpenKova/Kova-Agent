...
Kova can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Kova already owns. It does not replace native `plugin.yaml` plus
...
```bash
kova plugins install owner/repository --no-enable
kova plugins list
...
Kova validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Kova.
...
Portable `streamable-http` entries are routed through Kova' existing native
...
same full-trust posture as other installed Kova plugins.
...
records it as Published. Kova keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Kova does not expose a
...
continue to work after a normal Kova upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Kova inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Kova releases can therefore load a plugin whose manifest contains
...
Kova enforces this contract with frozen external-plugin fixtures discovered
...
`kova plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Kova itself. It reports invalid hook names, callbacks that do
...
`llm.task_override` (see `kova_cli/plugin_capabilities.py` for the
...
this Kova understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Kova validates them, and `kova plugins install` / `kova plugins doctor` surface missing ones with a `pip install` hint, but Kova **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Kova' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/OpenKova/Kova-Agent/issues/64165) and
[#15220](https://github.com/OpenKova/Kova-Agent/issues/15220). Plugin
...
Use plugin-relative config keys for user-visible behavior. Kova resolves them
...
- **Wrong directory layout:** Native packages use `~/.kova/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
does not ship with Kova core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `kova plugins list`
...
loader, the plugin is disabled and Kova continues. The gate exists
...
You normally never edit this key by hand. `kova plugins enable <name>`
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `kova_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Kova sends them. |
