...
├── kova_constants.py       # HERMES_HOME, profile-aware paths
...
→ [Plugin Guide](/developer-guide/plugins), [Memory Provider Plugin](./memory-provider-plugin)
...
→ [Trajectories & Training Format](./trajectory-format)
...
| **Profile isolation** | Each profile (`kova -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
