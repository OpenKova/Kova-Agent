...
This page covers the architecture of the egress credential-injection firewall (`kova egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy).
...
kova_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on KOVA_RUN_E2E=1).
...
kova egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.kova/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.kova/bin/iron-proxy (atomic).
...
KOVA_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `kova egress`
HERMES_HOME=/tmp/kova-egress-test python3 -m kova_cli.main egress --help
HERMES_HOME=/tmp/kova-egress-test python3 -m kova_cli.main egress setup --help
```
...
- User-facing setup + troubleshooting: [Egress proxy](https://kova-agent.nousresearch.com/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://kova-agent.nousresearch.com/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`kova secrets bitwarden`](https://kova-agent.nousresearch.com/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`kova egress`](https://kova-agent.nousresearch.com/docs/reference/cli-commands#kova-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://kova-agent.nousresearch.com/docs/reference/environment-variables#egress-proxy-sandbox-injected)
