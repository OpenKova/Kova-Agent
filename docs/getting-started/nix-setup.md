...
For a supported setup, use one of the standard [installation](./installation) paths - either Docker or an FHS environment.
:::
...
After `nix profile install`, `kova`, `kova-agent`, and `kova-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation) — `kova setup` walks you through provider selection, `kova gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.kova/`.
