...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in kova_state.py — kept local
# to avoid importing kova_state at module load time (its module-level
# DEFAULT_DB_PATH = get_kova_home() / "state.db" breaks tests that
# monkeypatch get_kova_home to return a str).
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.kova/prefill.json incident, 20/20 blocked with
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.kova.ai")
    )
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Kova' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
def run_conversation(
    ...
    # Adopt any ~/.kova/.env credential/base-url edits made since the last
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from kova_cli.lifecycle import (
                        has_hook,
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.kova.ai")
                ...
                if getattr(api_error, "status_code", None) == 404:
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `kova model`."
                        )
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Kova may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See kova-lcm engine
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.kova.ai", force=True)
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                # timeout (KOVA_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from kova_cli.plugins import get_pre_verify_continue_message
