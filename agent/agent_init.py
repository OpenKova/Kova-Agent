...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .kova.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / HERMES_HOME
    ...
    elif agent.provider in {"nous", "nous-portal"}:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from kova_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = kova_xai_default_headers()
    ...
    try:
        from kova_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `kova --resume <id>` can restore the bypass (CLI resume paths read
    ...
    # management API (the historical Kova behavior) or left to LM Studio's
    ...
    if codex_app_server_auto_compaction not in {"native", "kova", "off"}:
        _ra().logger.warning(
    ...
    # Nous Kova 3/4 are chat models, not tool-call-tuned. The interactive
