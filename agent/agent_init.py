...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$KOVA_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$KOVA_HOME`` just means the notice
...
def init_agent(
    ...
    # Accept the legacy "kova" value for backward compatibility with configs
    # written before the rename; "kova" and "kova" behave identically.
    if codex_app_server_auto_compaction == "kova":
        codex_app_server_auto_compaction = "kova"
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            ...
            if _kova_warn:
                _user_msg = (
                    "⚠ Kova models are NOT agentic — they "
