...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Kova process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("KOVA_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid KOVA_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
