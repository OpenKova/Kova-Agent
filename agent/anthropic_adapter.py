...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in kova_cli/runtime_provider.py.
    """
...
def _is_nous_portal_endpoint(base_url: str | None) -> bool:
    ...
    ``https://inference-api.kova.ai/v1/messages``.  Portal-specific
    behaviours key off this: Bearer JWT auth, verbatim catalog model ids,
    and native thinking-signature replay.

    Trusted hosts only:

    1. Prod hostname ``inference-api.kova.ai``
    2. The operator-set ``NOUS_INFERENCE_BASE_URL`` hostname (staging/preview)

    Lookalikes such as ``inference-api.kova.ai.attacker.test`` are
    rejected (hostname match, not substring).
    """
    if base_url_host_matches(base_url or "", "inference-api.kova.ai"):
        return True
    try:
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to kova's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to kova's outer conversation
    ...
    # from ``ANTHROPIC_API_KEY`` (Kova loads that into the process env from
    # ``~/.kova/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Kova notices an expired token, Claude Code may have already rotated it.
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      5. Anthropic credential_pool OAuth entry (~/.kova/auth.json)
    ...
    # 5. Kova credential_pool OAuth entry.
...
def _get_kova_oauth_file() -> Path:
    return get_kova_home() / ".anthropic_oauth.json"
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See kova-agent#13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See kova-agent#13848 (Kimi) and
