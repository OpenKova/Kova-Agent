...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``KOVA_CA_BUNDLE`` /
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Kova owns auxiliary retry + provider/model fallback policy (the
    ...
    # Kova sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Kova control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="kova-protected-aux-provider",
        daemon=True,
    ).start()
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://kova-agent.kova.ai",
    "X-Title": "Kova Agent",
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.kova.ai/v1"
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Kova plumbing (_reasoning_config
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import kova_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=kova_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.kova.ai"):
        return "nous"
...
def _auth_refresh_provider_for_route(
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.kova.ai"):
        return "nous"
    return normalized
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_kova_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``kova_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._kova_fallback_destination = _fallback_destination_from_entry(
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = kova_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Kova owns
...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(kova_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"nous", "nous-portal"}:
            from kova_cli.providers import nous_api_mode
    ...
    if _provider_for_portal in {"nous", "nous-portal"}:
    ...
    # Anthropic Messages adapters translate Kova reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Kova reasoning config for direct model calls
    ...
    try:
        ...
        _heal_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.kova.ai")
        )
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.kova.ai")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
