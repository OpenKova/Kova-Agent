...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(kova_home / "cache" / "bws_cache.enc.json"),
            str(kova_root / "cache" / "bws_cache.enc.json"),
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Kova cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global Kova
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Kova read (see
...
#
# Kova profiles are separate HERMES_HOME dirs under
