...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``kova trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Kova' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Kova OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Kova' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Kova conversation messages as Claude Code JSONL text.
