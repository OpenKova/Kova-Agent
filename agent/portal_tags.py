...
def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Kova session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Kova conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.kova/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Kova
    ...
    """
    tags = ["product=kova-agent", kova_client_tag()]
