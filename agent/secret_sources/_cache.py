...
cache problem must never block Kova startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    ...
    falling back to ``$KOVA_HOME`` / ``~/.kova`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if KOVA_HOME is ever made traversable.
            try:
