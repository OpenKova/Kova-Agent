...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Kova never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Kova continues with whatever credentials ``.env`` already had.
...
are cached in-process and on disk under ``<kova_home>/cache/op_cache.json``
so back-to-back short-lived ``kova`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a KOVA_HOME switch
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Kova to source it from, so normalize to that name here.
    if token_value:
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_kova_dotenv()`` after the .env files have loaded.
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`kova secrets onepassword token`) so
