...
* The ``iron-proxy`` binary is auto-installed into ``<kova_home>/bin/iron-proxy``
  on first use.  Kova pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<kova_home>/proxy/ca.{crt,key}`` is generated on
  first ``kova egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<kova_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``kova egress start``), pidfile
  at ``<kova_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<kova_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``kova egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<kova_home>/proxy/management.token`` (0600), and injected into the
...
# credential (Kova' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.kova/proxy/`` just to
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<kova_home>/bin/iron-proxy``  (our managed copy — preferred)
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``kova proxy install`` surface lets them
    ...
    with tempfile.TemporaryDirectory(prefix="kova-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``kova egress
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<kova_home>/proxy/management.token`` with 0600 perms.
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `kova egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `kova egress setup` and use "
            "`kova egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `kova egress setup`, "
            "then `kova egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``kova egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `kova egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<kova_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Kova dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``kova egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``kova egress setup`` should not invalidate the tokens
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_KOVA_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `kova egress install`."
        )
    ...
    # close-on-success path, every restart leaked one fd in the Kova
    # process.
    #
    ...
    # ``kova egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``kova egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `kova egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`kova egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `kova secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
