...
(``gateway/platforms/webhook.py``): inbound wakes Kova when the world
changes; outbound tells the world when Kova does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Kova-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``KOVA_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.kova/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/kova-events
          ...
          secret_env: KOVA_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `kova hooks list`
...
Headers::
    ...
    User-Agent:              Kova-Agent-Outbound-Webhook
    X-Kova-Event:          <hook event name>
    X-Kova-Delivery:       <delivery_id>
    X-Kova-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("KOVA_SAFE_MODE"):
        logger.info("KOVA_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Kova-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Kova-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
