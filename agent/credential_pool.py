...
# un-throttled log fires on *every* selection. On Windows several Kova
...
# readiness handshake ("Timed out connecting to Kova backend after
...
class CredentialPool:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Kova
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``KOVA_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "KOVA_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "KOVA_XAI_REFRESH_TIMEOUT_SECONDS"
        )
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        try:
            ...
            # (kova auth remove copilot gh_cli) must not pay either on
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_kova_dotenv() -> apply_onepassword_secrets()).  The raw
