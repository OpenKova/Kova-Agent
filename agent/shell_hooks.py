...
from kova_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def register_from_config(
    ...
    *,
    ...
    # Safe mode (--safe-mode / KOVA_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("KOVA_SAFE_MODE"):
        logger.info("KOVA_SAFE_MODE=1 — shell-hook registration skipped")
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    if event == "pre_verify":
        # "continue" (Kova) / "block" (Claude-Code Stop: block the stop) both
