...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<KOVA_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
    ...
    home = os.environ.get("KOVA_HOME") or os.path.join(
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<KOVA_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    inner = (
        ...
        f"-HostName Kova -HostProfileId kova -HostVersion 1.0.0 "
