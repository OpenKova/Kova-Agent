...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("KOVA_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("KOVA_CRON_SESSION")
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_kova_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Kova: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # a lifecycle command past the guard — same treatment as the `kova ...
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``kova send`` message
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Kova YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Kova approval prompts.
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from kova_cli.config import load_config_readonly as _load_cfg
