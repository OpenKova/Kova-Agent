...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(?:kova|hermes)\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart kova gateway (kills running agents)"),
    (r'\b(?:kova|hermes)\s+update\b', "kova update (restarts gateway, kills running agents)"),
