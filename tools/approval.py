...
# os.environ["KOVA_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export KOVA_INTERACTIVE.
_kova_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "kova_interactive",
    default=None,
)


def set_kova_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["KOVA_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``KOVA_INTERACTIVE`` env var for legacy callers.
    """
    return _kova_interactive_ctx.set("1" if interactive else "")


def reset_kova_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_kova_interactive_context`."""
    _kova_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``KOVA_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _kova_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("KOVA_INTERACTIVE")
...
# active profile home path such as /home/kova/.hermes/config.yaml. The
...
# theater. Mirrors _KOVA_ENV_PATH; matches the HERMES_HOME override form as
# well as ~/.hermes/.
_KOVA_CONFIG_PATH = (
    r'(?:~\/\.kova/|'
    r'(?:\$home|\$\{home\})/\.kova/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `kova` and `gateway` (e.g. `kova -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\b(?:kova|kova)\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart kova gateway (kills running agents)"),
    (r'\b(?:kova|kova)\s+update\b', "kova update (restarts gateway, kills running agents)"),
    ...
    # In-place edit of a Kova-managed security file (~/.hermes/config.yaml or
...
def _rewrite_resolved_kova_home(command: str) -> str:
    """Rewrite the resolved absolute Kova home prefix to ``~/.hermes/``.
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Kova ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"kova-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Kova approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``KOVA_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            KOVA_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("KOVA_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "KOVA_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set KOVA_INTERACTIVE or "
            "KOVA_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``KOVA_INTERACTIVE``) fails CLOSED — a plugin-
