...
# means a compromised transitive dependency could read every Kova secret
...
def _build_browser_env() -> dict:
    ...
    Strips Kova-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``kova_subprocess_env`` import is deferred to keep ``browser_tool``
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``kova doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_kova_dir("cache/web", "web_cache")
