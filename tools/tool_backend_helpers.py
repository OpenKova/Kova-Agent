...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``kova auth add
    ...
    2. The environment / ``~/.kova/.env``. Under a multiplexed gateway turn
       ...
       ``kova_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``kova auth
    ...
    when omitted, ``kova_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `kova auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``kova auth add openai-api``) via
