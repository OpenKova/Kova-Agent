...
# Base URL now resolved via kova_cli.models.deepinfra_base_url (shared).
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``kova tools``); otherwise the historical Edge backend remains active.
    """
...
# file, so retired models disappear from kova the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``kova_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `kova setup` to configure, "
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import kova_cli as _kova_cli

            _kova_version = str(_kova_cli.__version__)
        except Exception:
            _kova_version = "0.0.0"
        # Include Kova client context following Gemini's partner
