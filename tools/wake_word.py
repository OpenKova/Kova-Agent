"""Wake-word ("Hey Kova") detection — hands-free session trigger.
...
wake word, Kova opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey kova" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey kova" can't
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey kova"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    try:
        ...
        from kova_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the Kova backend "
...
def _sherpa_model_root() -> Path:
    ...
    return get_kova_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey kova", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey kova" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey kova").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="kova-kws-", delete=False, encoding="utf-8"
        )
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `kova tools` "
...
# ---------------------------------------------------------------------------
# Process-wide singleton (mirrors kova_cli.voice's continuous API)
# ---------------------------------------------------------------------------
...
def _lock_path() -> Path:
    ...
    return get_default_kova_root() / "runtime" / "wake-word.lock"
