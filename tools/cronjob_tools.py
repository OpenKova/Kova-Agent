...
# uses 30s) — comfortably below the 1800s default KOVA_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (KOVA_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with KOVA_CRON_TIMEOUT=0
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``kova -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    if not session_key:
        # Direct Python callers (`kova cron run`, tests) have no agent
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `kova cron create/edit --model`, or hand-edited jobs). The agent
