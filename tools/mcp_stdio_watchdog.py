...
<url>``) is spawned as a direct child of the Kova process. Kova's own
...
Kova process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Kova startup's opt-in
