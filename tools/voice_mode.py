...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled Kova venv at
    # ~/.kova/profiles/<name>/kova-agent/venv/), `pip install` on the
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.kova/config.yaml during a
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Kova TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"kova-tts-{_unique}.wav")
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("KOVA_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    KOVA_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
