...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (KOVA_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (kova trace upload / /upload-trace).
    #
    ...
    # presence, the `kova update` lazy-refresh pass re-asserts THIS pin on
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Kova under WSL to use Matrix on Windows."
        )
    return None
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        uv_env = kova_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $HERMES_HOME/bin is never on PATH, so a bare
        # which() misses the uv Kova installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Kova
    ...
    # deployment sets KOVA_MANAGED=true *and* KOVA_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from kova_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Kova cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Kova instead)."
            )
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``KOVA_DISABLE_LAZY_INSTALLS=1``) and sets
      ``KOVA_LAZY_INSTALL_TARGET``, installs are redirected to the writable
