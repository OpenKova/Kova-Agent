...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (kova_constants itself is stdlib-only, so no cycle).
        from kova_constants import get_kova_home

        return Path(get_kova_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Kova-home override for every
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("kova_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``kova_plugins.{name}``), not the exact module string.
                # A handler defined in ``kova_plugins.pkg.handlers`` is
                # still owned by the ``kova_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("kova_plugins.")
                    and not same_plugin
