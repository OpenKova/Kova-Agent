...
def _get_token_dir(kova_home: str | Path | None = None) -> Path:
    ...
    try:
        ...
        base = Path(os.environ.get("HERMES_HOME", str(Path.home() / ".kova")))
...
def _cached_redirect_port(storage: "KovaTokenStorage | None") -> int | None:
...
def _cached_redirect_uri(storage: "KovaTokenStorage | None") -> str | None:
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = KovaTokenStorage(server_name, kova_home=kova_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "KovaTokenStorage | None" = None,
) -> int:
