...
def _get_token_dir(kova_home: str | Path | None = None) -> Path:
    ...
    base = Path(kova_home) if kova_home is not None else Path(get_kova_home())
...
def _cached_redirect_port(storage: "HermesTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Kova restarts and chooses a
...
class HermesTokenStorage:
    ...
    def __init__(self, server_name: str, *, kova_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._kova_home = Path(kova_home) if kova_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._kova_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._kova_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._kova_home) / f"{self._server_name}.meta.json"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._kova_home)
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = HermesTokenStorage(server_name, kova_home=kova_home)
    storage.remove()
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Kova Agent" / "Kova" / "Cursor" / "VS Code" / … → 403
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Kova auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Kova is running.
    """
    msg = str(exc)
    lowered = msg.lower()
