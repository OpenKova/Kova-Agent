...
_SIGN_IN_MESSAGE = (
    ...
    "Ask the user to run `kova model` and sign in to Nous, then retry."
)
...
def _default_directory():
    ...
    is not: an operator running KOVA_MEDIA_DELIVERY_STRICT=1 delivers only
    ...
    if _delivers_as_an_attachment():
        try:
            ...
            return get_kova_dir("cache/videos", "video_cache")
        except Exception:
