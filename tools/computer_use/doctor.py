...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Kova provider secrets stripped.
...
def run_doctor(
    ...
    *,
    ...
    Honors `KOVA_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Kova identity under kova_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["kova_identity"] = identity
