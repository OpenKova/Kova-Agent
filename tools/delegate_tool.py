...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``kova-cli`` because those also contain
...
def _build_child_agent(
    ...
    # Blocked tools also live inside mixed platform bundles (kova-cli,
    # kova-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child Kova/Qwen subagent onto the parent's Claude Messages wire (or the
    ...
    elif _effective_provider_norm in {"nous", "nous-portal"}:
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_kova_dir("cache/delegation", "delegation_cache")
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from kova_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # KOVA_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Kova-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("KOVA_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Kova-Session-Id once the batch completes.
            ...
            # KOVA_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `kova -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("KOVA_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("KOVA_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and KOVA_SESSION_KEY is not in the CLI
...
def _load_config() -> dict:
    ...
    ``KOVA_IGNORE_USER_CONFIG=1`` (``kova chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("KOVA_IGNORE_USER_CONFIG") == "1"
