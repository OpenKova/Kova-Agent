...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``kova auth add <provider_id>``).
...
# DeepInfra STT base URL now resolved via kova_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``KOVA_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Kova's remote upload cap."""
...
def _command_stt_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Kova secrets by
...
def _run_command_stt(
    ...
    Child env is scrubbed of Kova secrets (salvage of #56332) while still
    ...
    scrubbed = kova_subprocess_env(inherit_credentials=False)
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `kova plugins list` to see "
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="kova-silk-")
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="kova-local-stt-") as output_dir:
            ...
            # Scrub Kova secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from tools.environments.local import kova_subprocess_env

            child_env = kova_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``KOVA_LOCAL_STT_LANGUAGE`` (env). When none is set, Groq
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="kova-stt-") as work_dir:
                try:
...
def _transcribe_deepinfra(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``kova_cli.models`` helpers so every DeepInfra surface resolves the
    ...
    from kova_cli.models import deepinfra_base_url, deepinfra_model_ids
