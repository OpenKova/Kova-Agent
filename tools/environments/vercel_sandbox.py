...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Kova'
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Kova
...
def _snapshot_store_path() -> Path:
    return get_kova_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.kova"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.kova"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.kova"
        )
