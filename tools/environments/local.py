...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    # Common recovery for config values like ``kova-agent`` when Kova was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./kova-agent``; use the current directory instead.
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Kova-managed inference credential — Claude subscription auth is not a
    # working Kova provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Kova venv path instead of the project's own
# ``.venv`` — silently clobbering the Kova environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Kova venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_kova_internal_secret(key: str) -> bool:
    """Return True for Kova-internal secrets injected under *dynamic* names.

    ``_KOVA_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Kova-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`kova_subprocess_env` helper all call it, so the
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (KOVA_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_KOVA_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_kova_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _KOVA_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_KOVA_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if _is_kova_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_kova_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _KOVA_PROVIDER_ENV_BLOCKLIST and not passthrough:
...
# legitimate child Kova spawns needs them, and they are the highest-value
...
# narrow subset of _KOVA_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in kova_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_kova_internal_secret, but _ID has no secret suffix, so it must be
...
def kova_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_KOVA_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Kova spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_KOVA_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Kova-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_kova_internal_secret`.
    for key in list(env):
        if key.startswith(_KOVA_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_kova_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _KOVA_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_kova_home(env)
    from kova_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose KOVA_SESSION_* mirror is a last-writer-wins
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`kova_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``HERMES_HOME`` / subprocess ``HOME`` contract) and the
    Kova secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_kova_internal_secret`` dynamic
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Kova home override into ``HERMES_HOME`` and apply the
      subprocess HOME contract (``kova_constants.apply_subprocess_home_env``).
...
def _find_bash() -> str:
    ...
    # system Git (or a stale KOVA_GIT_BASH_PATH pointing at one) must not
    ...
    # KOVA_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "KOVA_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Kova. If the override is blocked or later re-applied, "
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Kova-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Kova installs for
    itself, so on a machine where Kova provisioned its own toolchain a
    ...
    ``get_kova_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_kova_managed_node``, a first browser install).
    """
    try:
        from kova_constants import get_kova_home, iter_kova_node_dirs

        candidates = [*iter_kova_node_dirs(), get_kova_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Kova-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Kova
    ...
    for k, v in merged.items():
        if k.startswith(_KOVA_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if _is_kova_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_kova_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _KOVA_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
