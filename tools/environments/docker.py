...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``KOVA_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `kova egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `kova egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `kova egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `kova egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/kova-egress-ca.crt"
    ...
    env_overrides: dict[str, str] = {
        ...
        "_KOVA_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca",
    }
    ...
    # through the proxy.  Keep the KOVA_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"KOVA_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if egress_env_overrides:
            try:
                from kova_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from kova_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_KOVA_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_KOVA_EGRESS_NODE_OPTIONS_APPEND", None,
        )
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # keys are filtered. Also strip Kova-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_kova_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_kova_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _KOVA_PROVIDER_ENV_BLOCKLIST)
    ...
    def _find_reusable_container(
        ...
        try:
            ...
            if egress_label != "off":
                ...
                # kova-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "kova egress disable",
