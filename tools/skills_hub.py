...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _kova_home() / "skills"
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Neural Studio) but not activated
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``kova_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _kova_index_cache_file() -> Path:
    return _index_cache_dir() / "kova-index.json"
...
def _load_kova_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    kova_index_cache_file = _kova_index_cache_file()
    if kova_index_cache_file.exists():
        try:
            age = time.time() - kova_index_cache_file.stat().st_mtime
            if age < KOVA_INDEX_TTL:
                return json.loads(kova_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            logger.debug(
                "Kova index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
    ...
    try:
        ...
        kova_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
