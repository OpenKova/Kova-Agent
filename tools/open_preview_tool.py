#!/usr/bin/env python3
"""Open a URL, dev server, or file in the Kova desktop GUI's preview pane.

Gated on ``KOVA_DESKTOP`` (like ``read_terminal`` / ``close_terminal``) so it
...
def open_preview_tool(url: str, label: str = "") -> str:
    ...
    if not ok:
        return tool_error("The preview pane is only available in the Kova desktop app.")
...
def check_open_preview_requirements() -> bool:
    ...
    return env_var_enabled("KOVA_DESKTOP")


OPEN_PREVIEW_SCHEMA = {
    "name": "open_preview",
    "description": (
        "Open something in the preview pane beside the chat in the Kova desktop "
