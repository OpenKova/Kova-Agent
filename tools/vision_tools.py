...
# ``agent.auxiliary_client`` pulls credential_pool → kova_cli.auth → httpx
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: KOVA_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("KOVA_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_kova_dir("cache/vision", "temp_vision_images")
