...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``kova send`` (and the send_message
    ...
    ``kova send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if platform == Platform.SLACK and media_files:
        ...
        from kova_cli.plugins import discover_plugins as _dp_slack
        _dp_slack()
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    is running (standalone cron, ``kova send`` CLI).
    """
    media_files = media_files or []
