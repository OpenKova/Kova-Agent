...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    kova_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'kova_tools'", window
        )
        if m:
            missing = m.group(1)
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("KOVA_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            ...
            f"KOVA_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def execute_code(
    ...
    try:
        ...
        child_env["KOVA_RPC_TOKEN"] = rpc_token
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that calls Kova tools programmatically. "
