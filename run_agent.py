...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Kova-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.kova/sessions/``.  Without
...
class AIAgent:
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `kova --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from kova_cli import lifecycle as _lifecycle
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        # untrusted input (X-Kova-Session-Id header) and must not escape
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Kova' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Kova model request this cancels only that request;
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Kova YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``KOVA_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.kova/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``kova setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from kova_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from kova_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = kova_xai_default_headers()
        else:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        """
        if base_url_host_matches(self._base_url_lower, "kova.ai"):
            return True
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from kova_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated kova_logging._session_context (a
            ...
            # KOVA_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from kova_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
