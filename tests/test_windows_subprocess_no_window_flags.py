...
def _is_git_spawn(cmd) -> bool:
    ...
    ``bounded_git_probe`` lives in ``kova_cli._subprocess_compat`` and both
...
def test_copilot_gh_cli_probe_hides_gh_windows(monkeypatch):
    from kova_cli import copilot_auth
...
def test_stale_dashboard_windows_scan_hides_wmic(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        captured.append((cmd, kwargs))
        return _Completed(stdout="CommandLine=kova dashboard\nProcessId=123\n")
...
def test_tui_cli_exec_rpc_hides_python_window(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        captured.append((cmd, kwargs))
        return _Completed(stdout="kova 0.0-test\n")
    ...
    spawns = _spawns(captured, "kova_cli.main")
    assert len(spawns) == 1, captured
    cmd, kwargs = spawns[0]
    assert cmd[:3] == [server.sys.executable, "-m", "kova_cli.main"]
    assert kwargs["creationflags"] == _CREATE_NO_WINDOW
...
def test_cli_quick_command_exec_hides_console_window(monkeypatch):
    ...
    inst = object.__new__(cli_mod.KovaCLI)
...
def test_lsp_install_npm_hides_console_window(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        lsp_install, "kova_lsp_bin_dir", lambda: tmp_path / "lsp" / "bin"
    )
