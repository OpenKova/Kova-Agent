...
class TestConcurrency:
    def test_sync_back_waits_for_active_sync_transaction(self, tmp_path):
        ...
        def get_files():
            return [
                (str(path), f"/root/.kova/cache/images/{path.name}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            sync_back_future = executor.submit(mgr.sync_back, kova_home=tmp_path)
    ...
    def test_sync_back_does_not_overwrite_uploaded_credential_files(self, tmp_path, monkeypatch):
            ...
            lambda: [
                {
                    ...
                    "container_path": "/root/.kova/credentials/token.json",
                }
            ],
        )
        monkeypatch.setattr(
            "tools.credential_files.iter_skills_files",
            lambda container_base="/root/.kova": [
                {
                    "host_path": str(skill),
        ...
        def bulk_download(dest: Path) -> None:
            with tarfile.open(dest, "w") as tar:
                for name, data in {
                    "root/.kova/credentials/token.json": b"remote-token",
                    "root/.kova/skills/skill.py": b"remote-skill",
                }.items():
        ...
        mgr.sync(force=True)
        mgr.sync_back(kova_home=tmp_path)
