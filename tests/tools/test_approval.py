...
class TestDetectDangerousRm:
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            ...
            "rm -f /tmp/nested/../kova-verify-example.py",
            "rm -f /tmp/./kova-verify-example.py",
            "rm -f /tmp//kova-verify-example.py",
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\kova-victim\file.txt"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\kova-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\kova-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\kova-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
...
class TestKovaConfigWriteProtection:
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        assert dangerous is True
        assert key != "in-place edit of Kova config/env (perl/ruby)"
...
class TestWindowsAbsolutePathFolding:
    ...
    def test_windows_kova_home_config_folds(self, monkeypatch):
        ...
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("HERMES_HOME", r"C:\Users\tester\.kova")
        dangerous, key, _ = detect_dangerous_command(
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(
        ...
        monkeypatch.setenv("KOVA_LANGUAGE", lang)
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.hermes/kova-agent && source venv/bin/activate && python -m kova_cli.main gateway run --replace &disown; echo done"
    ...
    def test_kova_gateway_stop_detected(self):
        cmd = "kova gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_kova_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `kova` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "kova -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_kova_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "kova --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_kova_gateway_multiple_flags_detected(self):
        cmd = "kova -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_kova_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "kova -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_kova_gateway_start_not_flagged(self):
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Kova security config via IFS must be caught."""
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("kova_cli.config.load_config", return_value=cfg):
