...
class TestPollTransport:
    ...
    def test_a_cli_session_is_recognised_by_its_source(self, tmp_path, monkeypatch):
        # The CLI, TUI, and desktop leave KOVA_SESSION_PLATFORM empty and
        # identify themselves on KOVA_SESSION_SOURCE instead, so keying only
        # on the platform would miss them.
        monkeypatch.delenv("KOVA_SESSION_PLATFORM", raising=False)
        monkeypatch.setenv("KOVA_SESSION_SOURCE", "tui")
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
