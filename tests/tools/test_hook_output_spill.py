...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_user_overrides_are_respected(self):
        ...
        with patch("kova_cli.config.load_config", return_value=user_cfg):
            cfg = hos.get_spill_config()
...
class SpillIfOversizedTests(unittest.TestCase):
    ...
    def test_session_id_with_path_separators_is_sanitised(self):
        ...
        self.assertFalse(Path("/etc/passwd-kova-test").exists())
    ...
    def test_default_directory_uses_kova_home(self):
        ...
        try:
            ...
            # ~/.kova/hook_outputs depending on get_kova_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.kova/hook_outputs/sess")),
            ]
