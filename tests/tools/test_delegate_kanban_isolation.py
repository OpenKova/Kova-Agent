...
# cwd. Without an explicit PYTHONPATH the child resolves ``kova_cli`` /
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with KOVA_KANBAN_TASK set;
...
def test_delegate_child_terminal_env_scrubs_parent_kanban_keys(monkeypatch):
    monkeypatch.setenv("KOVA_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KOVA_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("KOVA_KANBAN_WORKSPACE", "/tmp/parent-workspace")
    monkeypatch.setenv("KOVA_KANBAN_CLAIM_LOCK", "lock")
    ...
    with delegated_child_context():
        env = _sanitize_subprocess_env({
            ...
            "KOVA_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            "KOVA_KANBAN_CLAIM_LOCK": "lock",
            "PATH": "/usr/bin",
        })
    ...
    assert "KOVA_KANBAN_WORKSPACE" not in env
    assert "KOVA_KANBAN_CLAIM_LOCK" not in env
...
def test_delegate_child_process_marker_scrubs_foreground_terminal_kanban_keys(monkeypatch):
    ...
    monkeypatch.setenv("KOVA_DELEGATED_CHILD_CONTEXT", "1")
    monkeypatch.setenv("KOVA_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KOVA_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("KOVA_KANBAN_DB", "/tmp/parent-kanban.db")
    monkeypatch.setenv("KOVA_KANBAN_WORKSPACE", "/tmp/parent-workspace")
    monkeypatch.setenv("KOVA_KANBAN_CLAIM_LOCK", "lock")
    ...
    assert "KOVA_KANBAN_TASK" not in env
    assert "KOVA_KANBAN_RUN_ID" not in env
    assert "KOVA_KANBAN_DB" not in env
    assert "KOVA_KANBAN_WORKSPACE" not in env
    assert "KOVA_KANBAN_CLAIM_LOCK" not in env
    assert env["KOVA_DELEGATED_CHILD_CONTEXT"] == "1"
    ...
    env = _scrub_child_env(
        {
            ...
            "KOVA_KANBAN_TASK": "t_parent",
            "KOVA_KANBAN_RUN_ID": "123",
            "KOVA_KANBAN_DB": str(home / "kanban.db"),
            "KOVA_KANBAN_WORKSPACE": str(tmp_path / "parent-workspace"),
            "PATH": "/usr/bin",
        },
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote KOVA_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".kova"
    home.mkdir()
    ...
    monkeypatch.setenv("KOVA_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("KOVA_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("KOVA_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("KOVA_KANBAN_"),
            is_windows=False,
        )
...
def test_delegate_child_execute_code_cannot_complete_parent_by_importing_kanban_db(
    ...
    code = "\n".join([
        ...
        "from kova_cli import kanban_db as kb",
        "observed = {",
        "    'marker': os.environ.get('KOVA_DELEGATED_CHILD_CONTEXT'),",
