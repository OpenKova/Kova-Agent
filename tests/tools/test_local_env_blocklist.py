...
See: https://github.com/OpenKova/Kova-Agent/issues/1002
See: https://github.com/OpenKova/Kova-Agent/issues/1264
"""
...
class TestActiveVenvMarkerStripping:
    ...
    into the Kova venv path instead of the project's own ``.venv`` —
    silently clobbering the Kova environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Kova venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.kova/kova-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/kova",
        })
...
class TestKovaInternalDynamicSecrets:
    """Dynamically-named Kova secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_KOVA_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_kova_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``kova_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_kova_internal_secret
        assert _is_kova_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_kova_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_kova_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_kova_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local import _is_kova_internal_secret
        assert _is_kova_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_kova_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local import _is_kova_internal_secret
        assert _is_kova_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_kova_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_kova_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local import _is_kova_internal_secret
        assert not _is_kova_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_kova_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_kova_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_kova_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_kova_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_kova_internal_secret("PATH")
        assert not _is_kova_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _KOVA_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _KOVA_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _KOVA_PROVIDER_ENV_BLOCKLIST
