...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and kova would run without zombie reaping.
    """
    ...
    assert "/opt/kova/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/kova/docker/main-wrapper.sh" in dispatcher, (
