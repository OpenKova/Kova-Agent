"""The conftest WAL gate must agree with kova_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``kova_state``. That is deliberate: importing
``kova_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.kova`` before the per-test ``HERMES_HOME`` redirect, which makes tests
...
from kova_state import is_sqlite_wal_reset_vulnerable
...
def test_conftest_gate_agrees_with_kova_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and kova_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_kova_state_at_collection():
    """The gate must stay import-free of kova_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.kova, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``kova_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "kova_state" or name.startswith("kova_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported kova_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.kova during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("kova_state", None)
