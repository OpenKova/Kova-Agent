...
# the REAL Kova root — capture it BEFORE the sandbox rewires HERMES_HOME,
...
def _kova_home_points_at_production(value: str) -> bool:
    ...
    HERMES_HOME=~/.kova``) hand pytest the PRODUCTION home. Historically
...
if _kova_home_points_at_production(os.environ.get("HERMES_HOME", "")):
    _SESSION_HERMES_HOME = tempfile.mkdtemp(prefix="kova-test-home-")
...
_KOVA_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.kova/kanban.db instead of the per-test HERMES_HOME.
    "KOVA_KANBAN_DB",
    "KOVA_KANBAN_BOARD",
    "KOVA_KANBAN_HOME",
...
def _capture_real_kanban_root() -> Path:
    ...
    if _PRE_SANDBOX_HERMES_HOME and not _kova_home_points_at_production(
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    Only patches when ``kova_cli.kanban_db`` is *already imported* — a
...
#     installs where HERMES_HOME is not ~/.kova) into the guard's
