"""The test suite must never write into the operator's real Kova logs.

`kova_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(kova_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_kova_home_is_sandboxed_before_imports(self):
        ...
        assert Path(home).resolve() != _real_kova_home().resolve(), (
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("kova_cli.main")

        real_logs = str(_real_kova_home() / "logs")
        offenders = [p for p in _all_file_destinations() if p.startswith(real_logs)]

        assert offenders == [], (
            "the test session is writing into the operator's real Kova logs:\n  "
            + "\n  ".join(offenders)
        )
