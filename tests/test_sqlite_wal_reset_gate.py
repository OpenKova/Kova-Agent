...
Kova must not *enable* multi-process WAL on SQLite builds that still contain
...
def _reset_wal_reset_bug_warnings():
    kova_state._wal_reset_bug_warned_paths.clear()
    yield
    kova_state._wal_reset_bug_warned_paths.clear()
...
class TestNoDowngradeUnderConcurrentOpeners:
    ...
    def test_configured_delete_refuses_when_probe_blocked(
        ...
        monkeypatch.setattr(kova_state, "resolve_journal_mode", lambda: "delete")
...
def test_doctor_warns_without_adding_issues(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(kova_state, "sqlite_source_id", lambda: "testid-abc")
