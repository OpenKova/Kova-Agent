...
class TestChromeDebugLaunch:
    ...
    def test_windows_launch_uses_browser_found_on_path(self):
        ...
        with patch("kova_cli.browser_connect.shutil.which", side_effect=lambda name: r"C:\Chrome\chrome.exe" if name == "chrome.exe" else None), \
             ...
             patch("kova_cli.browser_connect._wait_for_browser_debug_ready_or_exit", return_value="ready"), \
    ...
    def test_wait_for_browser_debug_ready_or_exit_detects_early_exit(self, monkeypatch):
        ...
        monkeypatch.setattr("kova_cli.browser_connect.time.sleep", lambda _seconds: None)
        with patch("kova_cli.browser_connect.is_browser_debug_ready", return_value=False):
    ...
    def test_launch_tries_next_browser_when_first_candidate_exits_before_debug_ready(self):
        ...
        with patch("kova_cli.browser_connect.get_chrome_debug_candidates", return_value=[brave, chrome]), \
             patch("kova_cli.browser_connect._wait_for_browser_debug_ready_or_exit", side_effect=["exited", "ready"]), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert KovaCLI._try_launch_chrome_debug(9222, "Linux") is True
    ...
    def test_launch_result_hints_singleton_forward_on_clean_exit(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kova_cli.browser_connect.chrome_debug_data_dir", lambda: str(tmp_path)
        )
        with patch("kova_cli.browser_connect.get_chrome_debug_candidates", return_value=[chrome]), \
             patch("kova_cli.browser_connect.is_browser_debug_ready", return_value=False), \
    ...
    def test_launch_result_no_hint_when_no_candidates(self):
        with patch("kova_cli.browser_connect.get_chrome_debug_candidates", return_value=[]):
            result = launch_chrome_debug(9222, "Linux")
    ...
    def test_connect_context_note_allows_expected_browser_use(self, monkeypatch):
        ...
        with patch(
                 "kova_cli.cli_commands_mixin.discover_local_cdp_url",
                 return_value="http://127.0.0.1:9222",
             ), \
             patch("kova_cli.cli_commands_mixin.is_browser_debug_ready", return_value=True), \
