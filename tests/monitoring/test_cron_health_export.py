...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("kova.gateway.up"), _M("kova.gateway.active_agents"),
        _M("kova.gateway.busy"), _M("kova.gateway.drainable"),
        _M("kova.gateway.restart_requested"),
        _M("kova.platform.up"), _M("kova.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("kova.cron.scheduler.heartbeat_age_seconds"),
        _M("kova.cron.scheduler.last_success_age_seconds"),
        _M("kova.cron.scheduler.catch_up_occurrences"),
        _M("kova.cron.jobs.enabled"), _M("kova.cron.jobs.running"),
        _M("kova.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Kova Agent-owned Relay transport health" in text
