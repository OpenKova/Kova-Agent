...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``kova gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "kova_feedback" in created_apps[0].registered_actions
        assert "kova_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("kova_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("kova_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_kova_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/kova",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/kova-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        a.handle_message.assert_awaited_once()
        msg_event = a.handle_message.call_args[0][0]
        assert msg_event.media_urls == ["/tmp/kova-cached.png"]
    ...
    async def test_collect_thread_root_images_resolves_connect_stub(
        ...
        assert urls == ["/tmp/kova-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_KOVA_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_kova_slack_user_agent_prefix_format(self):
        """Module constant matches the KovaAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._KOVA_SLACK_USER_AGENT_PREFIX.startswith("KovaAgent/")

    @pytest.mark.asyncio
    async def test_async_web_client_constructed_with_kova_user_agent_prefix(self):
        ...
        expected_prefix = _slack_mod._KOVA_SLACK_USER_AGENT_PREFIX
