...
def test_search_slow_ms_bridged_from_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_kova_home", home)
    monkeypatch.delenv("KOVA_SEARCH_SLOW_MS", raising=False)
    gateway_run._reload_runtime_env_preserving_config_authority()
    assert os.environ["KOVA_SEARCH_SLOW_MS"] == "250"


def test_env_survives_when_config_omits_search_knobs(tmp_path, monkeypatch):
    home = _write_home(tmp_path, {"auto_prune": False})
    monkeypatch.setattr(gateway_run, "_kova_home", home)
    monkeypatch.setenv("KOVA_CJK_FTS", "0")
    monkeypatch.setenv("KOVA_SEARCH_SLOW_MS", "700")
    gateway_run._reload_runtime_env_preserving_config_authority()
    assert os.environ["KOVA_CJK_FTS"] == "0"
    assert os.environ["KOVA_SEARCH_SLOW_MS"] == "700"
...
def test_config_false_disables_cjk_semantics(tmp_path, monkeypatch):
    """The bridged 'False' string must parse as OFF in kova_state."""
    from kova_state import _cjk_fts_config_enabled

    monkeypatch.setenv("KOVA_CJK_FTS", "False")
    assert not _cjk_fts_config_enabled()
    monkeypatch.setenv("KOVA_CJK_FTS", "True")
    assert _cjk_fts_config_enabled()
    monkeypatch.delenv("KOVA_CJK_FTS", raising=False)
