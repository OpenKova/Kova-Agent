...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        self._patch_roots(monkeypatch)
        monkeypatch.setenv("KOVA_KANBAN_HOME", str(tmp_path / "kova"))
        monkeypatch.setenv("KOVA_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "kova" / "kanban" / "boards" / "research"
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        ...
        fake_home = tmp_path / "home"
        kova_dir = fake_home / ".kova"
        (kova_dir / "mcp-tokens").mkdir(parents=True)
        secret = kova_dir / rel
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        ...
        kova_root = denied_root / ".kova"
        prof_cache = kova_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._KOVA_ROOT", kova_root
        )
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        kova_root = denied_root / ".kova"
        prof_dir = kova_root / "profiles" / "myprof"
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Kova
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../kova/cache/video/abc.mp4``
