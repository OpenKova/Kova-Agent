...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``KOVA_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "KOVA_SESSION_CHAT_ID": env.get("KOVA_SESSION_CHAT_ID"),
        "KOVA_SESSION_THREAD_ID": env.get("KOVA_SESSION_THREAD_ID"),
        "KOVA_SESSION_KEY": env.get("KOVA_SESSION_KEY"),
    }
...
def test_child_task_inherits_foreign_session_without_reset():
    ...
    assert captured["window"]["KOVA_SESSION_CHAT_ID"] == "MINE_CHAT", (
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("KOVA_SESSION_CHAT_ID", "KOVA_SESSION_THREAD_ID", "KOVA_SESSION_KEY"):
    ...
    assert captured["bound"]["KOVA_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["KOVA_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``KOVA_SESSION_*``
