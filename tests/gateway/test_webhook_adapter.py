...
class TestPayloadFilters:
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_kova_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.kova/scripts paths must resolve through KOVA_HOME for profiles."""
        monkeypatch.setenv("KOVA_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.kova/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
