...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.kova")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.kova"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/kova-agent": ("/www/kova-agent", "/www/kova-agent")})
    sessions = [
        _session("/www/kova-agent", branch="main"),
        _session("/www/kova-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/kova-agent"]
    assert _lane_ids(project) == ["/www/kova-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/kova-agent"
