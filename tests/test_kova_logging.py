...
def _reset_logging_state():
    ...
    kova_logging._reset_queued_handlers()
...
class TestSetupLogging:
    ...
    def test_creates_agent_log_handler(self, kova_home):
        ...
        agent_handlers = [
            h for h in kova_logging.rotating_file_handlers()
...
class TestAddRotatingHandler:
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        handlers = [h for h in kova_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("kova_cli.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in kova_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                kova_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(kova_logging.sys, "platform", "win32"):
            assert kova_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not kova_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(kova_logging.sys, "platform", "linux"):
            assert not kova_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, kova_home):
        kova_logging.setup_logging(kova_home=kova_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_kova_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in kova_logging.rotating_file_handlers()
        )

    def test_records_reach_file_through_queue(self, kova_home):
        kova_logging.setup_logging(kova_home=kova_home)
    ...
    def test_queue_preserves_per_handler_levels(self, kova_home):
        kova_logging.setup_logging(kova_home=kova_home)
