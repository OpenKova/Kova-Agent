...
class TestAcpExecAskGate:
    ...
    def test_interactive_context_var_routes_to_callback_without_env(
        ...
        tok = set_kova_interactive_context(True)
        try:
            result = check_all_command_guards(
            ...
            reset_kova_interactive_context(tok)

        assert called_with, (
            "set_kova_interactive_context(True) should route dangerous "
            "commands through the callback without KOVA_INTERACTIVE in env"
        )
        assert result["approved"] is True
