...
#
# Installs a real, earlier Kova the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `kova`.
#
...
# Keep sandbox state out of the default .kova-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".kova-sandbox-e2e-$ROUTE"
export KOVA_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/kova/.kova/kova-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/kova-agent.git"
...
LOG_DIR="${KOVA_E2E_LOG_DIR:-$(mktemp -d -t kova-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED kova accept FLAG on `kova update`?
#
...
# that silently rots. `kova update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "kova update --help 2>&1" | grep -qF -- "$flag"
}
...
require_kova_works() {
  local when="$1" out
  out="$(in_sandbox "kova --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "kova --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "kova runs $when"
}

# ── install the earlier Kova ─────────────────────────────────────────────
...
require_kova_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: kova update'
    ...
    # kova which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="kova update --yes"
    else
      update_cmd="kova update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "kova update failed ($update_cmd)"
    fi
    require_landed_on_target 'kova update'
    require_kova_works 'after kova update'
    ;;
  installer)
    ...
    require_kova_works 'after installer re-run'
    ;;
esac
