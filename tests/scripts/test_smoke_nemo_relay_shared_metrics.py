...
def test_resolve_kova_executable_falls_back_to_path(tmp_path, monkeypatch):
    executable = tmp_path / "bin" / "kova"
    monkeypatch.setattr(smoke.shutil, "which", lambda _name: str(executable))

    assert smoke._resolve_kova_executable(tmp_path / "repo") == executable
