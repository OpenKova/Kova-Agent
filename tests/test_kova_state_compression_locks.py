...
def test_non_expired_lock_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(
        kova_state, "psutil", SimpleNamespace(pid_exists=process_is_gone)
    )
...
def test_dead_pid_reclaim_via_os_kill_fallback_when_psutil_missing(
    ...
    monkeypatch.setattr(kova_state, "psutil", None)
    ...
    monkeypatch.setattr(kova_state.os, "kill", process_is_gone)
...
def test_probe_doubt_keeps_lease_until_ttl(
    ...
    monkeypatch.setattr(
        kova_state, "psutil", SimpleNamespace(pid_exists=probe_blows_up)
    )
...
def test_windows_uses_ttl_only_without_pid_probe(
    ...
    monkeypatch.setattr(kova_state.os, "name", "nt")
    monkeypatch.setattr(
        kova_state,
        "psutil",
        SimpleNamespace(
