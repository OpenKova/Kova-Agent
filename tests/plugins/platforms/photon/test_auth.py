...
def test_save_auth_never_world_readable(tmp_kova_home: Path) -> None:
    """auth.json must be created 0o600 — no window at process umask."""
    photon_auth.store_photon_token("secret-token")
    mode = (tmp_kova_home / "auth.json").stat().st_mode & 0o777
    assert mode == 0o600
...
# cross-process lock kova_cli/auth.py's ~15 other writers all use, so a
...
def _hold_auth_lock_then_release(hold_event: threading.Event, release_event: threading.Event) -> None:
    from kova_cli.auth import _auth_store_lock
