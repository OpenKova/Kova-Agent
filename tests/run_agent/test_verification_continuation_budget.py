...
def test_pre_verify_preserves_composed_report_at_budget_limit(agent, monkeypatch):
    ...
    agent._interruptible_api_call = model_call
    agent._handle_max_iterations = MagicMock(return_value="replacement summary")
    monkeypatch.setenv("KOVA_VERIFY_ON_STOP", "0")

    with (
        patch("kova_cli.plugins.has_hook", side_effect=lambda name: name == "pre_verify"),
        patch(
            "kova_cli.plugins.get_pre_verify_continue_message",
