...
class TestPortalEnvOverrideHelper:
    ...
    def test_kova_portal_base_url_wins(self, monkeypatch):
        monkeypatch.setenv(
            "KOVA_PORTAL_BASE_URL", "https://portal.staging-nousresearch.com/"
        )
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert (
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("kova_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior KOVA_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
