...
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (KOVA_MANAGED_DIR).
...
Subprocess (not in-process monkeypatching) so module-level ``_kova_home``
globals, managed-scope caches, and ``_under_pytest`` guards behave like
production: ``KOVA_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    code = textwrap.dedent(
        ...
        import os
        from pathlib import Path
        saved = Path(server._kova_home, "config.yaml").read_text(encoding="utf-8")
    ...
    out = _run_py(
        code,
        {
            ...
            "KOVA_MANAGED_DIR": str(managed_dir),
