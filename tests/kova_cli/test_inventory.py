...
def test_explicit_only_keeps_moa_when_raw_config_has_enabled_preset():
    ...
    with (
        _list_auth_returning(rows),
        patch("kova_cli.config.load_config", return_value=raw_config),
        patch("kova_cli.config.read_raw_config", return_value=raw_config),
        patch("kova_cli.auth.is_provider_explicitly_configured", return_value=False),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
