"""Tests for `kova chat --safe-mode` isolation."""
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["KOVA_SAFE_MODE"] == "1"
        assert os.environ["KOVA_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["KOVA_IGNORE_RULES"] == "1"
        return True
...
def test_prepare_agent_startup_applies_safe_mode_before_plugin_discovery(monkeypatch):
    ...
    plugins = types.ModuleType("kova_cli.plugins")
    ...
    monkeypatch.setitem(sys.modules, "kova_cli.plugins", plugins)
