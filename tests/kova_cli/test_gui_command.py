...
def test_gui_linux_falls_back_to_no_sandbox_when_userns_is_restricted(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("kova_cli.main._desktop_linux_needs_no_sandbox", return_value=True), \
         patch("kova_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_exits_when_sandbox_fixup_fails_without_safe_fallback(tmp_path, monkeypatch):
    ...
    with patch("kova_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("kova_cli.main._desktop_linux_needs_no_sandbox", return_value=False), \
         patch("kova_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    packaged_exe = root / "apps" / "desktop" / "release" / "linux-unpacked" / "kova"
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("kova_cli.main.subprocess.run", side_effect=run_side_effect) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
