...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Kova-managed
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["kova"], 0)

    with patch("kova_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("kova_cli.main._desktop_build_needed", return_value=True), \
         patch("kova_cli.main._write_desktop_build_stamp"), \
         patch("kova_cli.main.subprocess.run", side_effect=[subprocess.CompletedProcess([], 0), launch_ok]), \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("kova_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("kova_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("kova_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Kova.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Kova.app"
    _write_info_plist(app, "com.kovaai.kova")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Kova").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Kova Helper.app"
    _write_info_plist(helper, "com.kovaai.kova.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Kova.app component, silently skipping every .node/.dylib/crashpad
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`kova desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch, platform="linux")

    registered: list[Path] = []
    monkeypatch.setattr("kova_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "kova_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "kova.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("kova_cli.main._desktop_build_needed", return_value=False), \
         patch("kova_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("kova_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("kova_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("kova_cli.linux_desktop_entry.install_desktop_entry", boom)
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("kova_cli.linux_desktop_entry.install_desktop_entry", fail)
