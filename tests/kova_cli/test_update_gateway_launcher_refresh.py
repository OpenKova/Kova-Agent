...
2. ``kova_cli.main._refresh_windows_gateway_launchers`` — ``kova
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "kova_cli.main", "gateway", "run"]
    ...
    ), mock.patch("kova_cli.config.get_kova_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: kova update regenerates launchers
# ---------------------------------------------------------------------------
