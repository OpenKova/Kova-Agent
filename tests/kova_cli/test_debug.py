...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, kova_home):
        from kova_cli.debug import collect_share_bundle
    ...
    def test_no_redact_omits_banner(self, kova_home):
        from kova_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, kova_home):
        from kova_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, kova_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from kova_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, kova_home):
        import gzip
        import json as _json

        from kova_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "kova-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, kova_home, capsys):
        from kova_cli.debug import run_debug_share
        ...
        with patch("kova_cli.dump.run_dump"), patch(
            "kova_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, kova_home, capsys):
        from kova_cli.debug import run_debug_share

        with patch("kova_cli.dump.run_dump"), patch(
            "kova_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, kova_home):
        from kova_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("kova_cli.dump.run_dump"), patch(
            "kova_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("kova_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``KovaCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("kova_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`kova debug share` requires explicit consent before uploading.
    ...
    def test_aborts_on_user_decline(self, kova_home, capsys, monkeypatch):
        ...
        with patch("kova_cli.dump.run_dump"), \
             patch("kova_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())

        mock_upload.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_proceeds_on_user_accept(self, kova_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, kova_home, capsys, monkeypatch):
    ...
    def test_non_interactive_requires_yes(self, kova_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, kova_home, capsys, monkeypatch):
    ...
    def test_nous_path_also_gated(self, kova_home, capsys, monkeypatch):
        ...
        with patch("kova_cli.dump.run_dump"), \
             patch("kova_cli.diagnostics_upload.share_to_nous") as mock_nous:
            run_debug_share(self._args(nous=True))

        mock_nous.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_local_never_prompts(self, kova_home, capsys, monkeypatch):
