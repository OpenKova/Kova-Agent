...
class TestMissingLogNote:
    ...
    `kova debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, kova_home):
        from kova_cli.debug import _capture_log_snapshot

        (kova_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, kova_home):
        from kova_cli.debug import _capture_log_snapshot
        ...
        assert "Kova Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(kova_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, kova_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, kova_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from kova_cli.debug import _capture_log_snapshot

        (kova_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, kova_home):
