...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `kova update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        monkeypatch.setattr(
            kova_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("KOVA_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("KOVA_HOME", str(named_profile))
            hm._update_node_dependencies()
