...
reset).  Kova must detect that and lift the stale local cooldown instead
...
def _write_auth_store(kova_home, payload):
...
def test_clear_cooldowns_only_touches_quota_shaped_entries(tmp_path, monkeypatch):
    ...
    store = json.loads((kova_home / "auth.json").read_text())
...
def test_clear_cooldowns_scoped_to_access_token(tmp_path, monkeypatch):
    ...
    persisted = json.loads((kova_home / "auth.json").read_text())
