...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(kova_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(kova_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    kova_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_kova_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import kova_constants

    monkeypatch.delattr(kova_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(kova_constants, "apply_subprocess_home_env")

    kova_main._reload_updated_runtime_modules()

    assert callable(kova_constants.apply_subprocess_home_env)
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    try:
        ...
        restored = kova_main._restore_stashed_changes(
