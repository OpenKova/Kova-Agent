...
# via the platform's native composite. The composite ``kova-discord``
...
def test_discord_composite_only_enables_discord_toolsets():
    """Layer 1: ``platform_toolsets.discord: [kova-discord]`` is an explicit
    opt-in to the full Discord bundle (which includes the ``discord`` and
    ``discord_admin`` tools). They must not be silently stripped."""
    config = {"platform_toolsets": {"discord": ["kova-discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled, "discord toolset missing from kova-discord composite"
...
def test_discord_composite_plus_configurable_enables_discord_toolsets():
    ...
    config = {"platform_toolsets": {"discord": ["kova-discord", "spotify"]}}
    enabled = _get_platform_tools(config, "discord")
...
def test_discord_composite_plus_partial_explicit_enables_sibling():
    """Layer 3: ``[kova-discord, discord]`` lists discord explicitly but
    discord_admin arrives only via the composite. Both must survive."""
    config = {"platform_toolsets": {"discord": ["kova-discord", "discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["kova-telegram", "discord"]}}
...
def test_discord_explicit_workaround_still_works():
    ...
    config = {
        "platform_toolsets": {"discord": ["kova-discord", "discord", "discord_admin"]}
    }
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/OpenKova/Kova-Agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_configure_all_platforms_configures_selected_tool_missing_provider(monkeypatch):
    """Regression: `kova tools` → Configure all platforms → Web Search
    ...
    monkeypatch.setattr("kova_cli.tools_config._prompt_choice", choose_by_label)
    monkeypatch.setattr(
        "kova_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web"},
    )
    monkeypatch.setattr(
        "kova_cli.tools_config._toolset_needs_configuration_prompt",
...
def test_configure_single_platform_configures_selected_tool_missing_provider(monkeypatch):
    """Regression (per-platform sibling of the global flow): `kova tools` →
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/OpenKova/Kova-Agent/issues/6901
    """
    config = {
...
def test_computer_use_post_setup_respects_custom_driver_command_when_installed():
    ...
    # bare-command miss after KOVA_CUA_DRIVER_CMD was looked up via which().
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    seq = iter([1, 0, 1])
    with patch("kova_cli.model_switch.list_authenticated_providers",
...
def test_provider_readiness_env_vars_gate_keys(monkeypatch):
    ...
    monkeypatch.setattr("kova_cli.tools_config.get_env_value", lambda key: None)
    assert provider_readiness_status(provider, {}) == "needs_keys"

    monkeypatch.setattr("kova_cli.tools_config.get_env_value", lambda key: "sk-x")
...
def test_provider_readiness_local_install_rows_track_module_presence(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.tools_config._module_installed", lambda name: False
    )
    assert provider_readiness_status(kitten, {}) == "needs_setup"
    assert provider_readiness_status(piper, {}) == "needs_setup"

    monkeypatch.setattr(
        "kova_cli.tools_config._module_installed", lambda name: True
    )
...
# `kova tools post-setup <key>` child. On Windows each console child (npm,
...
def test_post_setup_no_window_flags_zero_on_posix(monkeypatch):
    ...
    from kova_cli.tools_config import _post_setup_no_window_flags
...
def test_provider_readiness_agent_browser_tracks_local_install(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.nous_subscription._local_browser_runnable", lambda: False
    )
    assert provider_readiness_status(provider, {}) == "needs_setup"

    monkeypatch.setattr(
        "kova_cli.nous_subscription._local_browser_runnable", lambda: True
    )
    assert provider_readiness_status(provider, {}) == "ready"
...
def test_provider_readiness_cloud_browser_hook_tracks_cli_only(monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.nous_subscription._has_agent_browser", lambda: False
    )
    assert provider_readiness_status(provider, {}) == "needs_setup"

    monkeypatch.setattr(
        "kova_cli.nous_subscription._has_agent_browser", lambda: True
    )
    assert provider_readiness_status(provider, {}) == "ready"
