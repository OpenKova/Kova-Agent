...
def test_has_agent_browser_resolves_via_kova_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the Kova Node
    ...
    monkeypatch.setattr(
        "kova_constants.with_kova_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "kova_constants.agent_browser_runnable",
