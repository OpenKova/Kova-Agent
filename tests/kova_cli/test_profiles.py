...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("kova_cli.profiles._cleanup_gateway_service"), \
             patch("kova_cli.profiles.time.sleep"), \
    ...
    def test_stops_profile_bound_backends_before_removal(self, profile_env):
        ...
        with patch("kova_cli.profiles._cleanup_gateway_service"), \
             patch("kova_cli.profiles._profile_bound_backend_pids", return_value=[4242]) as pids, \
    ...
    def test_rmtree_retries_transient_enotempty_then_succeeds(self, profile_env):
        ...
        with patch("kova_cli.profiles._cleanup_gateway_service"), \
             patch("kova_cli.profiles._profile_bound_backend_pids", return_value=[]), \
             patch("kova_cli.profiles.time.sleep"), \
             patch("kova_cli.profiles.shutil.rmtree", side_effect=flaky_rmtree):
            delete_profile("coder", yes=True)
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "kova_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "kova_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "kova_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "kova_cli.main", "--profile", "coder", "serve"]),
        ]
...
class TestWrapperScriptSecurity:
    ...
    def test_legit_alias_stays_inside_wrapper_dir(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from kova_cli.profiles import _get_wrapper_dir
...
class TestExportImport:
    ...
    def test_export_default_uses_allowlist_for_unrelated_dirs(self, profile_env, tmp_path):
        """Unrelated directories under KOVA_HOME are excluded by allow-list (#58394).

        Docker/custom deployments often set KOVA_HOME to a working
        ...
        Kova artifacts end up in the archive. Replaces the old
