...
(mirroring the desktop build), recorded in a stamp file under $KOVA_HOME,
NOT mtime comparison — so ``git pull`` / ``kova update`` that rewrite
...
def _isolated_kova_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOVA_HOME", str(tmp_path / "_kova_home"))
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        ...
        wrong.write_text("{}")
        # Correct location (kova_cli/web_dist/) is empty -> still needs build.
        assert _web_ui_build_needed(web_dir) is True
    ...
    def test_mtime_only_change_is_not_stale(self, tmp_path):
        ...
        ``git pull`` / ``kova update`` do) must NOT report stale."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        src = web_dir / "src" / "App.tsx"
        src.parent.mkdir(parents=True, exist_ok=True)
        src.write_text("export const A = 1\n")
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_npm_install_fallback_forces_include_dev_and_no_save(self, tmp_path):
        ...
        with patch("kova_cli.main.subprocess.run", side_effect=[ci_fail, install_ok]) as mock_run:
            result = _run_npm_install_deterministic("/usr/bin/npm", web_dir)
...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_with_dist_serves_stale_without_building(self, tmp_path):
        import fcntl
        from kova_cli.main import _build_web_ui as build
        ...
        try:
            ...
            with patch("kova_cli.main._do_build_web_ui") as mock_do:
    ...
    def test_uncontended_lock_builds_and_creates_lock_file(self, tmp_path):
        ...
        web_dir, _ = _make_web_dir(tmp_path)
        with patch("kova_cli.main._do_build_web_ui", return_value=True) as mock_do:
            result = build(web_dir)
