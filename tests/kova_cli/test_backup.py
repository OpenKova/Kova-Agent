...
class TestQuickSnapshot:
    ...
    def test_copies_discord_recovery_ledger(self, kova_home):
        from kova_cli.backup import create_quick_snapshot

        gateway_dir = kova_home / "gateway"
        gateway_dir.mkdir()
        ...
        copied = kova_home / "state-snapshots" / snap_id / "gateway" / ledger.name
    ...
    def test_max_file_size_skips_oversized_file(self, kova_home, capsys):
        ...
        snap_id = create_quick_snapshot(
            kova_home=kova_home, max_file_size=cap
        )
        assert snap_id is not None
        snap_dir = kova_home / "state-snapshots" / snap_id
    ...
    def test_max_file_size_none_copies_everything(self, kova_home):
        """Default (no cap) preserves manual /snapshot behavior."""
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(kova_home=kova_home, max_file_size=None)
        assert (kova_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_max_file_size_under_cap_copies(self, kova_home):
        from kova_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(
            kova_home=kova_home, max_file_size=1 << 30
        )
        assert (kova_home / "state-snapshots" / snap_id / "state.db").exists()
...
class TestRunPreUpdateBackup:
    ...
    @staticmethod
    def _snaps(kova_home):
        d = kova_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []

    def test_default_creates_quick_snapshot_only(self, kova_home, capsys):
        ...
        assert self._snaps(kova_home)
        assert not self._zips(kova_home)

    def test_backup_flag_forces_full(self, kova_home, capsys):
        ...
        assert len(self._zips(kova_home)) == 1

    def test_no_backup_flag_skips_everything(self, kova_home, capsys):
        ...
        assert not self._snaps(kova_home)
        assert not self._zips(kova_home)

    def test_config_off_disables_everything_silently(self, kova_home, capsys):
    ...
    def test_legacy_false_maps_to_off(self, kova_home, capsys):
        """Legacy boolean ``false`` (the old zip opt-out) now means off."""
        self._set_mode(kova_home, False)
        from kova_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        assert snap_id is None
    ...
    def test_legacy_true_maps_to_full(self, kova_home, capsys):
        """Legacy boolean ``true`` (the old always-zip opt-in) means full."""
        self._set_mode(kova_home, True)
        from kova_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_config_full_mode(self, kova_home, capsys):
        self._set_mode(kova_home, "full")
        from kova_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_config_quick_mode(self, kova_home, capsys):
        self._set_mode(kova_home, "quick")
        from kova_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_unknown_mode_falls_back_to_quick(self, kova_home, capsys):
        self._set_mode(kova_home, "bogus-mode")
        from kova_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_no_backup_flag_overrides_full_config(self, kova_home, capsys):
    ...
    def test_backup_flag_overrides_off_config(self, kova_home, capsys):
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_bom_live_file_still_counted(self, tmp_path):
        ...
        from kova_cli.backup import _count_cron_jobs, restore_cron_jobs_if_emptied
        kova_home = tmp_path / ".kova"
        jobs_path = kova_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(kova_home)
