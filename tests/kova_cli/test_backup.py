...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew kova_cli.backup's datetime forward instead of sleeping.
    ...
    import kova_cli.backup as _backup

    shim = getattr(_backup.datetime, "_kova_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _kova_test_shim = True
...
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, kova_home, capsys):
        ...
        first_id = create_quick_snapshot(label="complete", kova_home=kova_home)
        assert first_id is not None
        first_dir = kova_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", kova_home=kova_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = kova_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, kova_home=kova_home)
