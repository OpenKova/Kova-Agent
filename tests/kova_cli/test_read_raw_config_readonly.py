...
def test_freshness_after_config_edit(isolated_kova_home):
    from kova_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_kova_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_kova_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_kova_home):
    from kova_cli.config import read_raw_config_readonly

    cfg = isolated_kova_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
