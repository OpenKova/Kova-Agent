"""Tests for the post-update *import* guard in ``kova update``.
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not kova_main's. Both modules expose
    ...
    syntax_ok, _, _ = kova_main._validate_critical_files_syntax(tmp_path)
    ...
    ok, module, error = kova_main._validate_critical_modules_import(tmp_path)
    assert ok is False
...
def test_import_guard_passes_on_consistent_tree(monkeypatch, tmp_path):
    ...
    assert kova_main._validate_critical_modules_import(tmp_path) == (True, None, None)
...
def test_import_guard_ignores_non_import_errors(monkeypatch, tmp_path):
    ...
    ok, _, _ = kova_main._validate_critical_modules_import(tmp_path)
    assert ok is True
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("kova update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``kova update`` can run under a different Python than the install's.
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    (third-party) matched the probe's loose `startswith("kova")`. A user
    ...
    """
    from kova_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
