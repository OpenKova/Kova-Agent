...
and the ``kova kanban repair`` CLI verb."""
...
# ---------------------------------------------------------------------------
# repair_db() API + `kova kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `kova kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated KOVA_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".kova"
    home.mkdir()
