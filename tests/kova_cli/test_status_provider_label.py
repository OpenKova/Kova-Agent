"""`kova status` provider label honors config.yaml model.base_url (#3296)."""

from unittest.mock import patch

from kova_cli.status import _effective_provider_label


def _label_with(config, env_base_url=""):
    with patch("kova_cli.status.resolve_requested_provider", return_value="auto"), \
         patch("kova_cli.status.resolve_provider", return_value="openrouter"), \
         patch("kova_cli.status.load_config", return_value=config), \
         patch("kova_cli.status.get_env_value",
...
def test_non_openrouter_provider_untouched():
    with patch("kova_cli.status.resolve_requested_provider", return_value="anthropic"), \
         patch("kova_cli.status.resolve_provider", return_value="anthropic"):
