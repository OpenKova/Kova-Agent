...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull OpenKova/Kova-Agent:latest" in hint
        assert "recreate all Kova containers" in hint
        assert "kova update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `kova update`" in hint
...
def _run_doctor_with_managed_agent_browser(monkeypatch, tmp_path, runnable):
    ...
    Kova-managed node bin (~/.kova/node/bin), not on PATH or in
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        ...
        leaves doctor probing the REAL ~/.kova. On a dev machine with a
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_kova_tool_progress_warning_says_unsupported_since_floor(self):
        """KOVA_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"KOVA_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["KOVA_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"KOVA_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["KOVA_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"
