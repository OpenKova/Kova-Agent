...
class TestCredentialPoolEndpoints:
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `kova auth add`, which clears every suppression for the
    ...
    def test_delete_does_not_clobber_other_providers(self):
        ...
        from agent.credential_pool import load_pool
        from kova_cli.auth import _load_auth_store, read_credential_pool
        from kova_cli.config import save_env_value
...
class TestOpsEndpoints:
    ...
    def test_backup_output_uses_output_flag(self, monkeypatch):
        ...
        r = self.client.post(
            "/api/ops/backup",
            json={"output": "  /tmp/kova-test.zip  "},
        )

        assert r.status_code == 200
        assert captured == {
            "subcommand": ["backup", "-o", "/tmp/kova-test.zip"],
            "name": "backup",
        }
...
# ---------------------------------------------------------------------------
# _spawn_kova_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_kova_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _KOVA_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    ws._spawn_kova_action(["gateway", "restart"], "gateway-restart")

    assert "_KOVA_GATEWAY" not in captured["env"]
    assert captured["env"]["KOVA_NONINTERACTIVE"] == "1"
