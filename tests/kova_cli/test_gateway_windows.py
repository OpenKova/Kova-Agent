...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    # Current task is deleted first, then any other candidate names (the
    # profile-less Kova_Gateway and the legacy Kova_Gateway), then re-created
    # from scratch.
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Kova_Gateway_alice")
    assert calls[1][:4] == ("/Delete", "/F", "/TN", "Kova_Gateway")
    assert calls[2][:4] == ("/Delete", "/F", "/TN", "Kova_Gateway")
    assert calls[3][0] == "/Create"
    assert "/XML" in calls[3]
    assert "/SC" not in calls[3]
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_exec_schtasks",
        # /Query succeeds (task registered), /Delete is access-denied.
        lambda args: calls.append(("schtasks", tuple(args))) or (
            (0, "", "") if args[0] == "/Query" else (1, "", "ERROR: Access is denied.")
        ),
    )
    monkeypatch.setattr(gateway_windows, "_is_running_as_admin", lambda: False)
    monkeypatch.setattr(setup, "prompt_yes_no", lambda prompt, default=True: calls.append(("prompt", prompt, default)) or True)
