...
def _install_example_plugin(_isolate_kova_home):
    ...
    # plugin in exactly as a real operator would with `kova plugins
    # enable example`.
...
class TestWebServerEndpoints:
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(kova_state, "DEFAULT_DB_PATH", fake_db_path)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(kova_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")

        def _boom(*a, **k):
            raise AssertionError("SessionDB must not be constructed when db file is absent")

        monkeypatch.setattr("kova_state.SessionDB", _boom)
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide KOVA_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from kova_constants import get_kova_home

        path = get_kova_home() / "honcho.json"
    ...
    def test_get_honcho_config_returns_safe_defaults(self, monkeypatch, tmp_path):
        ...
        assert data["docs_url"] == "https://docs.honcho.dev/v3/guides/integrations/kova"
        ...
        assert fields["workspace"]["placeholder"] == "kova"
        assert fields["aiPeer"]["placeholder"] == "kova"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_kova_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["kova"]["workspace"] == "myws"
        assert cfg["hosts"]["kova"]["peerName"] == "eri"
        assert cfg["hosts"]["kova"]["environment"] == "local"
        assert cfg["hosts"]["kova"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["kova"]["apiKey"] == "hch-test-key"
    ...
    def test_put_honcho_blank_text_clears_key(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_kova_home() / "honcho.json").read_text(encoding="utf-8"))
        assert "workspace" not in cfg.get("hosts", {}).get("kova", {})
    ...
    def test_put_honcho_partial_save_preserves_other_keys(self, monkeypatch, tmp_path):
        ...
        host = json.loads((get_kova_home() / "honcho.json").read_text(encoding="utf-8"))["hosts"]["kova"]
    ...
    def test_put_honcho_number_stored_as_native_number(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_kova_home() / "honcho.json").read_text(encoding="utf-8"))
        assert cfg["hosts"]["kova"]["dialecticMaxChars"] == 1200
        assert isinstance(cfg["hosts"]["kova"]["dialecticMaxChars"], int)
    ...
    def test_put_honcho_first_save_merges_into_resolved_config(self, monkeypatch, tmp_path):
        ...
        global_path.write_text(
            json.dumps({"baseUrl": "https://kept.example", "hosts": {"kova": {"workspace": "kept"}}}),
            encoding="utf-8",
        )
        ...
        assert resp.status_code == 200
        assert not (get_kova_home() / "honcho.json").exists()
        cfg = json.loads(global_path.read_text(encoding="utf-8"))
        assert cfg["baseUrl"] == "https://kept.example"
        assert cfg["hosts"]["kova"] == {"workspace": "kept", "peerName": "eri"}
    ...
    def test_put_honcho_updates_legacy_dot_form_host_block(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("KOVA_HONCHO_HOST", "kova_work")

        path = self._seed_local_honcho({"hosts": {"kova.work": {"workspace": "w", "peerName": "eri"}}})
        ...
        assert set(hosts) == {"kova.work"}
        assert hosts["kova.work"] == {"workspace": "w", "peerName": "eri", "sessionStrategy": "per-repo"}
    ...
    def test_put_honcho_api_key_never_overwrites_oauth_token(self, monkeypatch, tmp_path):
        ...
        path = self._seed_local_honcho({"hosts": {"kova": {"apiKey": "hch-at-oauth-token"}}})
        ...
        assert cfg["hosts"]["kova"]["apiKey"] == "hch-at-oauth-token"
    ...
    def test_put_honcho_tolerates_null_hosts(self, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert json.loads(path.read_text(encoding="utf-8"))["hosts"]["kova"]["workspace"] == "myws"
    ...
    def test_memory_provider_config_honors_profile_param(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        # The suite pins KOVA_HONCHO_HOST=kova; this test exercises
        # profile-driven host resolution, so drop the override explicitly.
        monkeypatch.delenv("KOVA_HONCHO_HOST", raising=False)
        from kova_constants import get_kova_home
        ...
        worker_cfg.write_text(json.dumps({"hosts": {"kova_worker": {"workspace": "kept"}}}), encoding="utf-8")
        ...
        own = json.loads((get_kova_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_kova_home() / "images"
    ...
    def test_update_kova_returns_nix_guidance_without_spawning(self, monkeypatch):
        import kova_cli.web_server as web_server

        def fail_spawn(*_args, **_kwargs):
            raise AssertionError("Nix update guard should not spawn kova update")
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["kova-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/kova-update/status?lines=3")
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_kova_home() / "backups"
        assert archive.name.startswith("kova-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_kova_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import kova_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("KOVA_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "kova-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_kova_action", fail_spawn)
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`kova serve` (KOVA_SERVE_HEADLESS) must NOT serve the SPA even
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.setenv("KOVA_SERVE_HEADLESS", "1")
        app_ = FastAPI()
        ws.mount_spa(app_)
    ...
    def test_set_model_main_preserves_base_url_for_named_custom_provider(self):
        ...
        save_config({
            "model": {"provider": "nous", "default": "kova-4"},
            "providers": {
                "axet-proxy": {
                    "name": "Axet Proxy",
                    "base_url": "http://127.0.0.1:8081/v1",
...
class TestBuildSchemaFromConfig:
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see kova_cli/config.py).
...
class TestNewEndpoints:
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "kova"
        save_config(config)

        body = self.client.get("/api/tools/terminal/backends").json()
        ssh = next(r for r in body["backends"] if r["name"] == "ssh")
        assert ssh["status"] == "ready"
        assert "kova@devbox.example.com" in ssh["detail"]
...
class TestDiscoverUserThemes:
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_kova_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
            reset_kova_home_override(token)
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(kova_home, name="ocean"):
        themes_dir = kova_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="kova-theme-bootstrap">')
    ...
    def test_serve_index_no_bootstrap_for_builtin_theme(self, tmp_path, monkeypatch):
        ...
        resp = client.get("/chat")
        assert resp.status_code == 200
        assert "kova-theme-bootstrap" not in resp.text
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        KOVA_HOME override scopes a request to another profile."""
        from kova_constants import (
            reset_kova_home_override,
            set_kova_home_override,
        )
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_sets_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.delenv("KOVA_PYTHON_SRC_ROOT", raising=False)
        monkeypatch.delenv("KOVA_PYTHON", raising=False)
        monkeypatch.delenv("KOVA_CWD", raising=False)
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
        ...
        assert env["KOVA_PYTHON_SRC_ROOT"] == str(main_mod.PROJECT_ROOT)
        assert env["KOVA_PYTHON"] == sys.executable
        assert env["KOVA_CWD"] == os.getcwd()
    ...
    def test_resolve_chat_argv_replaces_invalid_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("KOVA_PYTHON_SRC_ROOT", "/definitely/missing/kova-src")
        monkeypatch.setenv("KOVA_PYTHON", "/definitely/missing/python")
        monkeypatch.setenv("KOVA_CWD", "/definitely/missing/cwd")
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_resolve_chat_argv_keeps_relative_python_under_tui_cwd(
        ...
        monkeypatch.setenv("KOVA_CWD", str(tmp_path))
        monkeypatch.setenv("KOVA_PYTHON", str(relative_python))
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        assert env["KOVA_PYTHON"] == command
    ...
    def test_resolve_chat_argv_falls_back_when_getcwd_is_missing(self, monkeypatch, tmp_path):
        ...
        assert env is not None
        assert env["KOVA_CWD"] == str(tmp_path)
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("KOVA_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
