...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("kova_cli.config.sanitize_env_file", return_value=2),
            patch(
                "kova_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("kova_cli.config.read_raw_config", return_value={}),
            patch("kova_cli.config.get_missing_env_vars", return_value=[]),
            patch("kova_cli.config.get_missing_config_fields", return_value=[]),
            patch("kova_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `kova setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_kova_home(self):
        from kova_cli.config_migrations import support_floor_message
        from kova_constants import display_kova_home

        msg = support_floor_message()
        assert f"{display_kova_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from kova_cli.config_migrations import run_migrations
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from kova_cli.config import _persist_migration, read_raw_config
