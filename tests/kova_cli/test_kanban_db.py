...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # KOVA_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is KOVA_SESSION_SOURCE, which the dispatcher
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import kova_state as _kova_state
    monkeypatch.setattr(
        _kova_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _kova_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("kova_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="kova_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        kova_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "kova_cli.kanban_db.sqlite3.connect",
