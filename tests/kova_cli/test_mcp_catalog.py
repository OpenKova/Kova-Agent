...
def _isolate_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kova_cli.config.get_hermes_home", lambda: hh
    )
    monkeypatch.setattr(
    ...
    # mcp_catalog grabs get_hermes_home() lazily through kova_constants
    monkeypatch.setattr(
        "kova_constants.get_hermes_home", lambda: hh
    )
    return hh
...
class TestManifestParsing:
    ...
    def test_transport_env_parsed_and_written_to_server_config(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from kova_cli.mcp_catalog import _build_server_config
...
class TestShippedCatalog:
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Kova fetches/launches is pinned
        to an exact version.
