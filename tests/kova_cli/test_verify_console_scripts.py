...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("hermes", "kova-agent", "hermes-acp"):
            (fake_scripts_dir / f"{name}.exe").write_bytes(b"fake")

        with patch("kova_cli.main._is_windows", return_value=True), \
             patch("kova_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("kova_cli.main._run_quarantined_install") as mock_install:
            from kova_cli.main import _verify_console_scripts_installed
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("kova_cli.main._is_windows", return_value=False), \
             patch("kova_cli.main._run_quarantined_install") as mock_install:
            from kova_cli.main import _verify_console_scripts_installed
    ...
    def test_load_console_script_names_reads_pyproject(self, temp_pyproject):
        from kova_cli.main import _load_console_script_names

        names = _load_console_script_names()
        assert names == ["hermes", "kova-agent", "hermes-acp"]
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("kova_cli.main._is_windows", return_value=False), \
             patch("kova_cli.main._run_quarantined_install") as mock_install, \
             patch("kova_cli.main._verify_console_scripts_installed") as mock_verify:
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        assert {"hermes.exe", "kova-agent.exe", "hermes-acp.exe"} <= names
