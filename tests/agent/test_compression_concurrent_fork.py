...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``kova_state`` module, whose ``SessionDB``
...
_NominalSessionDBImpostor.__module__ = "kova_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``kova_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = kova_state.SessionDB
    monkeypatch.setattr(kova_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(kova_state, "SessionDB", real_session_db_type)
