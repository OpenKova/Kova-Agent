...
path via ``kova chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp KOVA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point KOVA_HOME at a temp dir and turn moa.save_traces on."""
    kova_home = tmp_path / ".kova"
    kova_home.mkdir()
    monkeypatch.setenv("KOVA_HOME", str(kova_home))

    # save_moa_turn reads config via kova_cli.config.load_config; stub it to
