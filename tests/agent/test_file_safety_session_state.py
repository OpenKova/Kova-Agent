...
manipulation. Kova keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point KOVA_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_kova_home /
    get_default_kova_root) instead of monkeypatching private helpers —
    ...
    KOVA_HOME=<root>/profiles/<name> makes get_default_kova_root()
