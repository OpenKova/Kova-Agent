...
developer's REAL ~/.kova/state.db, and a pytest-spawned process flipped
...
REAL_ROOT = (Path.home() / ".kova").resolve()


class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.kova/state.db must fail hard."""
    ...
    def test_unnormalized_production_path_raises(self):
        ...
        sneaky = Path.home() / "subdir" / ".." / ".kova" / "state.db"
        with pytest.raises(RuntimeError, match="live-system guard"):
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(
            kova_state, "DEFAULT_DB_PATH", kova_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_kova_home_default_resolution_works(self, tmp_path, monkeypatch):
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        kova_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSubprocessChildCovered:
    def test_child_without_kova_home_is_refused(self, tmp_path):
    ...
    def test_child_with_tmp_kova_home_succeeds(self, tmp_path):
