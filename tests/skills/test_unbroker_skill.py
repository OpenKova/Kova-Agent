...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and kova-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # kova-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_kova_home():
    prev = os.environ.get("KOVA_HOME")
    with tempfile.TemporaryDirectory() as d:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("KOVA_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
