...
from _kova_home import get_kova_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("KOVA_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_kova_home() / "cache" / "citations" / "ledger.json"
