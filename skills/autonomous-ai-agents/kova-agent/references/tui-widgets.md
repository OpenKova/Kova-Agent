...
Author widget apps for the Kova TUI (`kova --tui`): glanceable ambient
...
- The TUI must be in use (`kova --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.kova/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.kova/logs/tui_gateway_crash.log` if a widget never appears.
