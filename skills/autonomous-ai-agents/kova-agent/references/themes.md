...
`~/.kova/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Kova itself.
...
- Write access to the Kova home dir — `~/.kova` by default, or `$HERMES_HOME`
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   kova config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `kova config set display.skin default` (or they can `/skin default`).
...
```
kova skin set <key> <hex>      # e.g. kova skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `kova skin set background "#08201f"` changes
only the background; `kova skin use <name>` / `kova skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.kova`** when a profile is active — resolve the real home
  from `$HERMES_HOME` first, falling back to `~/.kova`.
...
- **Never hand-edit `config.yaml` to activate.** Use `kova config set
...
- **You apply it, not the user.** `kova config set display.skin <name>` is
