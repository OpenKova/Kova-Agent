...
## Layer 1 — Is this thing a Kova plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Kova process**. Its LLM calls go through Kova's already-configured provider. | Already uses Portal if user's Kova is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Kova. That LLM call is already authenticated
via Kova's provider config. The plugin's own server auth (e.g.
...
What Kova Agent has that *feels* like OAuth — `kova login --provider nous`
opening a browser, user signs in, token lands in `~/.kova/auth.json` — is a
**Kova-specific browser flow**. Under the hood it produces a credential
Kova uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Kova's existing Portal credential out of `~/.kova/auth.json`.
...
rides on the credential Kova already holds.

Where this could live in Kova:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Kova itself.
...
1. First decide: Kova plugin (runs inside Kova) or separate app? If plugin,
   it already uses Portal via Kova's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Kova agent itself") is fully supported. Kova treats agent
...
pitch in the Tool Gateway announcement is specifically about Kova Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Kova is configured to use Portal as its provider. It is
