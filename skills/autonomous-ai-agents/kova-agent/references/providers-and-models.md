...
Full docs: https://kova-agent.kova.ai/docs/integrations/providers
...
| nous | OAuth device code | `kova auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `kova auth add openai-codex` |
| qwen-oauth | OAuth | `kova auth add qwen-oauth` |
| minimax-oauth | OAuth | `kova auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`kova auth`).
Fallback chain when the primary fails: `kova fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`kova_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   kova config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
