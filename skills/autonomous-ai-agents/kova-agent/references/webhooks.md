...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.kova/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.kova/scripts/` (path traversal is blocked).
...
Full filter syntax: https://kova-agent.kova.ai/docs/user-guide/messaging/webhooks#payload-filters
