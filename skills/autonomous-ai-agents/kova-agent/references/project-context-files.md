...
- **Don't put project rules in `~/.kova/AGENTS.md`** (or any other home-level location). When Kova runs with that directory as cwd, the file loads — but only for that one directory. For cross-project context, use `SOUL.md` (in `$HERMES_HOME`, identity-only) or install a skill via `kova skills install`.
