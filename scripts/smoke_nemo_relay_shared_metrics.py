"""Run a real Kova CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_kova_executable(kova_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "kova",
        Path(".venv") / "Scripts" / "kova.exe",
    ):
        candidate = kova_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("kova")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Kova executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Kova source checkout containing .venv/bin/kova",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["kova.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['kova.client.active']}"
        )
    [model] = by_name["kova.model_route.count"]
    expected_model = {
        "name": "kova.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['kova.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["kova.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['kova.task_run.started']}"
        )
    [terminal] = by_name["kova.task_run.finished"]
    ...
    [tool] = by_name["kova.tool_call.count"]
    ...
    lifecycle = by_name["kova.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["kova.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Kova development environment requires jsonschema"
        ) from exc
    ...
    if metrics["kova.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['kova.client.active']}"
        )
    [model] = metrics["kova.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['kova.model_route.count']}"
        )
    [terminal] = metrics["kova.task_run.finished"]
    ...
    [tool] = metrics["kova.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["kova.skill.lifecycle.count"]
    ...
    loads = metrics["kova.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    kova_repo = args.kova_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    kova = _resolve_kova_executable(kova_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="kova-relay-shared-metrics-"))
    home = root / "kova-home"
    ...
    (root / "kova.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "kova.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Kova exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Kova model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Kova did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Kova did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from kova_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "kova.shared_metrics.v2.schema.json",
    )

    print("Kova -> NeMo Relay shared-metrics smoke test passed")
