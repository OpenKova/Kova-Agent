...
batch (tracker: [#77056](https://github.com/OpenKova/Kova-Agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Kova home, same model, same tasks, same reps.
...
1. Create a dedicated Kova home with credentials for the models under test:
   ...
   HERMES_HOME=$ABEVAL_HOME kova plugins enable observability/nemo_relay
   ```
