...
measured in the production session DB) through `kova chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    HERMES_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Kova home with credentials for the
...
(tracker: OpenKova/Kova-Agent#77056).
"""
import json
import os
import re
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "KOVA_NEMO_RELAY_ATOF_ENABLED": "1",
                "KOVA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "KOVA_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "KOVA_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "kova_cli.main", "chat", "--query", q,
