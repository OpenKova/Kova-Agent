...
_nb_npm_range() {
    if [ -n "${KOVA_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$KOVA_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through kova_cli/npm_engine.py; the installer path
...
_nb_install_bundled_node() {
    ...
    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # KOVA_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${KOVA_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
...
# The managed node runs but is below KOVA_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$HERMES_HOME/node/bin/node" "$HERMES_HOME/node/node"; do
        ...
        [ "$major" -lt "$KOVA_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
