...
# (KOVA_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.kova/kova-agent/venv has bin/activate but no pytest, so an
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${KOVA_PYTHON:-}" ] && [ -x "$KOVA_PYTHON" ] \
    && "$KOVA_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: KOVA_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `kova` binary rather
  # than the devShell hook.
  PYTHON="$KOVA_PYTHON"
  echo "▶ no local venv — using Nix dev venv via KOVA_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and KOVA_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
#
#   * KOVA_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * KOVA_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     KOVA_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# KOVA_RUN_SLOW_PET_TESTS / KOVA_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no KOVA_TEST_* glob) so the "no
...
for _test_var in KOVA_TEST_IMAGE KOVA_TEST_WORKERS KOVA_TEST_PATHS \
  KOVA_TEST_FILE_TIMEOUT KOVA_TEST_FILE_RETRIES KOVA_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  ${KOVA_E2E_BROWSER:+KOVA_E2E_BROWSER="$KOVA_E2E_BROWSER"} \
