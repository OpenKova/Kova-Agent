...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[kova] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[kova`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[kova`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        HermesHome = $(if ($paths) { $paths.kova_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\kova\kova-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-kova'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-kova') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
