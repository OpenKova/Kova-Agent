...
#
# Usage:
#   curl -fsSL https://kova-agent.neuralstudio.in/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:OpenKova/Kova-Agent.git"
REPO_URL_HTTPS="https://github.com/OpenKova/Kova-Agent.git"
HERMES_HOME="${HERMES_HOME:-$HOME/.kova}"
...
#   data still at /root/.kova (HERMES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --kova-home PATH  Data directory (default: ~/.kova, or \$HERMES_HOME)"
...
print_banner() {
    ...
    echo "│             ⚕ Kova Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Neural Studio.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://kova-agent.neuralstudio.in/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Kova owns its own uv at $HERMES_HOME/bin/uv.  Always install there —
...
# with the Kova-managed Node $NODE_VERSION.
...
check_node() {
    ...
    if command -v node &> /dev/null && node_satisfies_build "$(node --version)"; then
        ...
        log_warn "min-release-age-exclude) — installing Kova-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Kova requires Node >=26) — installing Kova-managed Node $NODE_VERSION..."
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            # cannot succeed — mirror ``kova update`` and reset to the
...
setup_path() {
    ...
    if [ ! -x "$KOVA_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$KOVA_ENTRYPOINT" ]; }; then
        log_warn "Kova launcher prerequisites not found"
...
EOF
    ...
    # Also expose `kova-agent`. The `kova-agent` console script declared in
    ...
    rm -f "$command_link_dir/kova-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/kova-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$KOVA_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/kova-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$KOVA_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/kova-agent"
    log_success "Installed kova-agent launcher → $command_link_display_dir/kova-agent"

    # Also expose `kova-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `kova-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Kova as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/kova-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/kova-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$KOVA_BIN" "$KOVA_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/kova-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/kova-acp"
    log_success "Installed kova-acp launcher → $command_link_display_dir/kova-acp"
...
maybe_start_gateway() {
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $KOVA_CMD gateway > "$HERMES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.kova-bootstrap-complete, which tells the Kova
    ...
    local marker_path="$INSTALL_DIR/.kova-bootstrap-complete"
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Kova-managed Node to PATH, so
    ...
    # `kova desktop`, so install/repair and self-update agree about the app's
...
from kova_cli.main import _desktop_macos_relaunchable_fixup
