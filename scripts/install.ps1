...
$RepoUrlSsh = "git@github.com:OpenKova/Kova-Agent.git"
$RepoUrlHttps = "https://github.com/OpenKova/Kova-Agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Neural Studio.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Kova setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/OpenKova/Kova-Agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/OpenKova/Kova-Agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/OpenKova/Kova-Agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                ...
                if ($extractedDir) {
                    ...
                    # `kova update` (see the notes at the shared clone-path
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m kova_cli.main <command>"
                    } else {
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\kova_cli\web_server.py" 2>&1 | Out-Null
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: kova_cli/web_server.py"
        }
    }
...
function Install-Desktop {
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, kova-agent#38216).
