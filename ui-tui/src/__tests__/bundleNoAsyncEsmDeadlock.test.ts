...
describe('TUI bundle (issue #31227)', () => {
  ...
  it('does not bundle the upstream ink package or ink-text-input', () => {
    ...
    // that #31227 was about. The in-tree fork at @kova/ink replaces
    // all of `ink`; nothing in ui-tui imports `TextInput` from
    // `@kova/ink` so the re-export is unused dead weight.
  ...
  it('has the @kova/ink entry-exports module compiled to sync init', () => {
    // Sanity check that the alias swap to packages/kova-ink/src/entry-exports.ts
    // is still active and producing the expected synchronous init shape.
    expect(bundleSrc).toMatch(/var init_entry_exports = __esm\(\{\s*"packages\/kova-ink\/src\/entry-exports\.ts"\(\)/)
  })
})
