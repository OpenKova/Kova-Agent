...
/**
 * User widget apps — Kova authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$KOVA_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.kova/plugins/`: files under KOVA_HOME execute
...
const widgetsDir = () => join(process.env.KOVA_HOME?.trim() || join(homedir(), '.kova'), 'tui-widgets')
 ...
 *  every change, so a widget Kova writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Kova) ever writes must hot-load too — a 10s poll here read
