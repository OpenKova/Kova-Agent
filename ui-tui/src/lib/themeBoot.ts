 ...
 * `kova-boot-background` / `kova-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports KOVA_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.kova matches get_kova_home()'s default.
const bootFilePath = () => join(process.env.KOVA_HOME ?? join(homedir(), '.kova'), 'tui-theme-boot.json')

// Never touch the user's real ~/.kova from test runs (the TS suite has no
// KOVA_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into KOVA_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.KOVA_TUI_THEME || env.KOVA_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.KOVA_TUI_BACKGROUND
  ) {
    env.KOVA_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.KOVA_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.KOVA_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into KOVA_TUI_THEME.
