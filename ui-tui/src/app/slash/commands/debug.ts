...
import { terminalBackgroundHex } from '@kova/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $KOVA_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['KOVA_TUI_BACKGROUND', process.env.KOVA_TUI_BACKGROUND ?? '(unset)'],
            ['KOVA_TUI_THEME', process.env.KOVA_TUI_THEME ?? '(unset)'],
