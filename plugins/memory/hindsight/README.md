...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `hermes-{profile}` isolates memory per active Kova profile. Empty placeholders collapse cleanly (e.g. `hermes-{user}` with no user becomes `kova`). |
