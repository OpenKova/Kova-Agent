...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Kova
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Kova coalesces
...
- **Native polls are supported.** Kova posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `kova send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<kova-home>/runtime/photon-sidecar.json` (written after the sidecar's
