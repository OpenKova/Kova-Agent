...
//
// Inbound  (gRPC -> Kova): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Kova -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Kova-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `kova photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Kova can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "kova-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Kova'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Kova allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Kova, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-kova-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Kova supplied them so a known-good
