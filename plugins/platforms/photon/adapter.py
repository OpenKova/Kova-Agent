...
#
# Out-of-process senders (cron subprocesses, `kova send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_kova_home() honors profile overrides — never hardcode ~/.kova.
    from kova_constants import get_kova_home

    return get_kova_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (kova status, test collection, plugin discovery).
...
# artifacts so Kova sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `kova photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `kova photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `kova setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, kova status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: kova photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: kova photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``kova photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Kova does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `kova photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `kova photon setup`)"
                )
        # A `kova update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `kova send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`kova send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Kova gateway (which spawns the sidecar and "
                    "records its address under <kova-home>/runtime/"
