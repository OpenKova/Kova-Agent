...
# identify KovaAgent traffic — matching other Kova outbound surfaces
# that already set ``KovaAgent/<version>`` for platform-partner attribution.
try:
    from kova_cli import __version__ as _KOVA_VERSION
except Exception:
    _KOVA_VERSION = "unknown"
_KOVA_SLACK_USER_AGENT_PREFIX = f"KovaAgent/{_KOVA_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `kova slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_KOVA_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Kova has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Kova can "
            ...
            self._app.action("kova_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^kova_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("kova_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'kova slack manifest' if unsure), and (b) the other bot's "
    ...
    async def _handle_app_context_changed(
        ...
        # Kova owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_kova_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_kova_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_kova_no_thread_response"):
        ...
        force_process = bool(event.get("_kova_force_process"))
        ...
        # the shape produced by peer Kova agents in Socket Mode on some
    ...
    async def send_clarify(
        ...
        (unique ``kova_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``kova_clarify_other``).  A choice click resolves the clarify
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "kova_clarify_other" or token == "other":
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_kova_home() / "slack_tokens.json"
