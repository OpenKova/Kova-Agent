# A2A — Agent-to-Agent protocol for Kova
...
peer (another Kova, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
kova gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Kova serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.kova/a2a_audit.jsonl`.
- Conversations persist to `~/.kova/a2a_conversations/` — they survive context
