...
from kova_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from kova_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from kova_state_portability import SessionPortabilityMixin
from kova_state_schema import SessionSchemaMixin
from kova_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``kova -c``/``--resume`` to continue the most recent session in
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.kova`` even when a test fixture later
    ...
    2. Otherwise resolve ``get_kova_home()`` fresh so a runtime
...
# ~/.kova/state.db accumulated pytest fixture rows — sessions with
...
#: ``~/.kova``).  The test conftest injects the pre-sandbox production
...
def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Kova root for the guard.

    Deliberately avoids ``Path.home()`` / ``kova_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``kova_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "kova"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "kova"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".kova"
        return root.resolve()
    except Exception:
        return None
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Kova home *root*.
    ...
    under ``~/.kova/kova-agent/...``) so hermetic tests using unusual
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Kova root {root}). "
...
def _wal_reset_repair_hint() -> str:
    ...
    ``kova update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Kova-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Kova"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Kova-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `kova doctor`. This warning fires once per "
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with kova_cli.config.
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``kova_cli.sqlite_safe_read``).
    ...
    try:
        from kova_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Kova home tree (``get_kova_home()``) — the safe repair scope:
      Kova owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``kova_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_kova_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Kova needs read-write access to open the database. "
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("KOVA_FTS5_CJK_SO")
    if env:
...
def _cjk_fts_config_enabled() -> bool:
    ...
    return os.getenv("KOVA_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship kova_state without kova_cli). A
    ...
    """
    try:
        from kova_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "kova_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch kova_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``kova_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from kova_cli.backup import is_zeroed_sqlite_file
    ...
    if size <= 0:
        return False
    from kova_cli.sqlite_safe_read import read_header_bytes_preopen
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `kova snapshot list` / "
                "`kova snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Kova
    ...
    # `kova update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `kova snapshot list` / "
                    f"`kova snapshot restore <id>` if available. "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Kova process held the "
    ...
    # ── Opt-in v23 FTS storage optimization (`kova sessions optimize-storage`) ──
    #
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``kova --resume`` into a fresh
    ...
    def search_sessions(
        ...
        ``kova -c``/``--resume`` so the "last" session is the last one in
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``KOVA_SESSION_SOURCE``, so their runs
