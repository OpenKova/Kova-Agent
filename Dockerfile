...
# node:26 image instead (Kova pins its toolchain to Node 26 everywhere).
...
sys.exit('SQLite FTS5 trigram self-test failed') if db.execute(\"SELECT count(*) FROM docs WHERE docs MATCH 'kov'\").fetchone()[0] != 1 else None; \
db.close()"
...
#
# The [otlp] extra contains the SDK/exporter imported by Kova when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Kova production image.
#
...
COPY --chmod=0755 docker/kova-exec-shim.sh /opt/kova/bin/kova
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/kova/docker/entrypoint-dispatch.sh
...
ENTRYPOINT [ "/opt/kova/docker/entrypoint-dispatch.sh" ]
CMD [ ]
