...
| `kova secrets bitwarden status` | 显示配置、二进制版本，以及令牌是否存在/是否通过校验 |
| `kova secrets bitwarden token` | 轮换访问令牌：先向 Bitwarden 验证新令牌，验证通过后再写入 `.env` |
...
```bash
kova secrets bitwarden token                     # 隐藏输入提示
kova secrets bitwarden token --access-token 0.…  # 非交互式
```
...
| `Bitwarden rejected the machine-account access token … invalid_client` | 令牌已吊销、过期、机器账户被删除——或令牌属于其他区域（例如欧盟令牌访问了美国 identity 端点） | 运行 `kova secrets bitwarden token` 粘贴新令牌；区域不匹配时重新运行 setup 选择欧盟/自托管（或设置 `secrets.bitwarden.server_url`） |
| `bws exited 1: invalid access token` | 令牌已吊销或有误 | 运行 `kova secrets bitwarden token` 提供新令牌 |
