...
若两个键均未设置，Hermes 将根据环境中存在的 API key/URL 自动检测后端。`kova tools` 会引导用户完成选择。
...
| `requires_env` | 在 `kova plugins install` 期间进行交互式凭据提示（富格式说明参见[构建 Hermes 插件](/developer-guide/plugins#gate-on-environment-variables)） |
