...
想让 Kova 拥有稳定的默认风格？编辑 `~/.kova/SOUL.md`（如果使用自定义 Kova home，则为 `$HERMES_HOME/SOUL.md`）。Kova 现在会自动生成一个初始 SOUL 文件，并将该全局文件作为实例级个性来源。
...
:::tip
默认情况下，消息平台的会话永不自动重置 —— 上下文会一直保留，直到你手动 `/reset` 或触发上下文压缩。如需自动重置（空闲超时或每天固定时间），可在 `~/.kova/config.yaml` 的 `session_reset` 部分选择启用。
:::
