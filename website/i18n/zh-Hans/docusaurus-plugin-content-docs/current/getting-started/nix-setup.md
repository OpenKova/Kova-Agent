...
执行 `nix profile install` 后，`hermes`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`kova setup` 引导你完成提供商选择，`kova gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/`。
...
- 路由是透明的：`kova chat`、`hermes sessions list`、`kova version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it hermes-agent kova config set ...` 也会被屏蔽）
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `hermes-agent` 二进制文件存在且 `kova version` 可运行 |
...
| `kova version` 显示旧版本 | 容器未重启 | `systemctl restart hermes-agent` |
