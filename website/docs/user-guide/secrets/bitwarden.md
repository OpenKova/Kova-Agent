...
| `kova secrets bitwarden status` | Show config + binary version + token presence/validation |
| `kova secrets bitwarden token` | Rotate the access token: validate the new token against Bitwarden, then store it in `.env` |
...
Bitwarden Secrets Manager: → Run `kova secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
kova secrets bitwarden token                     # masked prompt
kova secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.kova/cache/bws_cache.enc.json`. |
...
| `Bitwarden rejected the machine-account access token … invalid_client` | Token revoked, expired, machine account deleted — or the token belongs to another region (e.g. EU token hitting the US identity endpoint) | Run `kova secrets bitwarden token` to paste a fresh token; for region mismatches re-run setup and pick EU/self-hosted (or set `secrets.bitwarden.server_url`) |
| `bws exited 1: invalid access token` | Token revoked or wrong | Run `kova secrets bitwarden token` with a new token |
