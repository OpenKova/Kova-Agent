...
Resolve provider API keys from [1Password](https://1password.com/) at process startup instead of storing them in plaintext inside `~/.kova/.env`. You keep your keys as 1Password items and reference them by `op://vault/item/field`; rotating a credential becomes a single change in 1Password.
...
2. You map environment-variable names to `op://` references in `~/.kova/config.yaml`.
3. Every time `kova` (or the gateway, or a cron job) starts, after `~/.kova/.env` has loaded, Kova runs `op read` for each reference and sets the resolved values into `os.environ`.
...
`op` supports two non-interactive-friendly modes; Kova works with either:

- **Service accounts** (recommended for servers/CI): create a service account in 1Password, grant it read access to the relevant vault, and export its token as `OP_SERVICE_ACCOUNT_TOKEN` in `~/.kova/.env`. The token is the credential — treat it like any other bearer token.
- **Desktop / interactive sessions** (laptops): run `op signin` (or enable CLI integration in the 1Password app). Kova passes your `OP_SESSION_*` variables through to the `op` child process. The 1Password cache key includes those session variables, so signing into a different account never serves a value cached under the previous identity.
...
1. **In `~/.kova/.env` (recommended).** `kova secrets onepassword setup --token <token>` writes the token to `~/.kova/.env`, exactly like Bitwarden's `BWS_ACCESS_TOKEN`. Because `load_kova_dotenv()` always loads `.env`, the token is available everywhere with zero extra setup. This is the simplest reliable option.

2. **In `~/.kova/.op.env` (gitignored).** If you'd rather keep the service-account token out of `.env` — for example so `.env` can be checked into a private dotfiles repo while the token stays out of version control — place it in `~/.kova/.op.env`:

   ```bash
   echo 'OP_SERVICE_ACCOUNT_TOKEN=ops_...' > ~/.kova/.op.env
   chmod 600 ~/.kova/.op.env
   ```

   Kova auto-loads `.op.env` at startup, **after** `.env`, and **never** overrides a token already present in the environment. `.op.env` is gitignored so the token never enters a committed file.
...
3. **Via systemd `EnvironmentFile` (Linux gateway).** If you run the gateway under systemd, you can inject the token directly into the service environment:
   ...
   ```ini
   [Service]
   EnvironmentFile=-/home/youruser/.kova/.op.env
   ```

   A token injected this way takes precedence — Kova detects that `OP_SERVICE_ACCOUNT_TOKEN` is already set and skips loading `.op.env` entirely.
...
```bash
kova secrets onepassword setup
```
...
```bash
kova secrets onepassword set OPENAI_API_KEY    "op://Private/OpenAI/api key"
kova secrets onepassword set ANTHROPIC_API_KEY "op://Private/Anthropic/credential"
```

### 4. Preview and confirm

```bash
kova secrets onepassword sync     # dry-run: resolve now, show what would apply
kova secrets onepassword status   # config + binary + references + auth
```

From now on, every `kova` invocation resolves the references at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `kova secrets onepassword setup` | Verify `op`, set account / token env var, enable |
| `kova secrets onepassword status` | Show config, binary, auth, and configured references |
| `kova secrets onepassword token` | Rotate the service-account token: validate with `op whoami`, then store it in `.env` |
| `kova secrets onepassword set ENV_VAR "op://…"` | Map an env var to a reference (stored stripped + validated) |
| `kova secrets onepassword remove ENV_VAR` | Drop a mapping |
| `kova secrets onepassword sync` | Dry-run: resolve references now and show what would apply |
| `kova secrets onepassword sync --apply` | Resolve and export into the current shell's environment |
| `kova secrets onepassword disable` | Flip `enabled: false`; leaves mappings in place |
...
| `service_account_token_env` | `OP_SERVICE_ACCOUNT_TOKEN` | Env var Kova reads the service-account token from. Its value is exported to the `op` child as `OP_SERVICE_ACCOUNT_TOKEN` (the name `op` expects). Leave the var unset to use a desktop/interactive session. |
...
1Password never blocks Kova startup. If anything goes wrong you'll see a one-line warning in stderr and Kova continues:

| Symptom | Cause | Fix |
|---|---|---|
| `the op CLI was not found on PATH` | `op` not installed / not on PATH | Install the CLI, or set `secrets.onepassword.binary_path` |
| `op read failed for 'op://…': …` | Locked session, expired token, or no vault access | `op signin`, run `kova secrets onepassword token` to rotate the service-account token, or grant the service account access |
...
- A 1Password service-account token can read every secret the account has access to. Store it in `~/.kova/.env` (not `config.yaml`), and revoke + regenerate from 1Password if it leaks.
- Kova refuses to let a resolved value overwrite the token env var itself, even with `override_existing: true`.
...
- **Single-machine personal setups** where `~/.kova/.env` is fine.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or anywhere you want centralized rotation and revocation across multiple Kova installations.
