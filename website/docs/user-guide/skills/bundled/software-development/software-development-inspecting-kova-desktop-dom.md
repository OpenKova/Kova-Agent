---
title: "Inspecting Kova Desktop Dom — Read the live Kova desktop DOM/CSS over CDP"
sidebar_label: "Inspecting Kova Desktop Dom"
description: "Read the live Kova desktop DOM/CSS over CDP"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Inspecting Kova Desktop Dom

Read the live Kova desktop DOM/CSS over CDP.
...
| Path | `skills/software-development/inspecting-kova-desktop-dom` |
| Version | `1.0.0` |
| Author | Kova Agent |
| License | MIT |
...
# Inspecting the live Kova desktop DOM
...
`debugging-kova-desktop`), or anything where the real question is "does this
look right".
...
- **no `KOVA_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`KOVA_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${KOVA_DESKTOP_CDP_PORT:-9222}/json/version
```
...
KOVA_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
KOVA_DESKTOP_CDP_PORT=9333 \
...
- **A throwaway `HERMES_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `kova:api` and may exit on its own. The renderer still mounts and the DOM is
