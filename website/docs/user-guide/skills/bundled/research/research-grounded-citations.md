...
Override per task with `--ledger <path>` or `KOVA_CITATION_LEDGER`.

## How to Run

```bash
S=~/.kova/skills/research/grounded-citations/scripts/sources.py
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `KOVA_CITATION_LEDGER`) if their
