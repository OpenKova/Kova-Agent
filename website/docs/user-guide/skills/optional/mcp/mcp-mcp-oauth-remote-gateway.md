...
| | |
|---|---|
| Source | Optional — install with `kova skills install official/mcp/mcp-oauth-remote-gateway` |
...
Kova' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Kova process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Kova runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Kova.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Kova' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Kova is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Kova** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Kova process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the kova user, an
empty or missing tokens directory (`$KOVA_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Kova detects a remote session it prints two
...
Both require an interactive terminal to the Kova host. The rest of this skill
is for when there is NO interactive TTY — Kova running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Kova Dashboard (try this BEFORE manual token surgery)

A remote Kova gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `kova dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'kova dashboard'`). It exposes a connector/MCP console —
...
across restarts) and anything reading credentials from `$KOVA_HOME/.env`. Those
...
uses these same env vars, so if Kova' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find KOVA_HOME and the config path

```bash
KOVA_HOME=$(python3 -c 'from kova_constants import get_kova_home; print(get_kova_home())')
echo "config: $KOVA_HOME/config.yaml"
echo "tokens: $KOVA_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Kova Agent (manual OAuth)",
...
`$KOVA_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
UA** — Cloudflare will 403 you even though Kova (which uses httpx) will succeed.
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Kova process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Kova needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Kova does natively).
...
- **Don't claim Kova doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Kova. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Kova' config.
