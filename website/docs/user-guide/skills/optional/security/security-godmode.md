...
5. **Kova models don't need jailbreaking** — nousresearch/hermes-3-* and hermes-4-* are already uncensored. Use them directly for the fastest path.
...
12. **execute_code sandbox has no env vars** — When Kova runs auto_jailbreak via execute_code, the sandbox doesn't inherit the Kova `.env`. Load dotenv explicitly: `import os; from dotenv import load_dotenv; load_dotenv(os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.hermes")), ".env"))`
