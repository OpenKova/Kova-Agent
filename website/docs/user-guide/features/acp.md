...
Kova Agent can run as an ACP server, letting ACP-compatible editors talk to Hermes over stdio and render:
...
- Configure Hermes provider credentials first with `kova model`, or set them in `~/.hermes/.env` / `~/.hermes/config.yaml`.
