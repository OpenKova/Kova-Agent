...
The legacy `hermes honcho setup` command still works (it now redirects to `kova memory setup`), but is only registered after Honcho is selected as the active memory provider.
