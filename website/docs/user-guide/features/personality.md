...
If the file is empty, whitespace-only, or cannot be read, Kova falls back to a built-in default identity ("You are Kova Agent, an intelligent AI assistant created by Neural Studio..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
