...
No admin rights required. The installer goes to `%LOCALAPPDATA%\kova\` and adds `hermes` to your **User PATH** — open a new terminal after it finishes.
...
9. **Adds `%LOCALAPPDATA%\kova\kova-agent\venv\Scripts` to User PATH and sets `HERMES_HOME=%LOCALAPPDATA%\kova`** — exposes the `hermes` command (and points it at your data dir) after you open a new terminal.
...
| `%LOCALAPPDATA%\kova\bin\` | Hermes's managed `uv.exe` (the Python manager it uses for updates). |
...
Hermes honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%LOCALAPPDATA%\kova\.env` (your `HERMES_HOME`) is the normal path — same as Linux:
