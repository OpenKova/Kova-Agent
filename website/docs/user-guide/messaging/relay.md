...
sidebar_position: 30
title: "Kova Relay"
description: "Connect Kova to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Kova Relay (Connector)
...
Kova Relay is not a chat platform itself — it is a **connector system** that
...
secret. `kova gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.kova/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.kova/.env`:
...
`kova gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.kova/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `kova auth add nous` (or `kova setup`) and retry.
...
startup. Restart it (`kova gateway restart`).
