...
1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   kova slack manifest --agent-view --write
   ```
   ...
   ```bash
   kova slack manifest --agent-view \
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Kova now
...
[`COMMAND_REGISTRY`](https://github.com/OpenKova/Kova-Agent/blob/main/kova_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Kova
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Kova can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Kova only supplies that context as a label; it does not read the viewed
channel's history.
...
The bang form also works behind a mention (`@Kova !stop`) and with
...
fails, Kova retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Kova /cmd` — reply as normal visible
messages instead.)
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions Kova, and "all" accepts every
      # other bot. Kova always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions Kova, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default Kova sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Kova is thinking…"`) —
...
By default Kova ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Kova instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Kova always ignores its own messages, in every mode, to prevent self-echo loops.
...
manifest — regenerate with `kova slack manifest`):
...
Delivery works even when the cron process isn't co-located with the gateway — Kova falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
