...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`kova send` CLI](/guides/pipe-script-output):

```bash
kova send ntfy:alerts-channel "Done!"
```
