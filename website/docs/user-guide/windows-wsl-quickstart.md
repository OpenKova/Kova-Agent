...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Hermes workflow (`kova chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.
