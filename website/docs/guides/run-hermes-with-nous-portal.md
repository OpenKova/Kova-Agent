...
These rows appear in `hermes tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Kova Agent runs the Portal login inline (without changing your inference provider or your other tools).
...
- Multiple Kova profiles where you're using the wrong one (check `hermes profile list`)
