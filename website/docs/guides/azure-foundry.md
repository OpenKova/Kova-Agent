...
With `AZURE_ANTHROPIC_KEY` set in `~/.hermes/.env`. Kova detects `azure.com` in the base URL and short-circuits around the Claude Code OAuth token chain so the Azure key is used directly with `x-api-key` auth.
