...
:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`kova import-agent`](../user-guide/import-from-other-agents.md).
:::
...
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `kova update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
