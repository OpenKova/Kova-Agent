...
4. From then on, Hermes refreshes the access token in the background — you stay signed in until you `kova auth logout xai-oauth` or revoke access from your xAI account settings.
...
This is **not** a stale-token problem — re-running `kova model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/hermes-agent/issues/26847)).
