...
- The routing is transparent: `kova chat`, `hermes sessions list`, `kova version`, etc. all exec into the container under the hood
...
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it hermes-agent kova config set ...` is also blocked)
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `hermes-agent` binaries exist and `kova version` runs |
...
```bash
# Native mode
sudo -u kova cat /var/lib/hermes/.hermes/.env
...
| `kova version` shows old version | Container not restarted | `systemctl restart hermes-agent` |
