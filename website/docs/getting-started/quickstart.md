...
|---|---|---|
| I just want Hermes working on my machine | `kova setup` | Run a real chat and verify it responds |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `kova model` → MiniMax (OAuth) |
...
|---|---|---|
| Hermes opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `kova model` again and confirm provider, model, and auth |
