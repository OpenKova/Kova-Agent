...
``kova_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import kova_state (cycle) — shared
module-level constants live in kova_state_common.
"""
...
# Moved methods logged under the "kova_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("kova_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``kova_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `kova update` until the user's
