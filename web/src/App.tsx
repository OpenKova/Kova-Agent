...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'kova update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "kova update";
    return (
      t.status.updateKovaConfirmMessage ??
      `This will run 'kova update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateKovaConfirmMessage, updateConfirmInfo]);
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Kova gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateKovaConfirmNow ?? "Update now"}
      description={
