...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Kova derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            kova-agent = prev.kova-agent.overrideAttrs (_old: {
              KOVA_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $KOVA_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$KOVA_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        kova-agent = prev.kova-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "kova-agent-env" {
    kova-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "kova-agent-editable-env" {
    kova-agent = dependency-groups;
  };
}
