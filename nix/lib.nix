      ...
      let
        ...
        excludedDirs =
          ...
          ++ [
            ...
            "nix"
            # Skills are shipped via KOVA_BUNDLED_SKILLS /
            # KOVA_OPTIONAL_SKILLS (see kova-agent.nix), not via the
            ...
            # __init__.py) shipped via symlinks + KOVA_BUNDLED_LOCALES
            # / KOVA_OPTIONAL_MCPS, not via the wheel. Excluding them
...
in
{
  ...
  #
  # Usage:
  #   npm = kovaNpmLib.mkNpmPassthru { dirs = [ "ui-tui" ]; };
  #   npm = kovaNpmLib.mkNpmPassthru { dirs = [ "apps/desktop" "apps/shared" ]; };
