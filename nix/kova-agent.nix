# nix/kova-agent.nix — Overridable Kova Agent package
#
...
let
  ...
  # lib.nix) — the wrapper exposes them via KOVA_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from KOVA_BUNDLED_PLUGINS.
  ...
  # at by KOVA_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  # exposed via KOVA_OPTIONAL_MCPS.
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in kova sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    ln -s ${bundledSkills} $out/share/kova-agent/skills
    ln -s ${bundledOptionalSkills} $out/share/kova-agent/optional-skills
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          ...
          --set KOVA_BUNDLED_PLUGINS $out/share/kova-agent/plugins \
          --set KOVA_BUNDLED_LOCALES $out/share/kova-agent/locales \
  ...
  passthru =
    ...
    in
    {
      ...
      devShellHook = ''
        export KOVA_PYTHON=${devPython}/bin/python3
      '';
