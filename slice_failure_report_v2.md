# CI Failure Report v2 — run 30699981302 (head e1b2db20)

Generated from `gh run view 30699981302 --log-failed`.

## 1. Overall job status

**Passed (was failing in run 30694412497 → now green):** `check-attribution` ✅, `ruff enforcement` ✅, `Python tests / Run tests slice 1/8` ✅, `e2e`, `ruff + ty diff`, `Windows footguns`, `shellcheck`, `hadolint`, `package-lock diff`, `Playwright E2E`, `uv lock`, `osv-scan`, `check-common-ancestor`, `docs-site`, `supply-chain`.

**Failed (14 jobs):**

| Job | Conclusion |
|---|---|
| Python tests / Run tests slice 2/8 | failure |
| Python tests / Run tests slice 3/8 | failure |
| Python tests / Run tests slice 4/8 | failure |
| Python tests / Run tests slice 5/8 | failure |
| Python tests / Run tests slice 6/8 | failure |
| Python tests / Run tests slice 7/8 | failure |
| Python tests / Run tests slice 8/8 | failure |
| JS & TS checks / apps/desktop / check:test:ui | failure |
| JS & TS checks / apps/desktop / check:test:desktop:all | failure |
| JS & TS checks / apps/desktop / check:test:desktop:platforms | failure |
| JS & TS checks / ui-tui / check | failure |
| Build&Test Docker image / build (amd64) | failure |
| Build&Test Docker image / build (arm64) | failure |
| Review label gate / Review label gate | failure |
| CI review comment (live) | failure (derived) |
| All required checks pass | failure (gate) |

**Python slices: 7/8 failing** (slice 1/8 now passes). **Non-Python:** 4 JS/TS, 2 Docker, 2 gates.

## 2. Python test slices

- **Python tests / Run tests slice 2/8**: 9 unique failing tests
- **Python tests / Run tests slice 3/8**: 57 unique failing tests
- **Python tests / Run tests slice 4/8**: 23 unique failing tests
- **Python tests / Run tests slice 5/8**: 29 unique failing tests
- **Python tests / Run tests slice 6/8**: 15 unique failing tests
- **Python tests / Run tests slice 7/8**: 18 unique failing tests
- **Python tests / Run tests slice 8/8**: 40 unique failing tests

**Total unique failing tests across slices: 191** (was 280 in run 30694412497)

## 3. Still-failing tests (deduped) + assertions


### Python tests / Run tests slice 3/8
- **Unique failing tests: 57**
...
**Deduped assertion/error lines (27):**
- `E                   TimeoutError`
...
- `E       assert 0 == 1`
- `E       assert 0 == 2`
- `E       assert 0.0003319799999985662 >= 0.04`
- `E       assert 1 == 0`
- `E       assert 11111 in []`
- `E       assert 12345 in []`
...
- `E       AssertionError: assert 'kova config:' in 'Failed to parse /tmp/pytest-of-runner/pytest-315/test_docker_config_migrate_doe0/config.yaml: while parsing a flow se...s saved to /tmp/pytest-of-runner/pytest-315/test_docker_config_migrate`
...
- `E       assert '/tmp/pytest-of-runner/pytest-186/test_format_message_mentions_p0/kova.exe' in "✗ Another kova.exe is running:\n    PID 1234  kova.exe\n    PID 5678  kova.exe\n\n  Updating now would fail to overwr.../F\n\n  Override with `ko`
- `E       assert 12345 in []`
- `E       assert False`
- `E       assert [] == [12345]`
...
- `E           AssertionError: assert 'kova config:' in '⚠️  hermes config: Failed to parse /tmp/pytest-of-runner/pytest-150/test_dedup_on_repeated_load_sa0/config.yaml: whil...s saved to /tmp/pytest-of-runner/pytest-150/test_dedup_on_repeated`
- `E           AssertionError: assert 'kova config:' in '⚠️  hermes config: Failed to parse /tmp/pytest-of-runner/pytest-150/test_logs_and_warns_on_parse_f0/config.yaml: whil...s saved to /tmp/pytest-of-runner/pytest-150/test_logs_and_warns_on`
- `E           AssertionError: edited file should re-warn`
- `E           assert 'kova config:' in '⚠️  hermes config: Failed to parse /tmp/pytest-of-runner/pytest-150/test_rewarns_after_file_edit0/config.yaml: while ...ping the previously loaded config for this process — edits to config.yaml are bein`
...
(node:112307) [DEP0040] DeprecationWarning: The `punycode` module is deprecated. Please use a userland alternative instead.
(Use `node --trace-deprecation ...` to show where the warning was created)
[command]/usr/bin/docker buildx rm builder-8cb8897b-790e-4c16-9c7d-3be15050c32d
builder-8cb8897b-790e-4c16-9c7d-3be15050c32d removed
State not set
...
State not set
Node 20 is being deprecated. This workflow is running with Node 24 by default. If you need to temporarily use Node 20, you can set the ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION=true environment variable. For more information see: https://github.blog/changelog/2025-09-19-deprecation-of-node-20-on-github-actions-runners/
(node:105606) [DEP0040] DeprecationWarning: The `punycode` module is deprecated. Please use a userland alternative instead.
(Use `node --trace-deprecation ...` to show where the warning was created)
[command]/usr/bin/docker buildx rm builder-a9ba8d02-9a3b-4148-b0bd-68d55adcf839
builder-a9ba8d02-9a3b-4148-b0bd-68d55adcf839 removed
State not set
##[warning]Node.js 20 is deprecated. The following actions target Node.js 20 but are being forced to run on Node.js 24: docker/setup-buildx-action@8d2750c68a42422c14e847fe6c8ac0403b4cbd6f. For more information see: https://github.blog/changelog/2025-09-19-deprecation-of-node-20-on-github-actions-runners/
```
...
```
  ...
  HEAD_SHA: e1b2db208d72dc493a8b9ef3ef7eb041984bc928
...
##[error]Process completed with exit code 1.
Terminate orphan process: pid (2849) (sleep)
##[warning]Node.js 20 is deprecated. The following actions target Node.js 20 but are being forced to run on Node.js 24: actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020. For more information see: https://github.blog/changelog/2025-09-19-deprecation-of-node-20-on-github-actions-runners/
```
...
```
  ...
  [627s] 44 completed, 0 pending (44 total jobs)
  Updated comment 5126615443
  All jobs done, but 14 failed: Build&Test Docker image / build (amd64, ubuntu-latest, linux/amd64, type=gha,scope=docker-amd64, type=gha,mode=max,scope=do..., Build&Test Docker image / build (arm64, ubuntu-24.04-arm, linux/arm64, type=gha,scope=docker-arm64, type=gha,mode=max,scope..., Python tests / Run tests slice 3/8, Python tests / Run tests slice 2/8, Python tests / Run tests slice 6/8, Python tests / Run tests slice 4/8, Python tests / Run tests slice 5/8, Python tests / Run tests slice 8/8, Python tests / Run tests slice 7/8, Review label gate / Review label gate, JS & TS checks / apps/desktop / check:test:ui, JS & TS checks / apps/desktop / check:test:desktop:all, JS & TS checks / ui-tui / check, JS & TS checks / apps/desktop / check:test:desktop:platforms
  Exiting with error so the run can be rerun via --failed.
##[error]Process completed with exit code 1.
```
## 5. Regression check — NEW failures vs original 280

**Result: ZERO new failures.** All 191 still-failing tests are a strict subset of the 280 from run 30694412497 (verified via `comm -13` on deduped test-name sets). **No regressions introduced by today's fixes** (lifecycle_guard, api_server, portal_tags, onboarding, skills_hub, honcho, acp, cli resume, banner, disk_cleanup, cmd_update mojibake, skin_engine twin alignment).

**What dropped (280 → 191, −89 tests fixed):**
- All of slice 1/8's 14 failures (portal_tags, onboarding, skills_hub, honcho client_id, acp, skin_engine, disk_cleanup) — slice 1/8 now fully green
- gateway_restart_loop (52), api_server (6), cmd_update migration (2), update_check (1), resume_hint (5), cmd_update_docker (3), tips, etc.

**Remaining 191 categories (unchanged from prior audits, not regressions):**
- `test_update_stale_dashboard` (hermes_cli FindStaleDashboardPids + Posix kill paths) — ~10, source/twin drift class
- `test_gui_command` / `test_gui_uninstall` (both twins) — desktop GUI tests, npm/electron env class
- `test_kanban_core_functionality` / `test_kanban_diagnostics` — kanban dispatcher class
- `test_subscription_cli` (14), `test_auth_xai_oauth_provider` (7), `test_gateway_restart_loop` (residual?) — need per-file triage
- `test_live_system_guard_self_test` (8), `test_packaging_build_guard` (4), `test_provider_attribution_headers` (5), `test_backup` (8+), `test_profiles` clone, `test_placeholder_usage`, `test_mcp_startup`, `test_web_server_cron_profiles`, docker/* (exec privilege, gateway supervised, etc.)
