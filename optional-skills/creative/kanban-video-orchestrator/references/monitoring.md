...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `hermes profile list` (profile exists?), `kova status` (gateway/dispatcher up?), and verify tenant. |
