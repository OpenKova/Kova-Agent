...
### 1. Start order: editor first, Kova second

Kova probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Kova session.
...
restart the editor, restart the Kova session.
...
server fails to bind (Output Log) or Kova' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.kova/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Kova

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Kova' connection
lives in `~/.kova/config.yaml` via `kova mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Kova — just don't
mistake it for the Kova setup step.
...
explicitly warns against overlapping calls. Kova executes same-turn tool
...
### 9. Timeouts: Kova gives up before Unreal does

Kova' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Kova runs elsewhere (SSH backend,
