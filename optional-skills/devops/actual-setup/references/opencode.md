...
is typically only in `~/.kova/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Kova,
...
`kova-custom-providers` skill, pitfall 2). Kova hits this because it forwards
