...
The server-building part may be correct while the Kova config is not. Load the `native-mcp` skill and configure the server in `~/.hermes/config.yaml`, then restart Kova.
