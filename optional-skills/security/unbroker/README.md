...
The design is **Kova-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
kova skills install official/security/unbroker
```

Then start a new Kova session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Kova home directory (`0600`, optionally
