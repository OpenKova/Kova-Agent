...
def get_kova_home() -> Path:
    ...
    ``kova_cli/kanban_db.py``).  See https://github.com/OpenKova/Kova-Agent/issues/18594.
    """
...
def get_process_kova_home() -> Path:
...
def get_default_kova_root() -> Path:
    ...
    ``~/.kova`` (e.g. ``/opt/data``), returns ``HERMES_HOME`` directly
    — that IS the root.
    ...
    try:
        ...
        # HERMES_HOME is under ~/.kova (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def get_kova_dir(
    ...
    *,
    ...
    Args:
        ...
        home: Optional explicit Kova home. Profile-aware callers that manage
...
def _bootstrap_managed_node_posix() -> bool:
    ...
    ``KOVA_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_kova_managed_node() -> str | None:
    """Install a Kova-managed Node tree and return its npm path.
    ...
    Kova never modifies a toolchain it does not own, so instead it provisions
    ...
    present.
    """
    existing = find_kova_node_executable("npm")
    if existing:
        return existing
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_kova_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_KOVA_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_kova_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _KOVA_NODE_TARGET_MAJOR
    return False
...
def find_kova_node_executable(command: str) -> str | None:
    ...
    Outdated trees (node major below ``_KOVA_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_kova_managed_node():
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/OpenKova/Kova-Agent/issues/25821.
    """
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Kova' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Kova itself runs inside WSL.
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``kova_cli`` modules using three different Windows
    ...
    A few sites outside ``kova_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``kova_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Kova itself. An ImportError
...
# `kova_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Kova.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("kova_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `kova update` is exactly
    ...
    return [
        ...
        "    kova update",
        "If that also fails, reinstall: https://kova-agent.kova.ai",
    ]
