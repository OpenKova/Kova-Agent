...
from tools.approval import (
    reset_kova_interactive_context,
    set_kova_interactive_context,
)
...
def _named_custom_provider_catalogs() -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    try:
        ...
        from kova_cli.providers import custom_provider_slug
    except ImportError:
        return []
...
# already uses (`kova_cli/moa_cmd.py`). This bounds each provider's row, not
...
class HermesACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Kova inventory is also used by ``kova model``, the TUI,
        ...
        try:
            ...
            from kova_cli.models import normalize_provider, provider_label
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.kova`` (ACP's extensibility
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # tools.approval (set_kova_interactive_context) rather than
        # os.environ["KOVA_INTERACTIVE"], so concurrent executor workers can't
        ...
        def _run_agent() -> dict:
            ...
            try:
                ...
                # Without it the prompt advertises the global Kova workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.kova/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global KOVA_CRON_SESSION (#37968).
            ...
            interactive_token = set_kova_interactive_context(True)
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_kova_interactive_context(interactive_token)
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Kova install tree into the session's
