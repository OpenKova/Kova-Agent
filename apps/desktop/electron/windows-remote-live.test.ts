...
function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Kova-Session-Token': token } }).then(async response => {
    if (!response.ok) {
  ...
  async () => {
    ...
    try {
      ...
      if (lock) {
        ...
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m kova_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
