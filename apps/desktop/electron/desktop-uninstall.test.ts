...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Kova.app/Contents/MacOS/Kova', 'darwin'),
    '/Applications/Kova.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Kova.app/Contents/MacOS/Kova', 'darwin'),
    '/Users/x/Applications/Kova.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Kova\\Kova.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Kova'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\kova-desktop\\Kova.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\kova-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Kova.exe', 'win32'), null)
})
...
test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/kova/linux-unpacked/kova', 'linux', {}), '/opt/kova/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/kova', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Kova.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Kova.app'), false)
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  ...
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.kova\/kova-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'kova_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  ...
  assert.match(script, /'-m' 'kova_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  ...
  // PYTHONPATH set so a system python can import kova_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\kova;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "kova_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Kova" >nul 2>&1/)
