// Profile-delete routing logic for the `kova:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_kova_home() and
...
/**
 * Parse a `kova:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `kova:api` request away from the primary/window backend
 ...
 * ensure_kova_home() recreates the directory the delete just removed.
 */
