...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'kova-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'kova-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Kova Test'], { cwd: dir })
