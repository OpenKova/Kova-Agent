...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/kova', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/kova/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/kova'), 'https://gw.example.com/kova/auth/native/refresh')
})
