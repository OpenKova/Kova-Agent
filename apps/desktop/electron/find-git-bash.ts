 ...
 *   1. KOVA_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\kova\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect KOVA_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.KOVA_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'kova', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'kova', 'git', 'usr', 'bin', 'bash.exe'))
  }
