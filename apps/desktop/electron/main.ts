...
const USER_DATA_OVERRIDE = process.env.KOVA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.KOVA_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.KOVA_DESKTOP_IS_PACKAGED)
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[kova] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. KOVA_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[kova] ${why}`)
  }
}
...
// never go through `kova desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Kova sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[kova] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[kova] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[kova] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[kova] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[kova] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}
...
// always-on timers. A completely idle, minimized Kova burned ~20% CPU
...
function loadInstallStamp() {
  ...
  for (const p of candidates) {
    try {
      ...
      console.warn(`[kova] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
function resolveHermesHome() {
  ...
  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'kova-home')
  }
...
const ACTIVE_KOVA_ROOT = path.join(HERMES_HOME, 'kova-agent')
...
const BOOT_FAKE_MODE = process.env.KOVA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.KOVA_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.KOVA_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.KOVA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.KOVA_DESKTOP_APP_NAME || 'Kova'
...
// electron-builder derives from the build `appId` (com.kovaai.kova) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.kovaai.kova')
}
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Neural Studio'
})
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.KOVA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.KOVA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// respawning kova serve backend children in a tight loop while boot is broken.
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_KOVA_ROOT
  })
}
...
function findOnPath(command) {
  ...
  // shell-script shim named `kova` — must not shadow `kova.cmd`/`kova.exe`.
...
// spawns `kova serve`; runtimes older than serve only have `dashboard`. We
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'kova_cli', 'subcommands', 'dashboard.py'), 'utf8')
...
function findPythonForRoot(root) {
  const override = process.env.KOVA_DESKTOP_PYTHON
...
// True when an install path lives inside the TrustedInstaller-protected
// `C:\Program Files\WindowsApps` directory. Executables there exist and
// `fs.statSync` reports them, but CreateProcess denies a regular user
// (`spawn EPERM`). Only Microsoft Store packages install here, so a registry
// `InstallPath` under it is never a spawnable Python for us.
function isProtectedWindowsAppsPath(installPath) {
  if (!IS_WINDOWS || !installPath) {
    return false
  }

  const programFiles = process.env.ProgramFiles || 'C:\\Program Files'
  const windowsApps = path.resolve(programFiles, 'WindowsApps')

  const normalized = path.resolve(String(installPath)).toLowerCase()
  const root = windowsApps.toLowerCase()
    ...
    for (const version of SUPPORTED_VERSIONS) {
      try {
        ...
        if (match) {
          ...
          // The Microsoft Store Python registers its InstallPath as the
          // packaged copy under `C:\Program Files\WindowsApps`, a
          // TrustedInstaller-protected directory. The file exists and stats
          // fine, but CreateProcess refuses to exec it for a regular user
          // (`spawn EPERM`), so returning it hands the spawn step a backend
          // that can never launch. Skip the hit — pass 3 (`py -3.11`)
          // resolves the spawnable app-execution-alias copy under
          // %LOCALAPPDATA%\Microsoft\WindowsApps instead.
          if (isProtectedWindowsAppsPath(installPath)) {
            continue
          }
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the kova-import
...
// findGitBash — locate bash.exe on Windows. Resolves KOVA_GIT_BASH_PATH
...
// normal KOVA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
function resolveUpdateRoot() {
  const candidates = [
    process.env.KOVA_DESKTOP_KOVA_ROOT && path.resolve(process.env.KOVA_DESKTOP_KOVA_ROOT),
    !IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isHermesSourceRoot(ACTIVE_KOVA_ROOT) ? ACTIVE_KOVA_ROOT : null
  ].filter(Boolean)
...
// Windows that binary owns ALL repo mutation — running `kova update` +
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged kova-setup or not — the resolver
      ...
      // `kova update` (backend) + `kova desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      // kova-setup.exe into HERMES_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `kova` on PATH / in the venv, so the
        ...
        // `kova update` defaults to main and would silently switch a
    ...
    if (!lock.unlocked) {
      ...
      // terminal running kova, an unkillable child). Handing off anyway
      ...
      const message =
        ...
        '(a second Kova window or a terminal running kova?). Close it and retry.'
    ...
    // Kova processes running from this venv.  The updater normally refuses
    ...
    let child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: HERMES_HOME,
      env: {
        ...process.env,
        HERMES_HOME,
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'kova.exe' : 'kova')
  ...
  // not just the `kova.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvHermes) || fileExists(path.join(updateRoot, '.kova-bootstrap-complete'))
...
function resolveHermesCliBinary(updateRoot) {
  const venvHermes = path.join(updateRoot, 'venv', 'bin', 'kova')
...
// Python-level pre-update snapshot inside `kova update`.
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const kova = resolveHermesCliBinary(updateRoot)
  ...
  // PYTHONUNBUFFERED: `kova update` writes to a pipe here, so CPython
  ...
  // `kova update` reaps stale `kova serve` backends (a code update
...
// Marker-independent: is the canonical install at ACTIVE_KOVA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isHermesSourceRoot(ACTIVE_KOVA_ROOT) &&
...
function resolveWebDist() {
  const override = process.env.KOVA_DESKTOP_WEB_DIST
...
function resolveHermesBackend(backendArgs) {
  ...
  const overrideRoot = process.env.KOVA_DESKTOP_KOVA_ROOT && path.resolve(process.env.KOVA_DESKTOP_KOVA_ROOT)
  ...
  // 3. ACTIVE_KOVA_ROOT — the canonical install at
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Kova runtime at ${ACTIVE_KOVA_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  if (process.env.KOVA_DESKTOP_IGNORE_EXISTING !== '1') {
    let hermesCommand = null
    const hermesOverride = process.env.KOVA_DESKTOP_HERMES
    ...
    if (hermesCommand) {
      ...
      // KOVA_DESKTOP_HERMES is an explicit deployment override (used by
...
async function ensureRuntime(backend) {
  ...
  if (!isHermesSourceRoot(ACTIVE_KOVA_ROOT)) {
    throw new Error(
  ...
  if (!fileExists(venvPython)) {
    ...
    // plus an importable kova_cli before it hands back the active runtime.
...
function multipartBody(upload) {
  const boundary = `----kova-${crypto.randomBytes(12).toString('hex')}`
...
function sendOpenFolderRequested() {
  ...
  webContents.send('kova:open-folder-requested')
}
...
// 'kova:power-battery' push below.
ipcMain.handle('kova:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('kova:power-battery', next)
    }
  }
}
...
// Neural Studio) instead of a static session token. The auth model is
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Kova
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Kova Cloud agent…' : 'Sign in to Kova gateway',
...
// feature; the server half lives in kova_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Kova Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:kova-remote-oauth`:
...
// authenticates via PRIVY, not the Kova gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for kova_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Kova Cloud sign-in.'))
...
// Discover the hosted (Kova Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Kova Cloud. Open Settings → Gateway, choose Kova Cloud, and sign in.'
    ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Kova Cloud session has expired. Sign in to Kova Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Kova Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.KOVA_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.KOVA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Kova Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Kova Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Kova Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (kova #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Kova gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
...
function activeSshTerminalTarget() {
  ...
  if (process.env.KOVA_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.KOVA_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.KOVA_DESKTOP_REMOTE_TOKEN
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Neural Studio" instead of a generic
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Kova on the remote host before connecting with Desktop SSH.'
            }
          }
...
function sendConnectionApplied() {
  ...
  webContents.send('kova:connection:applied')
}
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Kova backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_kova_home() recreates the deleted profile directory.
//
  ...
  const connectionPromise = (async () => {
    ...
    hermesProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        ...
        rejectBackendStart?.(new Error('Kova backend start was superseded by a newer connection attempt.'))
    ...
    hermesProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Kova backend exit (${signal || code})`)
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Kova backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('kova:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('kova:quick-entry:shown')
}
...
ipcMain.handle('kova:gateway:ws-url', async (_event, profile) => {
...
ipcMain.handle('kova:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('kova:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('kova:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('kova:zoom:get', event => {
...
ipcMain.on('kova:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('kova:bootstrap:continue-local', async () => {
...
ipcMain.handle('kova:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('kova:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('kova:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('kova:api', async (_event, request) => {
  ...
  // backend calls ensure_kova_home() which recreates the profile directory,
...
ipcMain.handle('kova:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))
...
ipcMain.handle('kova:data-url-read-max:get', () => ({
...
ipcMain.handle('kova:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('kova:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('kova:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('kova:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('kova:readClipboard', () => clipboard.readText())
...
ipcMain.handle('kova:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))
...
ipcMain.on('kova:active-work', (event, payload) => {
...
ipcMain.on('kova:keep-awake', (_event, on) => {
...
ipcMain.handle('kova:quick-entry:settings:get', async () => {
...
ipcMain.handle('kova:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('kova:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('kova:quick-entry:submit', {
...
ipcMain.on('kova:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('kova:quick-entry:state', payload)
  }
})

ipcMain.on('kova:quick-entry:dismiss', () => hideQuickEntryWindow())
...
// per-session secondary windows spawned via `kova:window:openSession`).
...
ipcMain.handle('kova:find-in-page', (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('kova:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
function terminalShellCommand() {
  ...
  const override = (process.env.KOVA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
ipcMain.handle('kova:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped kova_home the backend
...
ipcMain.handle('kova:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
