...
// ACTIVE_HERMES_ROOT — the canonical mutable Kova install. Same path
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Kova'
...
// `hermes update`). Instead we spawn the staged Kova-Setup binary with
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  // Put the Kova-managed Node and the venv on PATH so `hermes desktop`'s
