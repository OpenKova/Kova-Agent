// Unit tests for the pure Windows `kova` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvKovaCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `kova` shim shadows
//      the real kova.cmd/kova.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the kova.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvKovaCommand() — must probe the venv python via
//      canImportLegacyCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvKovaCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportLegacyCli: () => true,
    canImportKovaCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    kovaHome: '/fake/kova-home',
...
test('resolveVenvKovaCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvKovaCommand('/root/venv/Scripts/kova.exe', [], deps), null)
})

test('resolveVenvKovaCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvKovaCommand('/root/venv/Scripts/kova.cmd', [], deps), null)
})

test('resolveVenvKovaCommand: returns null when the basename is not kova/kova.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvKovaCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvKovaCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvKovaCommand('/root/venv/bin/kova.exe', [], deps), null)
})

test('resolveVenvKovaCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvKovaCommand('/root/venv/Scripts/kova.exe', [], deps), null)
})

test('resolveVenvKovaCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportLegacyCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvKovaCommand('/root/venv/Scripts/kova.exe', ['serve'], deps)
...
test('resolveVenvKovaCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvKovaCommand('/root/venv/Scripts/kova.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'kova_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvKovaCommand: is case-insensitive on kova.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvKovaCommand('/root/venv/Scripts/KOVA.EXE', [], deps))
  assert.ok(resolveVenvKovaCommand('/root/venv/SCRIPTS/kova.exe', [], deps))
})
