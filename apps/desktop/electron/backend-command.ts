// Backend subcommand routing for the desktop-managed Kova process.
//
// The desktop app launches its own headless backend via `kova serve` — it
...
// app hasn't updated yet, or an older `kova` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Kova profile to pin via `--profile`.
 */
 ...
 * `-m kova_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `kova_cli/subcommands/dashboard.py` source registers
