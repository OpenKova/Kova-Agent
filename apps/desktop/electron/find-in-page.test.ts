...
describe('installFoundInPageForwarder', () => {
  ...
  test('handles missing fields without throwing', () => {
    ...
    assert.deepEqual(wc.calls.send, [{ channel: 'kova:found-in-page', payload: { activeMatchOrdinal: 0, count: 0 } }])
  })
  ...
  test('two forwarders installed on distinct webContents do not cross-fire', () => {
    ...
    assert.deepEqual(wcA.calls.send, [{ channel: 'kova:found-in-page', payload: { activeMatchOrdinal: 1, count: 1 } }])
