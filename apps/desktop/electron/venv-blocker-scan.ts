...
const SCAN_MODULE = 'kova_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Kova process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Kova installation is free.\n' +
    '\n' +
    'Close other Kova windows and terminals, then retry.  If the problem\n' +
    'persists, run `kova update` in a terminal for detailed diagnostics.'
  )
}
