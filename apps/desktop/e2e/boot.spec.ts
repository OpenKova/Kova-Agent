 ...
 * packaged binary) with a real `kova serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → kova serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Kova title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Kova')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → kova serve is spawned → WS connects → config
