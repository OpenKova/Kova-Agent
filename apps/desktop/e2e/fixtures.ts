...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(kovaHome, 'config.yaml')
 ...
 */
export function writeEnvFile(kovaHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(kovaHome, '.env')
 ...
 */
function writeEmptyConfig(kovaHome: string): void {
  const configPath = path.join(kovaHome, 'config.yaml')
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.kovaHome)
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.kovaHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  delete (env as Record<string, string | undefined>).KOVA_DESKTOP_BIN
