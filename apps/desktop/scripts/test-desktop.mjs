...
// no longer bundles the Kova Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Kova.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Kova'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Kova.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Kova'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function ensurePackagedApp() {
  if (process.env.KOVA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Kova-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Kova-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Kova-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Kova-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.KOVA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.KOVA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
//   - The Kova Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of kova_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'kova-agent', 'kova_cli', 'main.py')
...
Fast rerun (skip rebuild if the packaged app already exists):
  KOVA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
