 ...
 * product name (`Kova`). If a PREVIOUS `npm run pack` was interrupted
 ...
 * Kova.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `kova desktop --build-only` (kova_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Kova.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Kova'}.exe`
