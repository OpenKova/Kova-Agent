...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `kova-dev-mock-${Date.now()}`))
  const kovaHome = path.join(root, 'kova-home')
  const userDataDir = path.join(root, 'electron-user-data')
  fs.mkdirSync(kovaHome, { recursive: true })
  fs.mkdirSync(userDataDir, { recursive: true })
  return { root, kovaHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(kovaHome, mockUrl) {
  fs.writeFileSync(
    path.join(kovaHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(kovaHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  const env = {
    ...
    KOVA_DESKTOP_APP_NAME: `KovaDevMock-${Date.now()}`,
  }
