#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Kova icon + version metadata onto the
// built Kova.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Kova.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Kova.exe>
//
