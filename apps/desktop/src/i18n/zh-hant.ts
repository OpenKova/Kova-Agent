...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: '正在啟動 Kova Desktop…'
    },
    errors: {
  ...
  settings: {
    ...
    notifications: {
      ...
      kinds: {
        ...
        plugin: {
          label: '外掛通知',
          description: 'Kova 在背景時，桌面外掛傳送了通知。'
        }
      },
      test: '傳送測試通知',
      testTitle: 'Kova',
    ...
    appearance: {
      ...
      reactionsTitle: '訊息回應',
      reactionsDesc: 'iMessage 風格的表情回應 — 你可以對訊息做出回應，Kova 也能回應你的訊息。',
      embedsTitle: '內嵌預覽',
      embedsDesc:
    ...
    quickEntry: {
      enabledTitle: '快速輸入',
      enabledDesc: '用全域快速鍵在任何地方喚出一個小輸入框，無需開啟 Kova 即可送出提示。',
    ...
    gateway: {
      ...
      sshTitle: '透過 SSH 連線',
      sshDesc:
        'Kova 會透過 SSH 在遠端啟動並以通道連線到本應用程式——無需自行啟動或公開任何服務。前提：已具備到該主機的金鑰 SSH 存取。',
      ...
      sshHermesPathTitle: 'Kova 路徑（選用）',
      sshHermesPathDesc: '遠端 kova 執行檔的完整路徑。留空 = 自動偵測。',
      ...
      sshReachable: (host, platform) => `可連線：${host}（${platform}）——已找到 Kova`,
      sshIncompleteHost: '連線前請輸入 SSH 主機。',
      sshErrUnreachable: '無法透過 SSH 連線到該主機。請檢查主機、連接埠和網路。',
      sshErrAuth:
        'SSH 驗證失敗。請將金鑰載入 ssh-agent（ssh-add），或在 ~/.ssh/config 中設定 IdentityFile——Kova 以非互動方式執行 ssh。',
      sshErrHostKey: '自上次連線以來主機金鑰已變更。請確認這是預期的，然後執行 ssh-keygen -R <host> 並重新連線。',
      sshErrNotInstalled:
        '遠端主機上未安裝 Kova。請在遠端安裝（curl -fsSL https://kova-agent.nousresearch.com/install.sh | sh）或設定 Kova 路徑。',
      sshErrPlatform: '不支援的遠端平台。Kova Desktop 的 SSH 模式支援 Linux、macOS 和 Windows 遠端主機。',
      sshErrTimeout: 'SSH 連線逾時。主機可能無法存取或處於睡眠狀態。',
      sshErrUpdateRequired: '使用 Desktop SSH 連線前，請更新遠端主機上的 Kova。',
    ...
    providers: {
      ...
      localEndpoint: {
        title: '本地 / 自訂端點',
        description: '將 Kova 指向任意 OpenAI 相容端點（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在載入提供方...'
    },
    sessions: {
  ...
  skills: {
    ...
    skillArchivedMessage: '可透過 kova curator restore 還原。'
  },
  ...
  starmap: {
    ...
    loading: '載入中…',
    emptyTitle: '尚無學習內容',
    emptyDesc: '當 Kova 為你的工作建立技能與記憶時，會顯示在這裡。'
  },
  agents: {
    close: '關閉代理',
    title: '派生樹',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '無法建立專案',
      staleBackend: '請更新 Kova 後端以建立專案——目前後端比桌面應用舊（設定 → 更新 → 後端）。',
  ...
  install: {
    ...
    setupChoiceTitle: '設定 Kova Desktop',
    setupChoiceDesc: '將此應用程式連線到您已執行的 Kova 閘道，或在這台電腦上本機安裝 Kova。',
    connectExistingTitle: '連線到現有 Kova',
    connectExistingShort: '連線現有環境',
    connectExistingDesc: '使用工作階段權杖或瀏覽器登入連線遠端後端。不會啟動本機安裝。',
    installLocalTitle: '本機安裝 Kova',
    installLocalDesc: '下載 Kova、建立 Python 環境，並在這台電腦上執行後端。',
    localStartUnavailable: '無法啟動本機安裝。請重新啟動 Kova Desktop 後再試一次。',
    remoteSetupTitle: '連線到現有 Kova',
    remoteSetupDesc: '輸入閘道 URL。Kova Desktop 會偵測需要權杖還是瀏覽器登入。',
    remoteUrlTitle: '閘道 URL',
    remoteUrlDesc: '使用 Kova 閘道的基礎 URL；遠端位址請包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/kova',
    probing: '正在偵測閘道驗證方式...',
    probeError: '無法連線到該 Kova 閘道。',
