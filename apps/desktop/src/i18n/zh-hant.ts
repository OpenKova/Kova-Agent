...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      sshKovaPathTitle: 'Kova 路徑（選用）',
      sshKovaPathDesc: '遠端 kova 執行檔的完整路徑。留空 = 自動偵測。',
      sshKovaPathPlaceholder: '自動偵測',
      ...
      sshErrNotInstalled:
        '遠端主機上未安裝 Kova。請在遠端安裝（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）或設定 Kova 路徑。',
    ...
    toolsets: {
      ...
      needsSignIn: '需要登入',
      needsSetup: '需要安裝',
      nousIncluded: '包含在 Kova 订阅中；登入 Kova Portal 即可啟用。',
      nousAuthNeededTitle: '登入 Kova Portal',
      nousAuthNeededMessage: provider => `已儲存 ${provider}，但在登入 Kova Portal 之前不會啟用。`,
      nousAuthSignIn: '登入',
      nousAuthDoneTitle: '已連接 Kova Portal',
      nousAuthDoneMessage: '訂閱後端現已啟用。',
      nousAuthFailed: 'Kova Portal 登入未完成',
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro 模型需要付費 Kova 订阅。',
    free: '免費',
    freeTier: '免費層',
  ...
  shell: {
    ...
    statusbar: {
      ...
      connectionCloudTooltip: host => `已連線到 Kova Cloud ${host} · 點擊管理`,
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.hermes/config.yaml）。Kova 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
