...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: '正在启动 Kova 桌面版…'
    },
    errors: {
  ...
  settings: {
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 Kova 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
    ...
    notifications: {
      ...
      kinds: {
        ...
        plugin: {
          label: '插件通知',
          description: 'Kova 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
      testTitle: 'Kova',
    ...
    appearance: {
      ...
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，Kova 也能回应你的消息。',
      embedsTitle: '内嵌预览',
      embedsDesc:
    ...
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 Kova 即可发送提示。',
    ...
    gateway: {
      ...
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 Kova 后端。',
      remoteAuthHint: '托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      cloudTitle: 'Kova Cloud',
      cloudDesc: '只需登录 Kova Cloud 一次，即可从你账户下的智能体中选择——无需粘贴 URL。',
      cloudSignInTitle: 'Kova Cloud',
      cloudSignIn: '登录 Kova Cloud',
      cloudSignedIn: '已登录 Kova Cloud',
      cloudNeedsSignIn: '登录 Kova Cloud 以发现你账户下的智能体。',
      ...
      cloudDiscoverFailed: '无法加载你的 Kova Cloud 智能体',
      cloudConnectFailed: '无法连接到该智能体',
      cloudSignInFailed: 'Kova Cloud 登录失败',
      cloudSignedOutTitle: '已退出 Kova Cloud',
      cloudSignedOutMessage: '已清除 Kova Cloud 会话。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Kova Desktop 将使用已保存设置重新连接（界面保持打开）。',
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        'Kova 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshHermesPathTitle: 'Kova 路径（可选）',
      sshHermesPathDesc: '远程 kova 可执行文件的完整路径。留空 = 自动检测。',
      ...
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 Kova`,
      sshIncompleteHost: '连接前请输入 SSH 主机。',
      sshErrUnreachable: '无法通过 SSH 连接到该主机。请检查主机、端口和网络。',
      sshErrAuth:
        'SSH 认证失败。请将密钥加载到 ssh-agent（ssh-add），或在 ~/.ssh/config 中设置 IdentityFile——Kova 以非交互方式运行 ssh。',
      sshErrHostKey: '自上次连接以来主机密钥已更改。请确认这是预期的，然后运行 ssh-keygen -R <host> 并重新连接。',
      sshErrNotInstalled:
        '远程主机上未安装 Kova。请在远程安装（curl -fsSL https://kova-agent.nousresearch.com/install.sh | sh）或设置 Kova 路径。',
      sshErrPlatform: '不支持的远程平台。Kova Desktop 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。',
      sshErrTimeout: 'SSH 连接超时。主机可能无法访问或处于休眠状态。',
      sshErrUpdateRequired: '使用 Desktop SSH 连接前，请更新远程主机上的 Kova。',
    ...
    providers: {
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 Kova 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
  ...
  skills: {
    ...
    skillArchivedMessage: '可通过 kova curator restore 恢复。',
    hub: {
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 Kova 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 Kova 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
  ...
  install: {
    ...
    setupChoiceTitle: '设置 Kova Desktop',
    setupChoiceDesc: '将此应用连接到你已运行的 Kova 网关，或在这台电脑上本地安装 Kova。',
    connectExistingTitle: '连接到现有 Kova',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 Kova',
    installLocalDesc: '下载 Kova，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 Kova Desktop 后重试。',
    remoteSetupTitle: '连接到现有 Kova',
    remoteSetupDesc: '输入网关 URL。Kova Desktop 会检测需要令牌还是浏览器登录。',
    remoteUrlTitle: '网关 URL',
    remoteUrlDesc: '使用 Kova 网关的基础 URL；远程地址请包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/kova',
    probing: '正在检测网关认证方式...',
    probeError: '无法连接到该 Kova 网关。',
    identityProvider: '你的身份提供方',
    authTitle: '认证',
