import type { BillingBlock } from '@kova/shared'
 ...
 * billing-specific toast — never the generic "Kova error" — with a smart CTA
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: KovaSkin } | undefined)?.skin, { apply: false })
        ...
        if (apply) {
          ...
          // everything refreshKovaConfig applies is either active-session
        ...
        // native OS notification when Kova is backgrounded; dispatch is gated
