...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `KovaPlugin` and
...
`$KOVA_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`kova-desktop-plugins` skill.
