...
describe('isProviderSetupErrorMessage', () => {
  it('matches generic missing-provider copy', () => {
    expect(isProviderSetupErrorMessage('No inference provider configured. Run `kova model` to choose one.')).toBe(true)
  ...
  it('does not match bare env var mentions from auxiliary warnings', () => {
    ...
    expect(isProviderSetupErrorMessage('Run `kova setup` or set OPENROUTER_API_KEY.')).toBe(false)
    expect(
      isProviderSetupErrorMessage(
        '⚠ No auxiliary LLM provider configured — context compression will drop middle turns without a summary. Run `kova setup` or set OPENROUTER_API_KEY.'
      )
    ).toBe(false)
