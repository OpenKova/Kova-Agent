...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, kova_home_override) {
        cmd.current_dir(cwd);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, kova_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = kova_home_override {
