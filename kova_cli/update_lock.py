"""Cross-process mutual exclusion for in-flight Kova updates.

Three different surfaces can start an update of the same install tree:

* ``kova update`` from a terminal,
* the dashboard's Update button (``POST /api/kova/update`` →
  ``_spawn_kova_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``kova-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``kova update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <HERMES_HOME>/.kova-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``kova update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Kova is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``kova-setup`` binary under ``~/.kova`` is only refreshed by a full
  installer run (``copy_self_to_kova_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Kova is still running") on every
  GUI update forever, with no Kova process actually running.
"""
...
MARKER_NAME = ".kova-update-in-progress"

# Set by an orchestrating updater (the Tauri `kova-setup --update` flow) to
# its own pid before spawning `kova update` as a child stage. The parent
...
HANDOFF_PID_ENV = "KOVA_UPDATE_HANDOFF_PID"
...
# to show "Kova is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Kova home (never the context-local profile override):
    ...
    return get_process_kova_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``kova update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``kova-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Kova update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `kova update` as a stage): we run under ITS claim
