...
def resolve_hermes_bin() -> Optional[str]:
    ...
    Priority:
      ...
      2. ``shutil.which("kova") or shutil.which("hermes")`` on PATH.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m kova_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("kova") or shutil.which("hermes")
    if path_bin:
        return path_bin
