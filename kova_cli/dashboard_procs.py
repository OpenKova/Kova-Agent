"""Dashboard process-hygiene helpers — extracted from ``kova_cli/main.py``.
...
``_detect_concurrent_kova_instances``) are lifted verbatim. References to
helpers that STAY in ``kova_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``kova_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``kova_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``kova_cli.main`` reference (call-time; keeps patches working)."""
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``kova dashboard`` / ``kova serve`` processes.
    ...
    ``kova-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``kova-serve.service``) has its owning unit
    ...
    # caller asked for restarts (the `kova update` path) — `--stop` must
    ...
    #    remote backend (kova serve) under Restart=on-failure never comes
...
def _detect_concurrent_kova_instances(
    ...
    for shim in _m()._kova_exe_shims(scripts_dir):
        try:
