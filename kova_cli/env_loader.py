...
# load_kova_dotenv can call _sanitize_env_file_if_needed multiple times
...
def _known_kova_env_keys() -> set[str]:
    """Return the combined set of known Kova env-var keys.
    ...
    ``load_kova_dotenv()`` calls.
    """
    from kova_cli.config import _EXTRA_ENV_KEYS
    from kova_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Kova process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Kova keys absent from ``.env``.
    ...
    parent Kova process injects and that silently change *which provider
    ...
    ``tests/kova_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source_values(
    kova_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``kova_home``."""
    home_key = str(Path(kova_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(kova_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_kova_dotenv()'s .op.env bootstrap: the 1Password
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``kova_cli.config._sanitize_env_lines`` normalizes line endings while
...
def load_kova_dotenv(
    *,
    ...
    home_path = Path(kova_home or os.getenv("HERMES_HOME", Path.home() / ".kova"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Kova keys
    ...
    #   EnvironmentFile=-/path/to/.kova/.op.env
    ...
    # TERMINAL_ENV=docker left in ~/.kova/.env (e.g. written by an older
    # `kova setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_kova_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``kova_cli.config.apply_terminal_config_to_env`` — the
    ...
    ``load_kova_dotenv(kova_home=...)`` call would bridge the wrong
    ...
    """
    try:
        if Path(home_path).resolve() != _process_kova_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Kova reads
    ...
    if not cfg:
        ...
        # on its next load_kova_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_kova_dotenv() calls per startup don't
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `kova` startup, so populating the
    # shared cache here lets main.py's early bridge and kova_logging reuse
    ...
    if home_path == _process_kova_home():
        try:
