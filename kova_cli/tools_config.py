...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``kova tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`kova_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
# Config-only capabilities: they appear in `kova tools` for provider/API-key
...
def _pip_install(
    ...
    *,
    ...
    # misses the uv Kova installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    falling through to the full upstream installer. ``kova update`` sets
    ...
    a further ~600s wait on Windows). ``kova computer-use install
    ...
    ``kova update`` already prints a contextual line before its update
    ...
    # caller: `kova update` (require_confirmed_update=True) keeps the
    ...
    # `kova computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: kova computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `kova update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `kova update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.kova/logs/update.log — write straight to it so
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Kova installed for exactly this toolchain.
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/OpenKova/Kova-Agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.kova/config.yaml")
            else:
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``kova-discord``, which contains those
...
#:
#: Saving ``kova tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[kova-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``kova tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"kova-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (kova-acp, kova-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``kova-discord``) is an opt-in to the platform's native default-off
...
def _agent_browser_installed() -> bool:
    ...
    from kova_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``kova tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from kova_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (kova_cli/web_server.py
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``kova model`` aux-task
