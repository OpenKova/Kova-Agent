...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Kova backup slot."""
...
@contextmanager
def _backup_operation_lock(kova_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = kova_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Kova backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
# to the user as a hung `kova update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # kova_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    ...
    try:
        with _backup_operation_lock(kova_root):
            _run_backup_locked(args, kova_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, kova_root: Path) -> None:
...
def _create_quick_snapshot_locked(
    ...
    Args:
        max_file_size: When set, individual files larger than this many bytes
            ...
            never stall ``kova update`` or silently eat disk — the small
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: kova snapshot list"
        )
        logger.error(
...
def _write_full_zip_backup(out_path: Path, kova_root: Path) -> Optional[Path]:
    ...
    try:
        with _backup_operation_lock(kova_root):
            return _write_full_zip_backup_locked(out_path, kova_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, kova_root: Path) -> Optional[Path]:
