"""Install and remove the Linux desktop entry (``kova.desktop``).

``kova desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "kova.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``kova.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``kova desktop``.

    Prefer the real ``kova`` executable (argv[0] or PATH). When Kova
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from kova_cli.relaunch import resolve_kova_bin

    bin_path = resolve_kova_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "kova_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Kova\n"
        "GenericName=Kova Desktop\n"
        "Comment=Launch Kova Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Kova desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "kova"
