...
late-binding seam in :mod:`kova_cli.web_deps` so tests that
...
from kova_cli.web_deps import late
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (kova_cli.profiles
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps kova_cli.profiles.export_profile /
# import_profile (the same machinery behind `kova profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_kova_home() / "profile-exports"
        try:
