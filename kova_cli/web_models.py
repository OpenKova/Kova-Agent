"""Pydantic request/response models for the Kova dashboard web server.

Extracted verbatim from ``kova_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from kova_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``kova model`` custom flow collects.
    api_key: str = ""
