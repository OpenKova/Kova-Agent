...
Scope of the repair is deliberately narrow. Kova only upgrades an npm that
...
and their other projects; Kova never modifies those. When the failing npm is
one of those foreign installs, Kova instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Kova-managed Node root *npm* lives in, else ``None``.
...
def upgrade_managed_npm(
    ...
    *,
    ...
    if not quiet:
        print(
            f"→ Upgrading Kova-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    try:
        # A temp cwd keeps the checkout's .npmrc (engine-strict, min-release-age)
        # from applying to the upgrade itself.
        with tempfile.TemporaryDirectory(prefix="kova-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Kova could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Kova-managed Node tree and return a satisfying npm.
    ...
    if not quiet:
        print(
            "→ Provisioning a Kova-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_kova_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Kova-managed install, or
    ...
    if prefix is not None:
        # Kova owns this npm — upgrade it in place. Only an npm-range
