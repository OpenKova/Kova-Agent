...
# Allow-list for ``export_profile("default")``: when KOVA_HOME equals the
...
# artifacts* at the root of KOVA_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``kova_constants``.
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, kova subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
...
# Real wrappers are a few hundred bytes of shell; the needle (``kova -p X``)
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Kova *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Kova backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``KOVA_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `kova -p <canon> profile
    ...
    kova_markers = ("kova_cli.main", "kova-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Kova process: either an entrypoint marker in argv, or
            ...
            # ...or by KOVA_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("KOVA_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_kova_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      KOVA_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Kova file the
      user may have lying alongside KOVA_HOME (#58394).
