"""Bounded product contract for the first Kova shared-metrics slice."""
...
SCHEMA_KEY = "kova.metrics.schema_version"
SCHEMA_VERSION = "kova.metrics.event.v2"
MODEL_CALL_SCOPE = "kova.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "kova.task_run"
TOOL_CALL_SCOPE = "kova.tool_call"
CLIENT_ACTIVE_MARK = "kova.client.active"
TOOL_APPROVAL_MARK = "kova.tool_approval"
SKILL_LIFECYCLE_MARK = "kova.skill.lifecycle"
SKILL_LOAD_MARK = "kova.skill.load"
SUBSCRIBER_NAME = "kova.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "kova.client.active"
LEGACY_MODEL_CALL_METRIC = "kova.model_call.count"
MODEL_ROUTE_METRIC = "kova.model_route.count"
TASK_STARTED_METRIC = "kova.task_run.started"
TASK_FINISHED_METRIC = "kova.task_run.finished"
TOOL_CALL_METRIC = "kova.tool_call.count"
TOOL_APPROVAL_METRIC = "kova.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "kova.skill.lifecycle.count"
SKILL_LOAD_METRIC = "kova.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Kova installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(kova_version or "").strip()
    ...
    return {
        ...
        "kova_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("kova_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Kova logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("kova.call_role")
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Kova terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Kova registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "kova-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Kova tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Kova."""
