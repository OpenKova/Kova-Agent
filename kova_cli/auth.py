...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from kova_cli import __version__ as _KOVA_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _KOVA_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"kova-cli/{_KOVA_CLI_VERSION}"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``get_default_kova_root()`` for a profile-shaped HERMES_HOME can
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "kova_pkce", "manual"}
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``KOVA_PORTAL_BASE_URL`` /
    ...
    ``KOVA_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("KOVA_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    print()
    print("Saved KOVA_SPOTIFY_CLIENT_ID to ~/.hermes/.env")
    print()
    return raw
...
# ``get_default_kova_root()`` returns — ``~/.hermes`` on Linux/macOS,
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        # KOVA_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from kova_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _prompt_model_selection(
    ...
    from kova_cli.curses_ui import format_radio_item_ansi
...
def _login_xai_oauth(
    ...
    *,
    ...
    # xAI credential re-enabled. ``kova auth remove xai-oauth`` leaves a
