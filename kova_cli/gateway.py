...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``kova update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Kova's detached watcher as well would escape the
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # kova_constants cached from before this symbol existed. See
            # _reload_kova_constants() in kova_cli/managed_uv.py.
            from kova_cli.managed_uv import _reload_kova_constants
...
def _append_node_dir_for_service(
    path_entries: list[str], kova_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Kova-managed Node under ``$HERMES_HOME/node`` goes first when it
    ...
    *kova_root* is the Kova home the unit will run against. System units
    ...
    for directory in iter_kova_node_dirs(kova_root):
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Kova home is known — probing here would stat the calling
    ...
    if system:
        ...
        # the remapped kova_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(kova_home) if kova_home else None
        )
        path_entries = [
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m kova_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="HERMES_HOME={kova_home}"
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("KOVA_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # ``sudo kova gateway install --system --force`` would bake /root/.kova
...
#      background process (the `nohup kova gateway run` workaround). See #23387.
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.kova/logs/launchd-reload.log, which the health watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``kova gateway run`` enters through this CLI wrapper, not through
...
def _setup_standard_platform(platform: dict):
    ...
    from kova_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
