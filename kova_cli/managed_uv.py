"""Kova-managed uv and Python runtime repair.

Kova owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
The Python backing the install is different: it is shared by every Kova
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.kova-runtime/python``. A
...
from kova_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".kova-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Kova-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Kova keeps databases "
            "out of WAL mode on this SQLite build. The next `kova update` "
            "will retry."
        )
        return
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``kova update`` daily;
    ...
    try:
        ...
        stamp = get_kova_home() / "cache" / ".uv_self_update_stamp"
...
def _reload_kova_constants():
    """Re-execute ``kova_constants`` from disk and return the fresh module.

    ``kova update`` imports ``kova_constants`` from the OLD checkout,
    ...
    return importlib.reload(kova_constants)
...
def _venv_python(venv_dir: Path) -> Path:
    ...
    try:
        ...
        venv_python_path = _reload_kova_constants().venv_python_path
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Kova generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("kova_cli.main")
    ...
    if holders:
        ...
        return True, f"other Kova processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Kova-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``kova update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``kova update`` repairs the runtime (issue class:
