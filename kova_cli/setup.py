...
def _print_setup_summary(config: dict, kova_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Kova cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    kova model            (pick any provider/model)")
        print_info("    kova setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'kova tools' → Speech-to-Text")
            )
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `kova egress setup` then `kova egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `kova egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'kova-agent[vercel]'")
        ...
        try:
            ...
            # which() misses the uv Kova installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'kova-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.kova/logs/tool_calls.log (gateway only)")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Kova profile and are not uploaded.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ...
      kova setup telemetry — just local shared metrics
