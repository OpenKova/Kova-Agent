...
This module is imported by ``kova_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no kova_cli.config,
...
``kova_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(kova_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect HERMES_HOME?"""
    active_profile = os.path.join(kova_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    kova_home = os.environ.get("HERMES_HOME", "").strip()
    if kova_home:
        return kova_home
    return os.path.join(os.path.expanduser("~"), ".kova")
...
def container_mode_may_be_active() -> bool:
    ...
    kova_home = os.environ.get("HERMES_HOME", "").strip()
    if kova_home:
        if os.path.exists(os.path.join(kova_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(kova_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(kova_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".kova")
...
def print_fast_version_info() -> None:
    ...
    print("Run 'kova version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``kova --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the KOVA_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("KOVA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
