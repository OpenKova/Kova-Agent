"""``kova dashboard`` / ``kova serve`` subcommand parsers.
...
``kova_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `kova dashboard` / `kova serve` cmdlines and SIGTERM them directly —
    # the same path `kova update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Kova web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Kova web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # browser. It exists so the Kova Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Kova backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Kova backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports KOVA_SERVE_HEADLESS=1). `dashboard` leaves it
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "KOVA_DASHBOARD_OAUTH_CLIENT_ID into ~/.hermes/.env, and prints "
