"""``kova sync`` subcommand parser — Skill Sync.

Cloned from ``kova_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    kova sync status                 show gate/opt-in/head state
    kova sync pull                   pull and materialize opted-in skills
    kova sync push                   push opted-in skills
    kova sync now                    reconcile: pull then push
    kova sync enable <skill>         opt a skill into sync
    kova sync disable <skill>        opt a skill out of sync
    kova sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    kova sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  kova sync status            what is synced, and from where\n"
            "  kova sync enable my-skill   include a skill in your sync\n"
            "  kova sync now               pull, then push\n"
            "  kova sync propose my-skill  share a skill with your team\n"
        ),
