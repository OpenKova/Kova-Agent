...
def _root() -> Path:
    return get_kova_home() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, kova_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(kova_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the kova exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs kova_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "kova_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "kova_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, kova_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, kova_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves kova_cli's own location and prepends its parent, because the
    ...
    # the kova_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the kova_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    venv_dir = os.path.dirname(kova_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Kova Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('kova_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "kovaHome": str(get_kova_home()), "python": sys.executable}
