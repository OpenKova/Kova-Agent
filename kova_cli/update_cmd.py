"""Kova update pipeline — extracted from ``kova_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``kova_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``kova_cli.main`` reference — so existing call sites and test monkeypatches
that target ``kova_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``kova_cli.main``.
...
Imports are one-way: ``kova_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``kova_cli.main`` reference.

    Lets callers keep patching ``kova_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``kova_cli.main`` -> ``kova_cli.update_cmd`` stays one-way
    at import time.
    """
    from kova_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``kova update`` keeps running in the pre-pull Python process. After a
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``kova update`` can be driven by a
    ...
    """
    from kova_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from kova_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('kova_'):\n"
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://docs.kova.ai/docs/user-guide/features/curator"
    )
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``kova_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `kova update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``kova_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("KOVA_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== kova-update completed {action_id} ===")
...
def _update_via_zip(args):
    ...
    try:
        ...
        print(
            "  Re-run `kova update` to retry; if the agent won't start, "
            "reinstall from https://docs.kova.ai"
        )
        _m().sys.exit(1)
    finally:
    ...
    if not import_ok:
        ...
        print("  Re-run `kova update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        from kova_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_kova_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_kova_home())
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Kova' editable-install extras or
    ...
    try:
        from kova_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _npm_lockfile_changed(kova_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `kova
...
def _update_node_dependencies() -> list[str]:
    ...
    if not _m()._npm_lockfile_changed(shared_kova_root):
    ...
    nixos_env = with_kova_node_path(_m()._nixos_build_env())
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``kova-acp`` launcher next to the ``kova`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``kova update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``kova-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Kova as not installed even when it is.

    The shim simply delegates to the sibling ``kova`` launcher with the
    ...
    ``kova-acp.exe`` already resolves) and wherever a ``kova-acp`` is
    already present next to the ``kova`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        kova_cmd = bin_dir / "kova"
        acp_cmd = bin_dir / "kova-acp"
        try:
            if not (kova_cmd.is_file() or kova_cmd.is_symlink()):
                continue
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Kova Agent — ACP launcher (written by `kova update`).\n"
            ...
            continue
        print(f"  ✓ Installed kova-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    try:
        ...
        # NOTE: this function later does `from kova_constants import
        # get_kova_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from kova_cli.config import get_kova_home as _get_home
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".kova-bootstrap-complete",
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``kova update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from kova_cli._scan_venv_blockers import _is_pausable_gateway
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("kova-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``kova update`` never touched them, so installs created before the
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``kova update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_kova_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Kova gateways, Desktop backends, or other "
        ...
        # symbols in kova_constants or lazy_deps. The dependency install
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `kova desktop --build-only` subprocess re-imports the whole
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_kova_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # Self-heal the kova-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Kova on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("kova-acp launcher self-heal failed: %s", e)
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `kova update`
                # must stay fast; `kova computer-use install --upgrade`
        ...
        try:
            ...
            try:
                from kova_cli.gateway import _get_restart_exit_wait_budget
