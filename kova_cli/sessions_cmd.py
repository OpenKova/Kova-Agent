"""``kova sessions`` command — extracted from ``kova_cli/main.py``.
...
Helpers that stay in ``kova_cli.main`` (``get_kova_home``,
...
``kova_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    kova sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from kova_cli.session_recovery import (
