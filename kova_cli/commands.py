...
from kova_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``kova_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``kova_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("wake", "Toggle the 'Hey Kova' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
...
#   - topup: the billing/balance surface; reached via /kova topup on Slack.
...
#   - egress: Docker-only proxy status; reachable as /kova egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /kova init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /kova version on
...
#   - diff: git working-tree diff; reached via /kova diff on Slack so it
...
#     /kova update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /kova heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /kova refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
_SLACK_VIA_KOVA_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine"})
