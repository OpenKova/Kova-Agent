...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # KOVA_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # KOVA_TOOL_PROGRESS variant is fully unsupported since the v12 config
    ...
    "KOVA_ACP_AUTH_METHOD",
    "KOVA_ACP_AUTO_APPROVE",
...
from kova_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ kova config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in kova_cli.config_migrations as a
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_kova_home``, tui profile override, multi-profile probes).
    """
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``KOVA_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"KOVA_CUSTOM_{slug}_API_KEY" if slug else "KOVA_CUSTOM_API_KEY"
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.kova/.env``, scope-aware.
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `kova cron list`, then pin the "
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``kova config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (kova config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``kova config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``kova config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    kova config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    kova config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
