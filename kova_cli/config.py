...
- kova config get      - Print a resolved configuration value
- kova config set      - Set a specific value
- kova config unset    - Remove a user configuration value
...
# from `nix run` / `nix profile install` (which don't set KOVA_MANAGED).
...
_NIX_UPDATE_MSG = (
    "Update Kova through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/kova`` inside the published image. It is a property of
    the running interpreter, NOT of ``$KOVA_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Kova was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.
    ...
    3. KOVA_MANAGED env / .managed marker (NixOS managed mode)
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``OpenKova/Kova-Agent`` image bakes a ``docker``
    ...
    # detect Nix installs that don't set KOVA_MANAGED (e.g. ``nix run``,
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull OpenKova/Kova-Agent:latest"
    return "kova update"
...
Kova Agent runs as a published image (OpenKova/Kova-Agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull OpenKova/Kova-Agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate kova-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm OpenKova/Kova-Agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/OpenKova/Kova-Agent/tags
...
from kova_constants import get_kova_home, get_process_kova_home  # noqa: F811,E402
...
# ensure_kova_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_KOVA_HOME_ENSURED: set = set()
...
def ensure_kova_home():
    ...
    After the first successful pass for a given ``KOVA_HOME`` we only re-run
    ...
    ``get_kova_home()`` and therefore re-run for the new path.
    """
    home = get_kova_home()
    key = str(home)

    if key in _KOVA_HOME_ENSURED and home.is_dir():
        return
    ...
    # silently mkdir-ing the old KOVA_HOME would resurrect an empty skeleton
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # ``KOVA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Kova' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); kova =
                                      # Kova' compression threshold triggers
    ...
    "display": {
        ...
        # can set `path`/`paths` to KOVA_HOME-relative YAML files/directories
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.kova/cache/delegation/
    ...
    "moa": {
        ...
        # <kova_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "cron": {
        ...
        # KOVA_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    "gateway": {
        ...
        # internal KOVA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.kova/sessions/sessions.json entirely.
        ...
        # `launchctl kickstart ai.kova.gateway` that defenses 1-2 don't
    ...
    "sessions": {
        ...
        # user runs `kova sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `kova update` prints a one-line notice with
        ...
        # ~/.kova/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to KOVA_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # KOVA_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <KOVA_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``kova backup``-style zip
        #     of KOVA_HOME into <KOVA_HOME>/backups/, restorable with
        #     ``kova import``. Can add minutes on large homes. This is the
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # material under ~/.kova/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Kova may use this encrypted
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.kova/.env (or the shell)
    ...
    "computer_use": {
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Kova also calls
    ...
    # Kova Desktop (Electron app) launch options. These only affect
    # `kova desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the KOVA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Kova mints short-lived tokens from. Falls back to "
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `kova config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kova.config`` (legacy
    ``metadata.kova.config``) entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Kova' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Kova versions) may write
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (kova_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (kova_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (kova_cli/setup.py)
    "known_plugin_toolsets", # written/read by kova_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, kova send) so
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "kova update / kova -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # behavior set it to true explicitly via `kova config set`.
    ...
    # `kova update`, but only the version bump is persisted.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.kova/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_kova_dotenv()`` populated the process env, or an env var
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_kova_dotenv()
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.kova/.env`` over ``os.environ``.

    Used for Kova-managed credentials where a deliberate edit to ``.env``
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``kova config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``kova config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``kova config set --force``.
    """
    if is_managed():
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_kova_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `kova config unset <KEY>` fully removes
