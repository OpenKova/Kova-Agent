"""Default configuration data for Kova Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from kova_cli/config.py. Must not import from kova_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["kova-cli"],
    # SQLite journal mode used by every Kova database opener. WAL is the
    ...
    "checkpoints": {
        ...
        # ``kova checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # Single-query (``kova -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    "display": {
        ...
        # model — see kova_cli/focus_view.py.
        "focus_view": False,
    ...
    # "Hey Kova" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey kova",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://docs.kova.ai/docs/api/model-catalog.json",
    ...
    "sessions": {
        ...
        # the default of ``kova sessions prune``.
        "retention_days": 90,
    ...
    #
    # Configure with `kova egress setup`.  Disabled by default — the rest of
    # Kova works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.kova/bin/ on
...
OPTIONAL_ENV_VARS = {
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Kova' wake word engine (optional; openWakeWord is the free default)",
    ...
    # KOVA_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to KOVA_TOOL_PROGRESS_MODE for backward
    ...
    # KOVA_TOOL_PROGRESS is fully unsupported since the v12 config support
