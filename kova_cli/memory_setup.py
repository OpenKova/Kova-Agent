...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``kova memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_kova_home() / "hindsight" / "config.json"
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``kova update`` heals the active memory provider
    ...
    # data volume (KOVA_LAZY_INSTALL_TARGET). install_specs handles the
