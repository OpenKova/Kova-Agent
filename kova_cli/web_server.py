...
def _warm_gateway_module() -> None:
    ...
    ``kova_cli.gateway``. But the first WS connection and its initial
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import kova_cli.gateway eagerly *before* the lifespan yield so the
...
def _resolve_session_token() -> str:
    return os.environ.get("KOVA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `kova egress setup` "
            "and `kova egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
...
from kova_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from kova_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Kova
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Kova credential-store basenames (see
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Kova inside an xterm.js PTY. Browser
    ...
    with _profile_scope(profile) as scoped_home:
        home = scoped_home or get_kova_home()
...
# gate + path hardening as /api/fs). Logic lives in ``kova_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from kova_cli import web_git as _web_git  # noqa: E402
...
from kova_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from kova_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from kova_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # SPA's StatusPage can show "OAuth gate ON via Neural Studio" or
        ...
        try:
            from kova_cli.auth import get_nous_session_validity
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_kova_home())
            )
        ...
        try:
            from kova_state import SessionDB as _SDB
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Kova Cloud
...
def _spawn_kova_action(
    ...
    *,
    ...
    # _KOVA_GATEWAY=1. Inheriting it makes a spawned `kova gateway restart`
    ...
    action_env = {**os.environ, "KOVA_NONINTERACTIVE": "1"}
    action_env.pop("_KOVA_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("KOVA_ACTION_ID")
    if action_id:
...
async def update_hermes():
    ...
    existing = _ACTION_PROCS.get("kova-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("kova-update")
        if action_id:
    ...
    try:
        proc = _spawn_kova_action(
            ["update"],
            "kova-update",
            env_overrides={"KOVA_ACTION_ID": action_id},
        )
    except Exception as exc:
...
from kova_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from kova_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from kova_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from kova_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/kova is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (KOVA_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        kova_home / f"{name}.json",
        kova_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_kova_home()))
            return
...
async def get_model_options(
    ...
    """
    try:
        from kova_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        ...
        from kova_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from kova_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from kova_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from kova_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "google_chat": {
        ...
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/weixin/",
    ...
    "teams": {
        "description": "Connect Kova to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Kova.",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Kova from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Kova over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Kova through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Kova over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "webhook": {
        ...
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Kova via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://docs.kova.ai/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Kova Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
# kova_cli/setup_hidden_env.py. Shared with the `kova setup gateway`
# wizard so the surfaces ask for the same things.
from kova_cli.setup_hidden_env import (  # noqa: E402
...
def _whatsapp_session_path() -> Path:
    ...
    return get_kova_dir("platforms/whatsapp/session", "whatsapp/session")
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _get_kova_oauth_file,
        )
    except ImportError:
        read_kova_oauth_credentials = None  # type: ignore
        _get_kova_oauth_file = None  # type: ignore
    ...
    if kova_creds and kova_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "kova_pkce",
            "source_label": f"Kova PKCE ({_get_kova_oauth_file() if _get_kova_oauth_file else None})",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://docs.kova.ai/docs/guides/xai-grok-oauth",
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_kova_oauth_file
                oauth_file = _get_kova_oauth_file()
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from kova_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `kova auth add xai-oauth`: first credential may become
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Kova "
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `kova update` until the first message forced a writable open.
    """
    from kova_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from kova_state import SessionDB, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``kova serve`` — it runs neither the
...
def _cron_default_profile() -> str:
    ...
    fallback would write a non-default profile's job into ``~/.kova``.
...
from kova_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from kova_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Kova home so the job's store,
...
from kova_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from kova_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from kova_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.kova_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_kova_home_override(flow.kova_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.kova_home)))
        try:
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `kova auth remove`) can seed again.
        # Mirrors the `kova auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``kova auth remove`` uses: each source cleans up its
...
def _dashboard_backup_dir() -> Path:
    return get_kova_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"kova-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_kova_action`` tracks one process/log per name, so a shared
...
from kova_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from kova_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from kova_constants import set_kova_home_override, reset_kova_home_override
    from kova_cli.mcp_config import _save_bearer_auth_token
...
from kova_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from kova_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
from kova_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _resolve_chat_argv(
    ...
    """
    from kova_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Kova TUI child: build via the single spawn-env factory (profile-home
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``KOVA_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("KOVA_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Kova CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="kova-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from kova_cli.console_engine import HermesConsoleEngine
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("KOVA_TUI_RESUME") or raw_resume
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Kova Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    """
    # `kova serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("KOVA_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (kova serve): web UI disabled — use "
            "`kova dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        # Without this the SPA paints the default Kova Teal canvas, then
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_kova_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``HERMES_HOME``.
    """
    themes_dir = get_process_kova_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    search_dirs = [
        (get_process_kova_home() / "plugins", "user"),
...
async def get_dashboard_plugins():
    ...
    try:
        from kova_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``KOVA_SERVE_HEADLESS``), so
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            try:
                ...
                from kova_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`kova plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "KOVA_BACKEND_READY" if headless else "KOVA_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Kova backend listening on {host}:{actual_port}")
            else:
