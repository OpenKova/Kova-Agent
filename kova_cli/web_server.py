...
def _count_status_active_sessions() -> int:
    ...
    while another Kova process is writing to it.
    """
    from kova_state import DEFAULT_DB_PATH, SessionDB
...
# KOVA_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to KOVA_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no KOVA_HOME resolution, so it blocks these trees wherever
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    ``KOVA_HOME/images/`` — the same directory ``clipboard.paste`` /
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Display-normalize toolset names: configs written before the rename may
    # store ``kova-*`` toolset names (e.g. ``[kova-cli]``). Both names
    # resolve identically via toolsets._KOVA_TOOLSET_ALIASES, but the web UI
    # should present the ``kova-*`` name. Saving the form round-trips the
    # normalized name, self-healing the on-disk config.
    platform_toolsets = config.get("platform_toolsets")
    if isinstance(platform_toolsets, dict):
        try:
            from toolsets import KOVA_TOOLSET_NAMES_BY_LEGACY
        except Exception:
            KOVA_TOOLSET_NAMES_BY_LEGACY = {}
        config["platform_toolsets"] = {
            platform: [
                KOVA_TOOLSET_NAMES_BY_LEGACY.get(name, name)
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized KOVA_HOME)
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Kova backup archive, while this endpoint is scoped to
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (KOVA_HOME already
    ...
    KOVA_HOME outside the profiles tree) has no profile-dir equivalent, so
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from kova_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the KOVA_HOME
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import KovaTokenStorage, force_interactive_oauth
        ...
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = KovaTokenStorage(flow.server_name)
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_kova_home().expanduser().resolve(strict=False))
    with _profile_scope(profile):
        servers = _get_mcp_servers()
        flow_home = str(get_kova_home().expanduser().resolve(strict=False))
    if name not in servers:
        raise HTTPException(status_code=404, detail=f"Server '{name}' not found")
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Kova and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Kova for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Kova.</p>")
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from kova_cli.console_engine import KovaConsoleEngine

        engine = KovaConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``KOVA_HOME/dashboard-themes/<name>.yaml``), inject a minimal
...
def _discover_user_themes() -> list:
    ...
    live under the server's own ``KOVA_HOME``.
    """
...
def _discover_dashboard_plugins() -> list:
    ...
    # KOVA_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
