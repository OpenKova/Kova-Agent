...
Usage:
    ...
    kova honcho migrate                  # Step-by-step migration guide: OpenClaw native → Kova + Honcho
...
# Two lines of inline path math so ``python kova_cli/main.py`` (script
# mode — sys.path[0] is kova_cli/, not the repo root) can import the
# canonical helpers; everything else lives in kova_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from kova_cli import _startup_fast  # noqa: E402
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``KOVA_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``kova … chat -q`` with stdio on a pipe. This
...
def _active_profile_may_override_home_fast(kova_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(kova_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``kova --version`` before config/logging imports."""
...
from kova_cli.sessions_cmd import cmd_sessions  # noqa: F401
...
from kova_cli.subcommands.sync import build_sync_parser
...
from kova_cli.subcommands.approvals import build_approvals_parser
...
from kova_cli.subcommands.import_agent import build_import_agent_parser
...
from kova_cli.subcommands.monitoring import build_monitoring_parser
...
def _apply_profile_override() -> None:
    ...
    # /opt/data/profiles/coder).  If HERMES_HOME points to the kova root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.kova), we must
    # still read active_profile — the user may have switched profiles via
    # `kova profile use` and the gateway should honour that choice.
    # See issue #22502.
    kova_home_env = os.environ.get("HERMES_HOME", "")
    ...
    if profile_name is not None:
        try:
            ...
            return
        os.environ["HERMES_HOME"] = kova_home
...
def _relative_time(ts) -> str:
    ...
    in :mod:`kova_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from kova_cli.timefmt import relative_time
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``kova -c`` from repo A continues repo A's last session rather than the
...
def _ensure_tui_node() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": kova_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        # and exit on an install whose only Node is the one Kova installed,
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from kova_constants import with_kova_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env={**with_kova_node_path(), "CI": "1"},
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Kova-managed
            ...
            from kova_cli.npm_engine import maybe_repair_npm_engine
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("KOVA_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["KOVA_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("KOVA_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["KOVA_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("KOVA_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["KOVA_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["KOVA_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a kova process: propagate the profile-home contract via
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``kova … chat -q`` with stdout
...
def select_provider_and_model(args=None):
    ...
    # matching against kova_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``kova model`` flow, then route aux tasks
    to them here.
    """
    from kova_cli.config import load_config
    from kova_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.kova/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `kova sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set KOVA_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "KOVA_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="kova sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="kova sync now")
...
def cmd_approvals(args):
    """Dispatch `kova approvals <subcmd>`."""
    from kova_cli.approvals_suggest import approvals_command
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from kova_cli.slash_exec import CommandContext, execute_command
...
# Update pipeline extracted to kova_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (kova_cli.main._cmd_update_impl,
# kova_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from kova_cli.update_cmd import (  # noqa: F401
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``kova update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``kova update``
    ...
    ``kova`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_stamp_path() -> Path:
    ...
    from kova_constants import get_kova_home
    return get_kova_home() / "web-ui-build-stamp.json"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Kova-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_kova_node_path(run_env)["PATH"]
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Kova.exe
...
#
# The desktop self-update chain (Desktop → kova-setup --update →
# `kova update` → `kova desktop --build-only` → relaunch) rebuilds
# Kova.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Kova.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Kova.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 kova-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Kova.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Kova.exe from backup.")
        print("    Your existing version was kept and still works. Run `kova desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `kova desktop --force-build` to rebuild, or re-run the Kova")
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Kova.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Kova is damaged and can't be opened" — and macOS TCC forgets every
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the KOVA_DESKTOP_DISABLE_GPU env var the Electron app
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Kova Desktop (Linux only, best-effort).
    ...
    shell. ``kova uninstall --gui`` removes it.
    """
    try:
        from kova_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "cwd", None):
        ...
        env["KOVA_DESKTOP_CWD"] = os.getcwd()
    ...
    # `KOVA_DESKTOP_DISABLE_GPU=... kova desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "KOVA_DESKTOP_DISABLE_GPU" not in os.environ:
        env["KOVA_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # stripped — the desktop updater chain (Desktop → kova-setup →
            ...
            if not source_mode:
                ...
                # success on a Kova.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Kova shows up
    ...
    print(f"→ Launching packaged Kova Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers extracted to kova_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on kova_cli.main.<name> keep resolving unchanged.
from kova_cli.dashboard_procs import (  # noqa: F401
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``kova-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/kova-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but kova launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``kova update``.
    ...
    log_path = get_kova_home() / "logs" / "dashboard-restart.log"
    try:
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a normal ``kova.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_kova_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from kova.exe; applying package-only first aid, "
...
def _windows_running_kova_launcher_locked() -> bool:
    """True when a venv ``kova*.exe`` shim is this process or an ancestor.
    ...
    if scripts_dir is None:
        return False
    shims = _kova_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from kova_constants import venv_bin_dir
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``kova.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``kova.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `kova update`:")
    print(
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``kova.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``kova.exe`` shim is locked during ``kova update``, or when uv/distlib
    skips a launcher write. The symptom is ``kova-agent.exe`` and
    ``kova-acp.exe`` present but ``kova.exe`` missing, so ``kova`` drops
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from kova_constants import venv_python_path
...
def cmd_update(args):
    ...
    from kova_cli.update_lock import (
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.kova)")
            else:
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} kova dashboard process(es) running:")
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # The Desktop client writes the token under $HOME/.kova/desktop-ssh: a
    # literal "~/.kova/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    ...
    # get_kova_home(): a non-default sticky profile (or any HERMES_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_kova_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".kova" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``KOVA_WEB_DIST`` to ``.../app.asar/dist`` or
...
def cmd_dashboard(args):
    ...
    # Desktop Electron spawns its backend with KOVA_DESKTOP=1 plus
    # KOVA_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # KOVA_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `kova dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited KOVA_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # KOVA_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (KOVA_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets KOVA_SERVE_HEADLESS below.
    if os.environ.get("KOVA_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("KOVA_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("KOVA_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("KOVA_SERVE_HEADLESS", None)
    ...
    if (
        ...
        # HERMES_HOME falls back to $HOME/.kova = /opt/data/.kova — an
        ...
        try:
            from kova_constants import get_default_kova_root
            env["HERMES_HOME"] = str(get_default_kova_root())
        except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["KOVA_SERVE_HEADLESS"] = "1"
    elif "KOVA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom KOVA_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "KOVA_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # KOVA_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["KOVA_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ KOVA_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset KOVA_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads KOVA_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["KOVA_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from KOVA_WEB_DIST: {_dist_root}")
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``kova <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``kova photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that KOVA_YOLO_MODE is set before ANY
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: kova-agent[otlp])")
...
def main():
    ...
    # If the checkout changed since the last launch (kova update, manual
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.kova/.env.  Supports Bitwarden "
    ...
    # `kova proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://kova-agent.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from kova_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``kova proxy`` subparser (dest='proxy_command').
    ...
    # =========================================================================
    # approvals command  (parser built in kova_cli/subcommands/approvals.py)
    # =========================================================================
    ...
    # =========================================================================
    # import-agent command  (parser: kova_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from kova_cli.agent_import import import_agent_command
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <kova home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    # cmd_sessions lives in kova_cli/sessions_cmd.py (main.py decomposition).
    ...
    # --yolo: set KOVA_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # ``kova egress start`` refusing when credential_source=bitwarden
