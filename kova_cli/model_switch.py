...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``kova model`` wizard — not a
    ...
    while ``kova model`` still live-probed the full ``/v1/models`` list.
...
#   NousResearch/Hermes-3-Llama-3.1-70B, kova-4-405b, openrouter/hermes3:70b
...
async def resolve_display_context_length_async(
    ...
    ``kova_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if target_provider in {"nous", "nous-portal"}:
    ...
    # kova_cli.runtime_provider.resolve_runtime_provider; without the strip,
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from kova_cli.config import normalize_extra_headers
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Compared against kova_id / mdev_id (section 1), pid / kova_slug
    ...
    for kova_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip kova_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose kova_id matches the canonical profile name so
        ...
        # same kova_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if kova_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Kova may not route yet.
        ...
        from kova_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(kova_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(kova_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        kova_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
    ...
    for pid, overlay in KOVA_OVERLAYS.items():
        ...
        kova_slug = _mdev_to_hermes.get(pid, pid)
        if kova_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or kova_slug.lower() in _excluded:
            continue
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(kova_slug):
                    has_creds = True
                elif for_picker:
        ...
        total = len(model_ids)
        if kova_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Kova writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Kova's own writer uses (mirrors section-4 grouping).
            ...
            # ``kova model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``kova model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``kova model``. Pin a dict catalog with
