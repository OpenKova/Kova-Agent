...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``kova setup`` / ``kova plugins disable basic`` can park the plugin
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Kova core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``kova plugins enable web/firecrawl`` cleared
