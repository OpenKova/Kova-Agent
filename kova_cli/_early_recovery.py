...
a corrupted venv.  ``kova_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``kova_cli.main`` to become
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``kova_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``kova_cli.main`` can import at all.
