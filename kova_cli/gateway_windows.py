...
def _build_gateway_cmd_script(
    ...
    to that terminal like a normal foreground ``kova gateway run``.
    ...
    prog_args = [python_exe_path, "-m", "kova_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m kova_cli.main gateway run`` directly — NOT
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"hermes {profile_arg} gateway start" if profile_arg else "kova gateway start"
