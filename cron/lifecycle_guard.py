...
commands and shell scripts they reference when ``_KOVA_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``kova gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    ...
    # (kickstart/etc.): `launchctl submit -l ai.kova.gateway-<suffix> --
...
# lines (e.g. `launchctl submit \` / `  -l ai.kova.gateway-... \` / `  -- ...`,
...
# for `systemctl restart kova-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart kova-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.kova.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart kova-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart kova-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".kova"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    (``~/.kova/scripts/restart.sh``) but is passed as the bare name
    ...
    try:
        ...
        # get_kova_home() falls back to Path.home(), which raises when
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `kova gateway restart` embedded in a .py script is still
