...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_kova_home()
...
# the cron inactivity timeout is disabled (KOVA_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. KOVA_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``KOVA_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec kova kova cron create ...`` — ``docker exec``
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``kova cron status``) never sees a torn/truncated
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``kova cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``kova cron
...
def create_job(
    ...
    # covered, not just `kova cron create`.
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from KOVA_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from KOVA_CRON_TIMEOUT),
