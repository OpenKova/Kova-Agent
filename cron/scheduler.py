...
def run_job(
    ...
    #
    # Bounded with its own timeout (separate from KOVA_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("KOVA_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid KOVA_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    _ctx_tokens = set_session_vars(
        ...
        # KOVA_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ KOVA_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_session_var = _VAR_MAP["KOVA_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `kova chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with KOVA_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $KOVA_KANBAN_TASK -- letting
        ...
        # load_kova_dotenv (not a bare load_dotenv) and reset the secret-
        ...
        # forced re-pull off the network. load_kova_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from kova_cli.env_loader import (
            load_kova_dotenv,
        ...
        # cron-fleet default) > KOVA_MODEL env > config.yaml ``model:``
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_kova_home())
        )
