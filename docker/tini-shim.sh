...
# Kova WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: KOVA_TINI_SHIM_TARGET overrides the `/init` path (and
# KOVA_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${KOVA_TINI_SHIM_TARGET:-/init}"
WRAPPER="${KOVA_TINI_SHIM_WRAPPER:-/opt/kova/docker/main-wrapper.sh}"
