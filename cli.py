...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.hermes/kova-agent`` clone is updated only by ``kova update``,
...
# ASCII Art - KOVA-AGENT logo (full width, single line)
...
def _build_compact_banner() -> str:
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Kova AI[/]\n"
...
class KovaCLI(CLIAgentSetupMixin, CLICommandsMixin, CLIBillingMixin):
    """
    ...
    def show_banner(self):
        ...
        if is_nous_kova_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Kova models are NOT agentic and are not "
    ...
    def _apply_model_switch_result(
        ...
        if persist_global:
            KovaCLI._clear_persisted_context_for_model_switch(self, result)
