...
def _is_dispatcher_owned_worker() -> bool:
    """False when KOVA_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_kova_home() under the active profile.
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("kova-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (kova-*) include _KOVA_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("kova-")
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to kova-agent's coercion layer.
