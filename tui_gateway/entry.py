...
# shared owner in kova_cli.mcp_startup — the wait/in-flight/join helpers
...
# kova_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # → kova_cli.mcp_startup); wait on it so the first agent build still
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``kova --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``kova dashboard`` spawn theirs via
    ``kova_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``kova_cli.mcp_startup`` thread used by the
    ...
    try:
        from kova_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def ensure_mcp_discovery_started() -> None:
    ...
    ``kova_cli.mcp_startup`` captures the caller's context-local override
