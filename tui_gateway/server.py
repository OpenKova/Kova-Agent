...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("KOVA_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("kova_gateway_runtime_session_record", default=None)
)
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Kova agent → needs provider credentials.
        ...
        env = build_subprocess_env(
            kova_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from kova_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``kova dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("KOVA_COMPUTE_HOST_CHILD") == "1"
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``kova`` / ``kova --tui`` / CLI) is started from a
    ...
    try:
        ...
        from kova_state import is_disk_full_error
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``kova_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    ``kova_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read KOVA_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # KOVA_SESSION_ID even though agent_init set it via
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``kova config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Kova activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``KOVA_DESKTOP=1`` and ``KOVA_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``KOVA_DESKTOP_TERMINAL=1`` → "tui"
        (``kova --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``kova --tui``.)
    """
    if is_truthy_value(os.environ.get("KOVA_DESKTOP")) and not is_truthy_value(
        os.environ.get("KOVA_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads KOVA_MODEL /
    KOVA_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`kova --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads KOVA_MODEL /
    # KOVA_INFERENCE_MODEL, which `kova --tui -m <model>` sets as a
...
def _load_approval_mode() -> str:
    ...
    canonical ``kova_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`kova_constants.resolve_reasoning_config` (per-model override >
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_KOVA_CORE_TOOLS`` — every other
    ...
    paths run with ``KOVA_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the Kova desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _KOVA_CORE_TOOLS (every other
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `kova --tui -m`
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / KOVA_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    ...
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_kova_home)
...
# (agent/billing_view.py + kova_cli/nous_billing.py) — same as /topup.
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=KOVA_SESSION_KEY`` (see
    ...
    # DB when KOVA_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``KOVA_UI_SESSION_ID`` as
...
def _session_images_dir(session: dict) -> Path:
    ...
    override, so ``get_kova_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _kova_home
    return base / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from kova_cli.model_switch import parse_model_switch_args
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under HERMES_HOME (~/.kova by default) — config/sessions/skills,
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    kova_home = os.path.normcase(os.path.realpath(str(get_kova_home())))
    return real == home or real == kova_home
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Kova sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Kova
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Kova" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``kova_cli.voice.set_voice_busy_probe``)
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via kova_cli.voice.speak_text with spoken barge-in.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            # are tolerated.
            try:
                from kova_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import kova_cli.voice  # noqa: F401
